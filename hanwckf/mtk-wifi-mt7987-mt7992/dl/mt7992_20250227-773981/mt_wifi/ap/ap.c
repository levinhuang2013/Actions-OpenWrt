...
#define DUAL_BAND   1
...

	{
		...
		pChOpInfo->target_ch = NewChannel;
		pChOpInfo->target_bw = wlan_get_target_bw(wdev, pChOpInfo->target_ch, wlan_config_get_bw(wdev));
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"target ch = %d, target bw = %d\n", pChOpInfo->target_ch, pChOpInfo->target_bw);
...
VOID APStartUpForMbss(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	if (wdev->func_idx == 0)
		MgmtTableSetMcastEntry(pAd, MCAST_WCID_TO_REMOVE, wdev, FALSE);
...
VOID MacTableMaintenance(RTMP_ADAPTER *pAd)
{
...
#endif /* MT_MAC */
	UINT16 PsStaNumByBand[MAX_BAND_NUM] = {0};
	UINT BandIdxByband = 0;
	UINT16 TkUsedThisBand = 0;
	UINT16 FlushPleThres = 0xFFFF;
	...
	call_traffic_notifieriers(TRAFFIC_NOTIFY_TPUT_DETECT, pAd, pEntry);
		...
		*/
		if (pEntry->PsMode == PWR_SAVE) {
			...
			BandIdxByband = HcGetBandByWdev(pEntry->wdev);
			PsStaNumByBand[BandIdxByband]++;
		...
		if (pEntry->NoDataIdleCount >= sta_idle_timeout) {
...
#endif
			{
				...
				if (IS_ENTRY_CLIENT(pEntry) && IS_MAP_ENABLE(pAd) && IS_MAP_R2_ENABLE(pAd))
					pEntry->DisconnectReason = REASON_DISASSOC_INACTIVE;
#endif
			}
		}
		...
		if (bDisconnectSta && (pMbss != NULL)) {
			...
			record_connect_info(pAd, pEntry->Addr, REASON_DEAUTH_STA_LEAVING, FALSE);
	...
	/*
		Default disable ps ple flush
		pAd->Mlme.PsFlushPeriod = 0
		pAd->Mlme.PsFlushSTAThreshold = 5
		pAd->Mlme.PsFlushTokenThreshold = 95
		pAd->Mlme.PsFlushPLEThreshold = 50
	*/
	if (pAd->Mlme.PsFlushPeriod && (pAd->Mlme.PeriodicRound % pAd->Mlme.PsFlushPeriod == 0)
		&& (pAd->MacTab->Size > pAd->Mlme.PsFlushSTAThreshold)) {
		if (BandIdx < MAX_BAND_NUM && PsStaNumByBand[BandIdx]) {
			hc_get_tx_sw_used_token_by_band(pAd, &TkUsedThisBand);
			if (TkUsedThisBand > (pMacTable->MaxToken[BandIdx] * pAd->Mlme.PsFlushTokenThreshold / 100)) {
				if (!FlushFlag) {
					FlushFlag = TRUE;
					if (FlushPleThres > (pMacTable->MaxToken[BandIdx] * pAd->Mlme.PsFlushPLEThreshold / 100) / PsStaNumByBand[BandIdx])
						FlushPleThres = (pMacTable->MaxToken[BandIdx] * pAd->Mlme.PsFlushPLEThreshold / 100) / PsStaNumByBand[BandIdx];
					if (FlushPleThres < 32)
						FlushPleThres = 32;
				}
			}
		...
		if (FlushFlag) {
			switch (BandIdx) {
			case 0:
				RTMP_FLUSH_PS_STA_PLE(pAd, FlushPleThres, 0x1);
				break;
			case 1:
				RTMP_FLUSH_PS_STA_PLE(pAd, FlushPleThres, 0x2);
				break;
			case 2:
				RTMP_FLUSH_PS_STA_PLE(pAd, FlushPleThres, 0x4);
				break;
			default:
				break;
			}
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_INFO,
				"Flush PLE buffer[token used:%d, FlushPleThres:%d, band:(%d)]\n",
				TkUsedThisBand, FlushPleThres, BandIdx);
		}
	}
...
#ifdef EM_PLUS_SUPPORT
				Reason = REASON_PREV_AUTH_INVALID;
#else
#ifdef MAP_R5
...
#endif
			record_connect_info(pAd, pEntry->Addr, REASON_DECLINED, FALSE);
			mac_entry_delete(pAd, pEntry, TRUE);
		}
	}
}
	...
	OUT struct eht_mld_info *reply_mld)
{
	...
	reply_mld->mld_grp = mld->mld_grp;
	os_move_mem(reply_mld->mld_addr, mld->mld_addr, MAC_ADDR_LEN);
	reply_mld->mld_type = mld->mld_type;
	...
	for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
		...
		aps->valid = 1;
		os_move_mem(aps->bssid, bss->pwdev->bssid, MAC_ADDR_LEN);
		aps->link_id = bss->link_id;
		if (wlan_config_get_eht_dscb_enable(bss->pwdev)) {
			aps->dis_subch_enable = 1;
			aps->dis_subch_bitmap = wlan_config_get_eht_dis_subch_bitmap(bss->pwdev);
		} else
			aps->dis_subch_enable = 0;

		aps->emlsr_mr = wlan_config_get_emlsr_mr(bss->pwdev);
	}

	return 0;
}

INT eht_ap_mld_get_by_mld_addr(
	IN struct wifi_dev *wdev,
	IN u8 *mld_addr,
	OUT struct eht_mld_info *reply_mld)
{
	...
	if (!wdev || !mld_addr || !reply_mld) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"invalid wdev:%p mld_addr:%p reply_mld:%p\n", wdev, mld_addr, reply_mld);
			return -RTMP_IO_EINVAL;
	}
	...
	return eht_ap_mld_get(wdev, mld_basic.mld_grp_idx, reply_mld);
}

INT eht_ap_mld_set_addr(struct wifi_dev *wdev, u8 mld_group_idx, u8 *mld_addr)
{
	int ret;
	struct eht_mld_info mld;
	struct eht_mld_info mld2;
	struct _RTMP_ADAPTER *pAd = NULL;

	if (!wdev || !mld_addr) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"invalid wdev:%p mld_addr:%p\n", wdev, mld_addr);
		return NDIS_STATUS_FAILURE;
	}

	pAd = (struct _RTMP_ADAPTER *)wdev->sys_handle;

	ret = eht_ap_mld_get(wdev, mld_group_idx, &mld);
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"mld_group_idx:%u not exist\n", mld_group_idx);
		return NDIS_STATUS_FAILURE;
	}

	if (memcmp(mld.mld_addr, mld_addr, MAC_ADDR_LEN) == 0)
		return NDIS_STATUS_SUCCESS;

	ret = eht_ap_mld_get_by_mld_addr(wdev, mld_addr, &mld2);
	if (ret == NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"mld mld_addr:%pM  exist\n", mld_addr);
		return NDIS_STATUS_FAILURE;
	}

	ret = bss_mngr_mld_group_addr_set(wdev, mld_group_idx, mld_addr);
	if (ret  != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"mld idx:%u set mld_addr:%pM fail, ret:%d\n", mld_group_idx, mld_addr, ret);
		return NDIS_STATUS_FAILURE;
	}
...
void record_connect_info(PRTMP_ADAPTER pAd, UCHAR *addr,
	UINT32 Reason, BOOLEAN is_connect)
{
	if (is_connect) {
		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			 "(caller:%pS): WLAN station connection establishment to %s Wifi radio failed: association error. StatusCode: %d, MAC-address: "MACSTR"\n",
			 OS_TRACE, get_band_str(hc_get_hw_band_idx(pAd)), Reason, MAC2STR(addr));
	} else {
		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			 "(caller:%pS): WLAN station disconnected from %s Wifi radio, MAC-address: "
			 MACSTR", reason=%d\n",
			 OS_TRACE, get_band_str(hc_get_hw_band_idx(pAd)), MAC2STR(addr), Reason);
	}
}
