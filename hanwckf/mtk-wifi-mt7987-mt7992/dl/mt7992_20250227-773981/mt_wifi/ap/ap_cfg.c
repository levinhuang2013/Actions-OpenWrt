...
INT show_pmkid_info(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
INT set_arp_flow_control_proc(PRTMP_ADAPTER pAd, RTMP_STRING *arg);
...
struct PRIV_SET_PROC RTMP_PRIVATE_AP_SUPPORT_PROC[] = {
	...
	{"TxCmdEDCA",				Set_TxCmdEDCA_Proc},
	{"PerStaWMM",				Set_PerStaWmm_Proc},
	...
	{"RoamStaCalib",		Set_RoamStaCalib_Proc},
...
#endif /* DBG_ENHANCE */
#endif /* DBG */
	{"ba_iot_test",					set_ba_iot_test_action},
	...
	*/
	...
	{"UnicastAll",				Set_UnicastAll_Enable_Proc},
...
#endif /* MCAST_RATE_SPECIFIC */
	{"arpflowcontrol",			set_arp_flow_control_proc},
#endif /* CONFIG_AP_SUPPORT */
...
#endif
	{"FixedRatePerAcReg",		Set_Fixed_Rate_Per_AC_Reg_Proc},
	{"FixedRatePerAcUnReg",		Set_Fixed_Rate_Per_AC_UnReg_Proc},
	{"FixedRatePerAcQuery",		UniCmdRAGetFixedRatePerAcProc},
	...
	{"AutoRateBW",              Set_AutoRate_BW_Limited},
#endif /* DBG */
#endif /* MT_MAC */
	...
	*/
	...
	{"psflush", Set_PsFlush_Proc},
	{"psflushnum", Set_PsFlushNum_Porc},
#ifdef RLM_CAL_CACHE_SUPPORT
...
INT Channel_Info_MsgHandle(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, POS_COOKIE pObj)
{
	...
	if (wrq->u.data.length != sizeof(OFFCHANNEL_SCAN_MSG)) {
		...
		case TRIGGER_DRIVER_CHANNEL_SWITCH:
			...
			/*non-csa flow should release ChannelOpCharge here,
			CSA flow will release ChannelOpCharge when CSA count down to zero*/
...
INT Show_VLAN_GTK_Info_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < WTBL_MAX_NUM(pAd); i++) {
		...
		if (pEntry->vlan_id > 0) {
			MTWF_PRINT("STA %d %02x:%02x:%02x:%02x:%02x:%02x vlan_id %d:\n",
				i, PRINT_MAC(pEntry->Addr), pEntry->vlan_id);
		}
	}
	...
	IN	RTMP_STRING *arg)
{
	...
	pAd->ApCfg.iwpriv_event_flag = TRUE;
	if (!AutoChSelScanStart(pAd, pwdev)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
			"AutoChSelScanStart fail for ACS!!\n");
		return FALSE;
	}
	/*non-csa flow should release ChannelOpCharge here,
	CSA flow will release ChannelOpCharge when CSA count down to zero*/
...
/*
   ...
   Arguments:
    ...
    Note:
	Usage:
		iwpriv ra0 set ACSCheckTime=second
...
static INT show_apcfg_info(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	ret = snprintf(pos, end-pos, "show ap cfg info:\n");
	...
	BandIdx = HcGetBandByWdev(wdev);
	ret = snprintf(pos, end-pos, "%-24s%-16s%-8s\n", " ", "WIFI_DRIVER", "PEAK_VALUE");
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
	/*WirelessMode*/
	...
	if (cfg_mode == 16) {
		ret = snprintf(pos, end-pos, "%-24s%-16ld%ld\n", "WirelessMode", cfg_mode,
		...
		ret = snprintf(pos, end-pos, "%-24s%-16ld%ld/%ld\n", "WirelessMode", cfg_mode,
	...
	ret = snprintf(pos, end-pos,
		...
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"TxPower", apcfg_para_setting.tx_power_percentage, apcfg_for_peak.tx_power_percentage,
		"WifiCert", apcfg_para_setting.wifi_cert, apcfg_for_peak.wifi_cert,
		"TxPreamble", apcfg_para_setting.tx_preamble, apcfg_for_peak.tx_preamble,
		"RTSThreshold(config)", apcfg_para_setting.conf_len_thld, apcfg_for_peak.conf_len_thld,
		"RTSThreshold(operate)", apcfg_para_setting.oper_len_thld, apcfg_for_peak.oper_len_thld,
		"FragThreshold(config)", apcfg_para_setting.conf_frag_thld, apcfg_for_peak.conf_frag_thld,
		"FragThreshold(operate)", apcfg_para_setting.oper_frag_thld, apcfg_for_peak.oper_frag_thld,
		"TxBurst", apcfg_para_setting.bEnableTxBurst, apcfg_for_peak.bEnableTxBurst,
		"ShortSlot", apcfg_para_setting.bUseShortSlotTime, apcfg_for_peak.bUseShortSlotTime);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
	...
	ret = snprintf(pos, end-pos,
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"HT_BW(config)", apcfg_para_setting.conf_ht_bw, apcfg_for_peak.conf_ht_bw,
		"HT_BW(operate)", apcfg_para_setting.oper_ht_bw, apcfg_for_peak.oper_ht_bw);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
	...
	ret = snprintf(pos, end-pos, "%-24s%-16d%d\n", "HT_BSSCoexistence",
	...
	ret = snprintf(pos, end-pos,
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"HT_TxStream", apcfg_para_setting.ht_tx_streams, apcfg_for_peak.ht_tx_streams,
		"HT_RxStream", apcfg_para_setting.ht_rx_streams, apcfg_for_peak.ht_rx_streams);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;

#ifdef DOT11_HE_AX
	/*HE_TxStream */ /*HE_RxStream */
	...
	ret = snprintf(pos, end-pos,
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"HE_TxStream", apcfg_para_setting.ht_tx_streams, apcfg_for_peak.ht_tx_streams,
		"HE_RxStream", apcfg_para_setting.ht_rx_streams, apcfg_for_peak.ht_rx_streams);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
#endif
#ifdef DOT11_EHT_BE
	/*EHT_TxStream */ /*EHT_RxStream */
	...
	ret = snprintf(pos, end-pos,
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"EHT_TxStream", apcfg_para_setting.ht_tx_streams, apcfg_for_peak.ht_tx_streams,
		"EHT_RxStream", apcfg_para_setting.ht_rx_streams, apcfg_for_peak.ht_rx_streams);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
#endif
	...
	ret = snprintf(pos, end-pos,
		...
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"HT_BADecline", apcfg_para_setting.ba_decline, apcfg_for_peak.ba_decline,
		"HT_AutoBA", apcfg_para_setting.ba_en, apcfg_for_peak.ba_en,
		"HT_AMSDU", apcfg_para_setting.AmsduEnable, apcfg_for_peak.AmsduEnable,
		"HT_BAWinSize", apcfg_para_setting.ba_rx_wsize, apcfg_for_peak.ba_rx_wsize,
		"HT_GI", apcfg_para_setting.ht_gi, apcfg_for_peak.ht_gi,
		"HT_STBC", apcfg_para_setting.ht_stbc, apcfg_for_peak.ht_stbc,
		"HT_LDPC", apcfg_para_setting.ht_ldpc, apcfg_for_peak.ht_ldpc,
		"HT_RDG", apcfg_para_setting.bRdg, apcfg_for_peak.bRdg);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
#endif
	...
	if (WMODE_CAP_5G(wmode)) {
		...
		ret = snprintf(pos, end-pos,
			...
			"%-24s%-16d%d\n"
			"%-24s%-16d%d\n",
			"VHT_BW(config)", apcfg_para_setting.conf_vht_bw, apcfg_for_peak.conf_vht_bw,
			"VHT_BW(operate)", apcfg_para_setting.oper_vht_bw, apcfg_for_peak.oper_vht_bw,
			"VHT_SGI", apcfg_para_setting.vht_sgi, apcfg_for_peak.vht_sgi,
			"VHT_STBC", apcfg_para_setting.vht_stbc, apcfg_for_peak.vht_stbc,
			"VHT_BW_SIGNAL", apcfg_para_setting.vht_bw_signal, apcfg_for_peak.vht_bw_signal,
			"VHT_LDPC", apcfg_para_setting.vht_ldpc, apcfg_for_peak.vht_ldpc,
			"Vht1024QamSupport", apcfg_para_setting.vht_1024_qam, apcfg_for_peak.vht_1024_qam);
		if (os_snprintf_error(end-pos, ret))
			return FALSE;
		pos += ret;
	}
	...
	if (WMODE_CAP_5G(wmode)) {
		...
		ret = snprintf(pos, end-pos,
			...
			"%-24s%-16d\n"
			"%-24s%-16d\n",
			"DfsEnable", apcfg_para_setting.bDfsEnable, apcfg_for_peak.bDfsEnable,
			"CACMemoEn", pAd->CommonCfg.DfsParameter.CACMemoEn,
			"DfsPreCac", pAd->CommonCfg.DfsParameter.bPreCacEn);
		if (os_snprintf_error(end-pos, ret))
			return FALSE;
		pos += ret;
	}
	...
	ret = snprintf(pos, end-pos,
		...
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"MuOfdmaDlEnable", apcfg_para_setting.MuOfdmaDlEnable, apcfg_for_peak.MuOfdmaDlEnable,
		"MuOfdmaUlEnable", apcfg_para_setting.MuOfdmaUlEnable, apcfg_for_peak.MuOfdmaUlEnable,
		"MuMimoDlEnable", apcfg_para_setting.MuMimoDlEnable, apcfg_for_peak.MuMimoDlEnable,
		"MuMimoUlEnable", apcfg_para_setting.MuMimoUlEnable, apcfg_for_peak.MuMimoUlEnable);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
	...
	ret = snprintf(pos, end-pos,
			...
			"%-24s%-16d%d\n"
			"%-24s%-16lu%lu\n",
			"CommonCfg.ETxBfEnCond", apcfg_para_setting.ETxBfEnCond, apcfg_for_peak.ETxBfEnCond,
			"ETxBfEnCond", apcfg_para_setting.ETxBfEnCond, apcfg_for_peak.ETxBfEnCond,
			"CommonCfg.ITxBfEn", apcfg_para_setting.ITxBfEn, apcfg_for_peak.ITxBfEn,
			"ITxBfEn", apcfg_para_setting.ITxBfEn, apcfg_for_peak.ITxBfEn,
			"MUTxRxEnable", apcfg_para_setting.MUTxRxEnable, apcfg_for_peak.MUTxRxEnable);
		if (os_snprintf_error(end-pos, ret))
			return FALSE;
		pos += ret;
#endif
	...
	ret = snprintf(pos, end-pos,
		"%-24s%-16d%d\n"
		"%-24s%-16d%d\n",
		"HE_BW(config)", apcfg_para_setting.conf_he_bw,
		(WMODE_CAP_AX_6G(wmode) ? HE_BW_160 :
		(WMODE_CAP_AX_5G(wmode) ? HE_BW_160 : HE_BW_2040)),
		"HE_BW(operate)", apcfg_para_setting.oper_he_bw,
		(WMODE_CAP_AX_6G(wmode) ? HE_BW_160 :
		(WMODE_CAP_AX_5G(wmode) ? HE_BW_160 : HE_BW_2040)));
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
...
#endif
#ifdef MLR_SUPPORT
	apcfg_para_setting.bMLREnable = pAd->CommonCfg.bMLREnable;
	apcfg_para_setting.MLRVersion = pAd->CommonCfg.MLRVersion;
	MTWF_PRINT("%-24s%-16d\n", "MLREnable",
			  apcfg_para_setting.bMLREnable);
	MTWF_PRINT("%-24s0x%-16x\n", "MLRVersion",
			  apcfg_para_setting.MLRVersion);
#endif
	...
	ret = snprintf(pos, end-pos, "%-24s%-16u\n", "current channel",
	...
	ret = snprintf(pos, end-pos, "%-24s%-16s\n", "extension channel", str);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;
	return TRUE;
...
INT Show_Eap_Stats_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	UCHAR i, str[15] = "";
	INT ret = 0;
	UINT64 avg_transmit = 0, ret_avg_dly[16] = {0};
#ifdef EAP_ENHANCE_STATS_SUPPORT
	UINT64 avg_latency = 0, avg_cnt = 0;
	...
	if (!VALID_UCAST_ENTRY_WCID(pAd, Wcid) || (Wcid >= WTBL_MAX_NUM(pAd))) {
		...
		if (IS_VALID_ENTRY(pEntry) && IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC)) {
			...
			if (pEntry->tx_pkt_num) {
				avg_transmit = pEntry->tx_latency_avg / pEntry->tx_pkt_num;
#ifdef EAP_ENHANCE_STATS_SUPPORT
				avg_latency = pEntry->air_latency_avg / pEntry->tx_pkt_num;
				avg_cnt = pEntry->tx_cnt_avg / pEntry->tx_pkt_num;
#endif /* EAP_ENHANCE_STATS_SUPPORT */
			}

			MTWF_PRINT("transmit delay:  min/max/avg = %d/%d/%lld (TU)\n",
				pEntry->tx_latency_min, pEntry->tx_latency_max, avg_transmit);
#ifdef EAP_ENHANCE_STATS_SUPPORT
			MTWF_PRINT("tranmit cnt:  min/max/avg = %d/%d/%lld\n",
				pEntry->tx_cnt_min, pEntry->tx_cnt_max, avg_cnt);
			MTWF_PRINT("\nair delay: min/max/avg = %d/%d/%lld (TU)\n",
				pEntry->air_latency_min, pEntry->air_latency_max, avg_latency);
#endif /* EAP_ENHANCE_STATS_SUPPORT */

			MTWF_PRINT("\n%-10s%-10s%-10s\n", "tx times", "pkt cnt", "avg delay");
			for (i = 1; i < 16; i++) {
				if (pEntry->tx_pkt_ret[i])
					ret_avg_dly[i] = pEntry->tx_pkt_ret_dly_avg[i] / pEntry->tx_pkt_ret[i];
				MTWF_PRINT("%-10hhd%-10lld%-10lld\n", i, pEntry->tx_pkt_ret[i], ret_avg_dly[i]);
			}

			MTWF_PRINT("\n%-15s%-10s\n", "tx delay(ms)", "pkt cnt");
			for (i = 0; i < 16; i++) {
				if (i == 0)
					ret = snprintf(str, sizeof(str), "x <= %hhd", i);
				else if (i < 15)
					ret = snprintf(str, sizeof(str), "%hhd < x <=%hhd", i-1, i);
				else
					ret = snprintf(str, sizeof(str), "x > %hhd", i-1);
				if (os_snprintf_error(sizeof(str), ret)) {
					MTWF_PRINT("%s: snprintf error!\n", __func__);
					continue;
				}
				MTWF_PRINT("%-15s%-10lld\n", str, pEntry->tx_pkt_hist[i]);
			}

			/* reset cnt */
			MTWF_PRINT("\nReset sta all cnt!\n");
#ifdef EAP_ENHANCE_STATS_SUPPORT
			pEntry->air_latency_min = 0;
			pEntry->air_latency_max = 0;
			pEntry->air_latency_avg = 0;
			pEntry->tx_cnt_min = 0;
			pEntry->tx_cnt_max = 0;
			pEntry->tx_cnt_avg = 0;
#endif /* EAP_ENHANCE_STATS_SUPPORT */
			pEntry->tx_latency_min = 0;
			pEntry->tx_latency_max = 0;
			pEntry->tx_latency_avg = 0;
			pEntry->tx_pkt_num = 0;
			for (i = 0; i < 16; i++) {
				pEntry->tx_pkt_ret[i] = 0;
				pEntry->tx_pkt_ret_dly_avg[i] = 0;
				pEntry->tx_pkt_hist[i] = 0;
			}
		} else {
	...
	IN	RTMP_STRING *arg)
{
	...
	if (pEntry) {
		...
		if (IS_MAP_ENABLE(pAd) && IS_MAP_R2_ENABLE(pAd))
			pEntry->DisconnectReason = REASON_DEAUTH_STA_LEAVING;
#endif
		MlmeDeAuthAction(pAd, pEntry, REASON_DEAUTH_STA_LEAVING, FALSE);
		/*		MacTableDeleteEntry(pAd, pEntry->wcid, Addr); */
	}
	...
	IN RTMP_STRING *arg)
{
	...
	if (value == 2) {
		...
		MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_WARN,
			"[PMF] apidx=%d\n", pObj->ioctl_if);
		APMlmeKickOutAllSta(pAd, pObj->ioctl_if, REASON_DISASSOC_STA_LEAVING);
...
INT	show_pmkid_info(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 i = 0;
	struct _NDIS_AP_802_11_PMKID *pmkid_cache;
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		...
		if (pBssInfo->Valid) {
			MTWF_PRINT("Cacheidx=%d, own mac="MACSTR", peer mac="
				MACSTR", is_ft=%d, is_mlo_connect=%d\n",
				i, MAC2STR(pBssInfo->own_mac), MAC2STR(pBssInfo->peer_mac),
				pBssInfo->is_ft, pBssInfo->is_mlo_connect);
...
{
	union _HTTRANSMIT_SETTING *transmit, *bmcTransmit;
	BSS_INFO_ARGUMENT_T bss_info_argument;
	struct wifi_dev *wdev = NULL, *firstWdev = NULL;
	BOOLEAN status = TRUE;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UINT32 ratetype = 0, phymode = 0, mcs = 0, autorate = 0;
	INT i4Recv = 0, i = 0;
	UCHAR bandIdx, mbandIdx;
	BOOLEAN autoRateEn = FALSE;

	if (!arg)
		goto Error;

	i4Recv = sscanf(arg, "%d-%d-%d-%d", &(ratetype), &(phymode), &(mcs), &(autorate));
	if (i4Recv < 3 || i4Recv > 4)
		goto Error;
	...
	if (i4Recv == 4)
		autoRateEn = (autorate > 0) ? TRUE : FALSE;
	...
	if (autoRateEn)
		wdev->eap.eap_hprirate_en |= 0x80;
	else
		wdev->eap.eap_hprirate_en &= ~(0x80);

	MTWF_PRINT("highpri rateen(%x), autoRateEn(%hhu), type/phymode/mcs = %d-%d-%d\n",
		wdev->eap.eap_hprirate_en, autoRateEn, ratetype, phymode, mcs);

	/* Sync config to same band all bss */
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		/* Record first ready bss wdev */
		if (firstWdev == NULL && WDEV_BSS_STATE(mwdev) == BSS_READY)
			firstWdev = mwdev;
	}
	/* Sync config to same band apcli */
	for (i = 0; i < MAX_MULTI_STA; i++) {
		struct wifi_dev *mwdev = &pAd->StaCfg[i].wdev;
	...
	/* copy bc/mc rate setting */
#ifdef MCAST_VENDOR10_CUSTOM_FEATURE
	bmcTransmit = (wdev->channel > 14) ? (&wdev->rate.MCastPhyMode_5G) : (&wdev->rate.MCastPhyMode);
#else
	bmcTransmit = &wdev->rate.mcastphymode;
#endif
	NdisMoveMemory(&bss_info_argument.BcTransmit, bmcTransmit, sizeof(union _HTTRANSMIT_SETTING));
	NdisMoveMemory(&bss_info_argument.McTransmit, bmcTransmit, sizeof(union _HTTRANSMIT_SETTING));
	bss_info_argument.mloFixedRateIdx = wdev->bss_info_argument.mloFixedRateIdx;
	bss_info_argument.priv = wdev;

	if (AsicBssInfoUpdate(pAd, &bss_info_argument) != NDIS_STATUS_SUCCESS) {
		MTWF_PRINT("Fail to apply the bssinfo, BSS Idx = %d!\n", bss_info_argument.ucBssIndex);
		status = FALSE;
	}
...
Error:
	MTWF_PRINT("error input parameter!\n");
	MTWF_PRINT("iwpriv ra0 set highprirate=[ratetype]-[phymode]-[mcs]-[autorate]\n");
	MTWF_PRINT("[ratetype]: 0-ARP, 1-DHCP, 2-PING, 3-EAPOL\n");
	MTWF_PRINT("[phymode] : 0-Disable	[mcs]:ignore\n");
	MTWF_PRINT("[phymode] : 1-CCK	[mcs]:0~3\n");
	MTWF_PRINT("[phymode] : 2-OFDM	[mcs]:0~7\n");
	MTWF_PRINT("[autorate]: 0-Disable, 1-Enable, Set ARP/DHCP/PING Auto Rate\n");
	...
	IN RTMP_STRING *arg)
{
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		bss_info_argument.ucBssIndex = wdev->bss_info_argument.ucBssIndex;
		bss_info_argument.u8BssInfoFeature = BSS_INFO_BROADCAST_INFO_FEATURE;
		NdisMoveMemory(&bss_info_argument.BcTransmit, transmit, sizeof(union _HTTRANSMIT_SETTING));
		NdisMoveMemory(&bss_info_argument.McTransmit, transmit, sizeof(union _HTTRANSMIT_SETTING));
		bss_info_argument.mloFixedRateIdx = wdev->bss_info_argument.mloFixedRateIdx;
		bss_info_argument.priv = wdev;
...
INT set_arp_flow_control_proc(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	UINT arg_value = 0;
	UCHAR Enable;
	UINT16 threshold = 255;
	UCHAR disable_fr = 0;
	UINT ifIndex = 0;
	RTMP_STRING *arg0_ptr;
	RTMP_STRING *arg1_ptr;
	RTMP_STRING *arg2_ptr;

	pObj = (POS_COOKIE) pAd->OS_Cookie;
	ifIndex = pObj->ioctl_if;

	if ((pObj->ioctl_if_type == INT_MAIN) || (pObj->ioctl_if_type == INT_MBSSID)) {
		if (!((ifIndex < MAX_MBSSID_NUM(pAd)) && (ifIndex < MAX_BEACON_NUM))) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
					"invalid mbss id(%d)\n", ifIndex);
			return FALSE;
		}
	...
	if (arg0_ptr == NULL || arg1_ptr == NULL || arg2_ptr == NULL) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
			 " Invalid parameters\n");
		return FALSE;
	}

	arg_value = (UINT)os_str_tol(arg0_ptr, 0, 10);
	if (arg_value > 0)
		Enable = TRUE;
	else
		Enable = FALSE;

	threshold = (UINT)os_str_tol(arg1_ptr, 0, 10);

	arg_value = (UINT)os_str_tol(arg2_ptr, 0, 10);
	if (arg_value > 0)
		disable_fr = TRUE;
	else
		disable_fr = FALSE;
	...
	CmdArpFlowControlEnable(pAd, Enable, threshold, disable_fr);
...
#endif
		/* Comment out MtCmdSetA4Enable, because it is necessary to enable */
		/* CAP_CTRL bit 4 for MLO regardless of whether WDS is enabled or not. */
...
INT mtk_cfg80211_set_ai_antinoise(RTMP_ADAPTER *pAd, USHORT ai_antinoise)
{
	int ret = 0;

	pAd->CommonCfg.ai_antinoise = ai_antinoise;

	MTWF_PRINT("%s Set ai_antinoise %X\n", __func__, ai_antinoise);
	...
	char *arg)
{
	...
	if (map_mode == MAP_TURNKEY) {
		for (i = BSS0; (i < pAd->ApCfg.BssidNum) && VALID_MBSS(pAd, i); i++) {
			map_a4_init(pAd, i, TRUE);
			MTWF_PRINT("%s current A4 init is %d\n", __func__, pAd->ApCfg.MBSSID[i].a4_init);
		}
	} else {
		for (i = BSS0; (i < pAd->ApCfg.BssidNum) && VALID_MBSS(pAd, i); i++) {
			map_a4_deinit(pAd, i, TRUE);
			MTWF_PRINT("%s current A4 init is %d\n", __func__, pAd->ApCfg.MBSSID[i].a4_init);
		}
	}
	...
	* since upper layer should configure interface roles */
	...
	if (!IS_MAP_ENABLE(pAd)) {
		...
		for (ifIndex = 0; (ifIndex < MAX_MBSSID_NUM(pAd)) && (ifIndex < MAX_BEACON_NUM); ifIndex++) {
			...
			APMlmeKickOutAllSta(pAd, ifIndex, REASON_DISASSOC_STA_LEAVING);
#ifdef WSC_INCLUDED
			pWscControl = &pAd->ApCfg.MBSSID[ifIndex].wdev.WscControl;
#endif /* WSC_INCLUDED */
		...
		UCHAR type, UCHAR interval, UCHAR tx_mode)
{
	...
	if (WDEV_BSS_STATE(wdev) == BSS_READY) {
		...
		/* sync to bss manager */
		bss_mngr_update_he_bss_discov(wdev, BSS_IOB_DISCOV_UPDATE);
...
BOOLEAN out_band_discovery_update(struct wifi_dev *wdev, UCHAR rnr_6g_in_probe)
{
	...
	if (WDEV_BSS_STATE(wdev) == BSS_READY) {
		/* sync to bss manager */
		bss_mngr_update_he_bss_discov(wdev, BSS_OOB_DISCOV_UPDATE);
	}
