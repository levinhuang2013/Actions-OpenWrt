...
/**
* ChPrioEnableCheck - check if channel priority enable or not.
* @pAd: pointer of the RTMP_ADAPTER
**/
BOOLEAN ChPrioEnableCheck(CHANNEL_CTRL *pChCtrl)
{
	if (pChCtrl->ChListNum == 0) {
		MTWF_DBG(NULL, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"channel list num is 0\n");
		return FALSE;
	}

	/*check if ch prio enabled*/
	return pChCtrl->ChList[0].Priority == CHAN_PRIO_UNSPECIFIED ?
			FALSE : TRUE;
}
