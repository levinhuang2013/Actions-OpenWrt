...
static inline BOOLEAN is_tcp_ctrl_frame(IN PNDIS_PACKET	pPacket)
{
	if (RTMP_GET_PACKET_TCP(pPacket)) {
		...
		if (*(pSrcBuf + LENGTH_802_3 + IP_HDR_LEN + 13) & (BIT(2) | BIT(5) | BIT(6) | BIT(7)))
			return TRUE;
	}
	...
	IN PUCHAR pMacAddr)
{
	if (((pMacAddr[0] == 0x01)
		...
		&& (pMacAddr[3] == 0x00)
		&& (pMacAddr[4] == 0x00)
		&& ((pMacAddr[5] == 0x0e) || (pMacAddr[5] == 0x03) || (pMacAddr[5] == 0x00) || (pMacAddr[5] == 0x13)))
		)
		return TRUE;
	...
	*/
	...
	if (pHeader_802_11->FC.Type == FC_TYPE_MGMT) {
		...
		if (wdev->eap.eap_mgmrate_en >= (1 << MGMT_TYPE)) {
			...
			NdisMoveMemory(&tmp_transmit, transmit, sizeof(union _HTTRANSMIT_SETTING));
			transmit = &tmp_transmit;
			tx_rate = (UCHAR)transmit->field.MCS;
			...
			if (pMacEntry && pMacEntry->mlo.mlo_en) {
				if (pMacEntry->mld_entry)
					is_single_link = (pMacEntry->mld_entry->link_num == 1) ? TRUE : FALSE;

				if (is_single_link || TX_BLK_TEST_FLAG(tx_blk, fTX_ForceLink)
					|| RTMP_GET_PACKET_TYPE(tx_blk->pPacket) == TX_ALTX)
					MTWF_DBG(pAd, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_INFO,
						"Follow mgmt rate setting, fr_idx(%hhu)\n", tx_blk->fr_tbl_idx);
				else {
					if (transmit->field.MODE == MODE_CCK) {
						tx_blk->fr_tbl_idx = wdev->bss_info_argument.mloFixedRateIdx;
						MTWF_DBG(pAd, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_INFO,
							"Should use MLO Fixed Rate Idx(%hhu) in CCK Mode!\n", tx_blk->fr_tbl_idx);
					}
				}
			}
#endif /* DOT11_EHT_BE */
	...
	*/
	...
	if (pHeader_802_11->FC.Type == FC_TYPE_MGMT) {
		...
		case SUBTYPE_BLOCK_ACK_REQ:
			...
			if (wdev->eap.eap_mgmrate_en & (1 << MGMT_TYPE)) {
				transmit = &wdev->eap.mgmphymode[MGMT_TYPE];
				NdisMoveMemory(&TransmitSetting, transmit, sizeof(union _HTTRANSMIT_SETTING));
				transmit = &TransmitSetting;
				tx_blk->fr_tbl_idx = transmit_to_fr_idx(transmit->word);
#ifdef DOT11_EHT_BE
				if (pMacEntry && pMacEntry->mlo.mlo_en) {
...
INT ap_tx_pkt_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *tx_blk)
{
	...
	if (pDfsParam && pDfsParam->CSARadarDetect)
		mode_value = 1;

	if (pDot11h->ChannelMode != CHAN_NORMAL_MODE &&
			!(pDot11h->ChannelMode == CHAN_SWITCHING_MODE && mode_value == 0)) {
		struct _RTMP_CHIP_CAP *cap = PD_GET_CHIP_CAP_PTR(pAd->physical_dev);
		UINT8 tx_hw_hdr_len = cap->tx_hw_hdr_len;
		UCHAR *pData = GET_OS_PKT_DATAPTR(tx_blk->pPacket);
		HEADER_802_11 *pHeader_802_11 = (HEADER_802_11 *)(pData + tx_hw_hdr_len);
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry)) {
		...
		if (IS_ENTRY_CLIENT(pEntry))
			ApSendConnFailMsg(pAd,
				...
				REASON_CLS2ERR);
#endif
		record_connect_info(pAd, pEntry->Addr, REASON_CLS2ERR, FALSE);
...
#endif
		/*ApLogEvent(pAd, pAddr, EVENT_DISASSOCIATED); */
		record_connect_info(pAd, pEntry->Addr, Reason, FALSE);
		mac_entry_delete(pAd, pEntry, TRUE);
	}
...
#endif /* TXRX_STAT_SUPPORT */
static UCHAR is_bcast_attack(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk)
{
	...
	if (FC && (FC->Type == FC_TYPE_DATA) && (pRxBlk->Addr1 != NULL) && IS_BROADCAST_MAC_ADDR(pRxBlk->Addr1)) {
		pEntry = MacTableLookup(pAd, pRxBlk->Addr2);

		if (pEntry && IS_ENTRY_CLIENT(pEntry) && IS_AKM_WPA_CAPABILITY(pEntry->SecConfig.AKMMap) && (pEntry->Sst == SST_ASSOC) && (VALID_UCAST_ENTRY_WCID(pAd, pEntry->wcid))) {
			tr_entry = &pAd->MacTab->Content[pEntry->wcid].tr_entry;

			if (tr_entry && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED) && (FC->Wep == 0))
				return TRUE;
		}
	}
	return FALSE;
...
INT ap_rx_pkt_allowed(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _RX_BLK *pRxBlk)
{
...
#ifdef OPENSYNC_WDS
					&& pFmeCtrl->SubType != SUBTYPE_DATA_NULL
#endif
					)
#endif
				{
	...
	if (is_bcast_attack(pAd, pRxBlk) == TRUE) {
		tr_cnt->rx_invalid_wcid_drop++;
		MTWF_DBG(pAd, DBG_CAT_RX, CATRX_RXINFO, DBG_LVL_WARN,
			"TA is ported secured, but receive a plaintext broadcast frame after 4-way HS, drop!\n\n");
		return FALSE;
		}
	if (!pEntry) {
#ifdef IDS_SUPPORT
...
INT ap_rx_pkt_foward(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pPacket)
{
	...
	if (pMbss && (pHeader802_3[0] & 0x01)) {
		...
		/* LLDP pkt handled by os or 1905*/
		if (isLLDP_or_1905(pHeader802_3)) {
			to_air = FALSE;
		}
	} else {
...
#ifdef DYNAMIC_VLAN_SUPPORT
		pSrcEntry = MacTableLookupForTx(pAd, pHDR->SAAddr2, wdev);
		if (pSrcEntry && pSrcEntry->vlan_id) {
			UCHAR *data_p;
			UINT16 TCI;
			UINT16 TypeLen;

			data_p = GET_OS_PKT_DATAPTR(pForwardPacket);
			TypeLen = (data_p[12] << 8) | data_p[13];
			TCI = (pSrcEntry->vlan_id & 0x0fff) | ((0 & 0x7) << 13);
			if (TypeLen != ETH_TYPE_EAPOL)
				pForwardPacket = RtmpOsVLANInsertTag(pForwardPacket, TCI);
		}
		if (pForwardPacket == NULL)
			return to_os;
#endif /* DYNAMIC_VLAN_SUPPORT */
...
#endif
		{
			RTMP_SET_PACKET_WIFI_FORWARD(pForwardPacket, TRUE);
#ifdef A4_CONN
			RTMP_SET_PACKET_A4_FWDDATA(pForwardPacket, TRUE);
#endif
			RTMPSendPackets((NDIS_HANDLE)wdev, (PPNDIS_PACKET) & pForwardPacket, 1,
...
INT ap_ieee_802_3_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_WDS(pEntry))
		...
		if (protoType == ETH_P_IP)
			IGMPSnooping(pAd, pDA, pSA, pData, pDataEnd, pEntry, pRxBlk);
		else if (protoType == ETH_P_IPV6)
			MLDSnooping(pAd, pDA, pSA,  pData, pDataEnd, pEntry, pRxBlk);
	}
...
INT ap_ieee_802_11_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	*/
	...
	/*4-address null trigger WDS*/
#if defined(OPENSYNC_WDS) && defined(MWDS)
	if (pFmeCtrl->SubType & SUBTYPE_DATA_NULL) {
		if (pFmeCtrl->FrDs && pFmeCtrl->ToDs) {
			if (IS_WCID_VALID(pAd, pEntry->wcid)
				&& wdev->bSupportMWDS) {
				pEntry->bSupportMWDS = 1;
				if (tr_entry && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
					MlmeEnqueueWithWdev(pAd, OPENSYNC_WDS_MACHINE, 0,
						pRxBlk->DataSize, pData, wdev->func_idx, wdev, FALSE, NULL);
			}
		}
	}
#endif
