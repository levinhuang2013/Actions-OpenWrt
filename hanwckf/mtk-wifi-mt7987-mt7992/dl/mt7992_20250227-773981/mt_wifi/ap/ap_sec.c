	...
	IN struct wifi_dev *wdev)
{
	...
	/*
	   ...
	   IEEE 802.11 2020/P.268 -
	   IEEE Std 802.11 provides the following security protocols for protection
	   of individually addressed robust Management frames: CCMP and GCMP.
	 */
	 ...
	 */
		pSecConfig->PmfCfg.MFPC = TRUE;
		if (!(IS_AKM_WPA3(pSecConfig->AKMMap) || IS_AKM_WPA3_192BIT(pSecConfig->AKMMap))
			&& (IS_AKM_WPA2(pSecConfig->AKMMap) || IS_AKM_WPA2PSK(pSecConfig->AKMMap)))
			pSecConfig->PmfCfg.MFPR = FALSE;
		else
			pSecConfig->PmfCfg.MFPR = TRUE;
		...
		&& (IS_CIPHER_CCMP_OR_GCMP(pSecConfig->PairwiseCipher))
	...
	IN PVOID SystemSpecific3)
{
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(ad, i); i++) {
		...
		if (IS_ENTRY_CLIENT(pEntry) &&
			...
			MlmeDeAuthAction(ad, pEntry, REASON_DEAUTH_STA_LEAVING, FALSE);
		}
	}
}
	...
	IN RTMP_STRING * arg)
{
...
#ifdef CFG_RSNO_SUPPORT
			if (wdev->rsneo_len || wdev->rsneo2_len)
	...
	for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
		...
		if (tr_entry->PortSecured < ARRAY_SIZE(portsecured))
			MTWF_PRINT(" %d\t%d\t%s\t\t%s\t\t%s\t\t%d\t\t%s",
					 ...
					 portsecured[tr_entry->PortSecured]);
#ifdef DOT11W_PMF_SUPPORT
		MTWF_PRINT("\t%d/%d", pSecConfig->PmfCfg.MFPC, pSecConfig->PmfCfg.MFPR);
#endif
#ifdef DISABLE_HOSTAPD_BEACON
		MTWF_PRINT("/%02x%02x", pSecConfig->RsnCap[0], pSecConfig->RsnCap[1]);
	...
	for (idx = 0; idx < MAX_APCLI_NUM; idx++) {
		...
		if ((pApCliEntry->ApcliInfStat.Enable == TRUE)
			...
			if (pEntry)
				MTWF_PRINT("\t%s",
					pEntry->SecConfig.PmfCfg.UsePMFConnect ? "TRUE" : "FALSE");
#endif
			MTWF_PRINT("\n");
		}
	}
...
#ifdef DOT11W_PMF_SUPPORT
			MTWF_PRINT("\t%s",
				pSecConfig->PmfCfg.UsePMFConnect ? "TRUE" : "FALSE");
#endif
			MTWF_PRINT("\n");
	...
	if (isConnect)
		...
		for (wcid = 0; VALID_UCAST_ENTRY_WCID(pAd, wcid); wcid++) {
			...
			if (((pMacEntry)
				 ...
				 && MAC_ADDR_EQUAL(pMacEntry->wdev->if_addr, wdev->if_addr))
			   ) {
				PortSecured = WPA_802_1X_PORT_SECURED;
				break;
			}
...
VOID WpaSend(RTMP_ADAPTER *pAdapter, UCHAR *pPacket, ULONG Len)
{
	...
	if (RTMPEqualMemory((pPacket+12), EAPOL, 2)) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPA, DBG_LVL_INFO,
			"Send Deauth, Reason : REASON_DEAUTH_STA_LEAVING\n");
		MlmeDeAuthAction(pAdapter, pEntry, REASON_DEAUTH_STA_LEAVING, FALSE);
	}
}
	...
	IN INT idx)
{
	...
	if (pInfo->Valid &&
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_NOTICE,
			"own mac = "
			MACSTR", is_mlo_connect=%d, delete PMKID CacheIdx=%d\n",
	...
	IN UCHAR is_ft)
{
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		if ((pPMKIDCache->BSSIDInfo[i].Valid == TRUE)
				...
				&& (pPMKIDCache->BSSIDInfo[i].is_ft == is_ft)) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_INFO,
				"=== Find valid PMKID for STA ===\n");
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_INFO,
				"own_mac="MACSTR
				", peer_mac="MACSTR", is_ft=%d, is_mlo_connect=%d\n",
				MAC2STR(pPMKIDCache->BSSIDInfo[i].own_mac),
				MAC2STR(pPMKIDCache->BSSIDInfo[i].peer_mac),
				pPMKIDCache->BSSIDInfo[i].is_ft,
				pPMKIDCache->BSSIDInfo[i].is_mlo_connect);

			if ((pPMKIDCache->BSSIDInfo[i].is_mlo_connect
						&& MAC_ADDR_EQUAL(&pPMKIDCache->BSSIDInfo[i].own_mac, own_mld_mac))
					|| (!pPMKIDCache->BSSIDInfo[i].is_mlo_connect
						&& MAC_ADDR_EQUAL(&pPMKIDCache->BSSIDInfo[i].own_mac, own_link_mac))) {
				RTMPDeletePMKIDCache(pPMKIDCache, (UCHAR *) &pPMKIDCache->BSSIDInfo[i].own_mac, i);
				return;
			}
		}
	}
