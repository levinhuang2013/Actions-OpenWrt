...
/*
 * @ch_band: 0 on 2G, 1 on 5G, 2 on 6G
 */
static u_int32 tm_ch_num_to_freq_by_band(u_int32 ch_num, u_char ch_band)
{
	u_int32 ch_in_mhz = 0;
	...
	if (ch_band == 0) {
		...
		if (ch_num >= 184 && ch_num <= 200)
			ch_in_mhz = 4000 + ch_num * 5;
		else if (ch_num >= 1 && ch_num <= 181)
			ch_in_mhz = 5000 + ch_num * 5;
	} else if (ch_band == 2) {
		if (ch_num >= 1 && ch_num <= 233)
			ch_in_mhz = 5950 + ch_num * 5;
	}
...
#ifdef NON_WIFI_FREQ_5MHZ_SHIFT_SUPPORT
	SetFreq = tm_ch_num_to_freq_by_band(central_ch0, ch_band);
#else
