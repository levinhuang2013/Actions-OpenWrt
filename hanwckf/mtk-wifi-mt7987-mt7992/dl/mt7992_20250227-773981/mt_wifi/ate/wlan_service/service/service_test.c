...
/* if enable 5M_shift, ctrl_ch will be recalculate */
#ifdef NON_WIFI_FREQ_5MHZ_SHIFT_SUPPORT
	if (configs->bw != TEST_BW_20 && configs->ch_band == 1) {
		if (channel + ch_offset > 200)
			configs->ctrl_ch = channel + ch_offset - 200;
		else if (channel + ch_offset <= 0)
			configs->ctrl_ch = channel + ch_offset + 200;
		else
			configs->ctrl_ch = channel + ch_offset;
	}
#endif /* NON_WIFI_FREQ_5MHZ_SHIFT_SUPPORT */


#ifndef NON_WIFI_FREQ_5MHZ_SHIFT_SUPPORT
