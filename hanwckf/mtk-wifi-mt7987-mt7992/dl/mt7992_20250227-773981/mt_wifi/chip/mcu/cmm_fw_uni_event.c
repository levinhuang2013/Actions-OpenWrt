	...
	UINT32 len)
{
	...
	/* do HW RRO if add RRO BA section successful */
		...
		ba_entry->RetryCnt = 0;
		if (ba_entry->REC_BA_Status == Recipient_Initialization
...
static VOID UniEventGetFixedRatePerAcInfo(RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
	UINT8 i;
	struct _UNI_EVENT_FIXED_RATE_PER_AC_INFO_T *CmdFixedRatePerAcInfo
		= (struct _UNI_EVENT_FIXED_RATE_PER_AC_INFO_T *)Data;

	MTWF_PRINT("Per AC Fixed Rate Entry Array:\n");
	for (i = 0; i < 8; i++) {
		MTWF_PRINT("\tWICD(%d)",
			CmdFixedRatePerAcInfo->u2WlanIdxPerAcFixedRateList[i]);
	}
	MTWF_PRINT("\n");
}
	...
	EVENT_RXD *event_rxd)
{
	...
	case UNI_EVENT_RA_FIXED_RATE_PER_AC_INFO:
		UniEventGetFixedRatePerAcInfo(pAd, tag, tags_len);
		break;
	...
	UINT16 wcid)
{
	...
	if (wdev->FtmCtrl.FTMInit) {
		...
		pPeerInfo = &pFTMEntry->VerdictPeerInfo;
		if (pPeerInfo->create_STAREC == 1) {
			mac_entry_delete(pAd, pEntry, TRUE);
			pPeerInfo->create_STAREC = 0;
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_INFO,
...
static VOID UniEventLocNasapNegoResult(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct _UNI_EVENT_LOC_TRIGGER_T *TlvData)
{
	struct _UNI_EVENT_LOC_NASAP_NEGO_RESULT_T *pLocNASAPNegoResult;
	struct _MAC_TABLE_ENTRY *pEntry = NULL;
	VOID *pAddr;
	void *pPeerInfoArray;
	UINT8 ind = 0;
	...
	pLocNASAPNegoResult = (struct _UNI_EVENT_LOC_NASAP_NEGO_RESULT_T *)TlvData;

	/* get pEntry */
	pAddr = (UCHAR *)(&pLocNASAPNegoResult->aucAddr);
	pEntry = MacTableLookup(pAd, pAddr);
	if (!pEntry)
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,
				"Can't find pEntry of "MACSTR", notify FW\n", MAC2STR(pAddr));
		...
		for (ind = 1; ind < MAX_FTM_TBL_SIZE; ind++) {
			...
			if (NdisCmpMemory(pAddr, pPeerInfo->Responder, MAC_ADDR_LEN) == 0) {
				...
				// switch channel
				if (pLocNASAPNegoResult->ucSeitchChannel) {
					pPeerInfo->status = FTMISTA_WAIT_SWITCH_BACK;
					FtmSwitchChannel(pAd, wdev, pPeerInfo->ori_channel, ACTION_SWITCH_BACK);
					pPeerInfo->status = FTMISTA_UNUSED;
	...
	EVENT_RXD *event_rxd)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_LOC_TAG_NASAP_NEGO_RESULT:
			UniEventLocNasapNegoResult(pAd, wdev, (void *)tag);
			break;
							...
							UINT16 rsp_payload_len)
{
	...
	// prEventRxRate->u1RxMode == MODE_UNKNOWN : there is not any rx data now.
	if (rx_rate->u1RxMode != MODE_UNKNOWN) {
		...
		if (pEntry && pEntry->wdev &&  IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) {
			...
			if (prEventResult->arTxRateResult[Idx].u1TxMode == MODE_EHT_MU) {
				...
				EHTLastTxRate.field.ShortGI =
					prEventResult->arTxRateResult[Idx].u1ShortGI;
...
/*For RX rate */
			if (prEventResult->arTxRateResult[Idx].u1RxMode == MODE_EHT_MU) {
				...
				EHTLastRxRate.field.ShortGI =
					prEventResult->arTxRateResult[Idx].u1RxGi;
...
/**
 * @Unsolicited event handler of BSSINFO Beacon Critical Update Flag Clear
 *
 * @param *fixed event common part
 * @param *tlv event tlv
 */
VOID UniEventBssInfoBcnCritUpd(
	struct physical_device *physical_dev,
	struct UNI_EVENT_BSSINFO_T *fixed,
	struct UNI_EVENT_BSSINFO_BCN_CRIT_UPD_T *tlv)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_BSSINFO, DBG_LVL_INFO,
		"UNI_EVENT_BSSINFO_BCN_CRI_UPD: fw_bss_idx=%d\n",
		fixed->ucBssInfoIdx);
}
/**
 * @Unsolicited event handler of BSSINFO
 *
 * @param *physical_dev physical_dev
 * @param *pData event common part
 * @param Length length of event body
 * @param *event_rxd event_rxd
 */
VOID UniEventBssInfoUnsolicitHandler(
	VOID *physical_dev,
	UINT8 *pData,
	UINT32 Length,
	EVENT_RXD *event_rxd)
{
	...
	UINT16 offset = 0;
	UINT32 tags_len;
	UINT32 fixed_len = sizeof(struct UNI_EVENT_BSSINFO_T);
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		switch (TAG_ID(tag)) {
		case UNI_EVENT_BSSINFO_BCN_CRI_UPD:
			UniEventBssInfoBcnCritUpd(physical_dev, (void *)fixed, (void *)tag);
			break;
		default:
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_BSSINFO, DBG_LVL_NOTICE,
				"invalid event tag = %d\n", TAG_ID(tag));
			break;
		}
	}
...
static PROCESS_RX_UNSOLICIT_EVENT_FUNCTION arEventTable[UNI_EVENT_ID_MAX_NUM] = {
	...
	[UNI_EVENT_ID_BSSINFO] = UniEventBssInfoUnsolicitHandler, /* 0x74 */
#ifdef ZERO_PKT_LOSS_SUPPORT
