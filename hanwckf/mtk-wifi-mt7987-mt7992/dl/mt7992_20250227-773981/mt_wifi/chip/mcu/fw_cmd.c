...
INT32 CmdArpFlowControlEnable(RTMP_ADAPTER *pAd, UCHAR Enable, UINT16 threshold, UCHAR disable_fr)
{
	struct cmd_msg *msg = NULL;
	INT32 Ret = 0;
	struct _EXT_CMD_ID_ARP_FLOW_CONTROL ExtArpFlowControl = {0};
	struct _CMD_ATTRIBUTE attr = {0};

	msg = AndesAllocCmdMsg(pAd, sizeof(struct _EXT_CMD_ID_ARP_FLOW_CONTROL));
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2CR4);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_ARP_FLOW_CONTROL);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, 0);
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, NULL);
	...
	ExtArpFlowControl.ucEnable = Enable;
	ExtArpFlowControl.threshold = threshold;
	ExtArpFlowControl.arp_disable_fr = disable_fr;

	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"%s():ucEnable=%d, threshold=%d, arp_forcerate_disable=%d\n", __func__,
		ExtArpFlowControl.ucEnable,
		ExtArpFlowControl.threshold, ExtArpFlowControl.arp_disable_fr);

	AndesAppendCmdMsg(msg, (char *)&ExtArpFlowControl,
					  sizeof(struct _EXT_CMD_ID_ARP_FLOW_CONTROL));
	Ret = AndesSendCmdMsg(pAd, msg);
Error0:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_ERROR,
			 "%s:(Ret = %d)\n", __func__, Ret);
...
INT32 CmdUnicastAllEnable(RTMP_ADAPTER *pAd, BOOLEAN Enable, UINT8 band_idx, UINT8 omac_idx, UINT8 bss_idx)
{
	struct cmd_msg *msg = NULL;
	INT32 Ret = 0;
	struct _EXT_CMD_ID_UNICAST_ALL ext_unicast_all;
	struct _CMD_ATTRIBUTE attr = {0};

	msg = AndesAllocCmdMsg(pAd, sizeof(struct _EXT_CMD_ID_UNICAST_ALL));
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2CR4);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_UNICAST_ALL);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, 0);
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, NULL);
	...
	ext_unicast_all.uc_omac_idx = omac_idx;
	ext_unicast_all.ucUnicastAllEnable = Enable;
	ext_unicast_all.uc_band_idx = band_idx;
	ext_unicast_all.uc_bss_idx = bss_idx;

	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"omac_idx=%d, en=%d, bss_idx = %d\n",
		omac_idx, Enable, bss_idx);


	AndesAppendCmdMsg(msg, (char *)&ext_unicast_all,
					  sizeof(struct _EXT_CMD_ID_UNICAST_ALL));
	Ret = AndesSendCmdMsg(pAd, msg);
Error0:
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "(Ret = %d)\n", Ret);
