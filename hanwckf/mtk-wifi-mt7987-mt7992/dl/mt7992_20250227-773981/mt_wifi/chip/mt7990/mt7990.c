...
/* both offsets are protect range
 * range offset begin, end */
static UINT32 ddie_EFUSE_PROTE[] = {
	0x10,  0x18f,
	0x1b0, 0x2bf,
	0x2c6, 0x2ff,
	0x301, 0x30f,
...
static UINT32 adie_7975_EFUSE_PROTE[] = {
	0x5c0, 0x62f,
	0x6c0, 0x6ff,
	0x7a1, 0x7a1,
	0x7a9, 0x7a9,
...
static UINT32 adie_7976_EFUSE_PROTE[] = {
	0x0,   0x7f,
	0x790, 0x798,
	0x79a, 0x79f,
	0x7a6, 0x7a6,
...
static UINT32 adie_7977_EFUSE_PROTE[] = {
	0x0,   0x6e,
	0x70,  0x7c,
	0x7e,  0x10f
};

UINT16 gsku = 0, gfSku = 0;
/* Return option - TRUE: writeable, FALSE: Protection */
BOOLEAN mt7990_write_efuse_field_valide(RTMP_ADAPTER *pAd, UINT32 ee_offset)
{
	UINT16	i = 0, aidx = 0;
	UINT32 *ptr = &ddie_EFUSE_PROTE[0];
	UINT32 *adie[3] = {&adie_7976_EFUSE_PROTE[0],
		&adie_7977_EFUSE_PROTE[0],
		&adie_7977_EFUSE_PROTE[0]};
	UINT32 adie_offset[6] = {MT7990_ADIE0_OFFSET, MT7990_ADIE2_OFFSET - 1,
		MT7990_ADIE1_OFFSET, 0x2a00 - 1,
		MT7990_ADIE2_OFFSET, MT7990_ADIE1_OFFSET - 1};
	UINT32 adie_size[4] = {(sizeof(adie_7976_EFUSE_PROTE)/sizeof(UINT32)),
		(sizeof(adie_7977_EFUSE_PROTE)/sizeof(UINT32)),
		(sizeof(adie_7977_EFUSE_PROTE)/sizeof(UINT32)), 0};

	/* D-die */
	if (ee_offset < MT7990_ADIE0_OFFSET) {
		for (i = 0; i < (sizeof(ddie_EFUSE_PROTE)/sizeof(UINT32)); i += 2) {
			if (ee_offset >= ptr[i] && ee_offset <= ptr[i+1]) {
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
					"invalid Ddie ee_offset: 0x%x\n", ee_offset);
				return FALSE;
			}
		}
		return TRUE;
	...
	if (gfSku == 0) {
		gsku = mt7990_get_sku_decision(pAd);
		gfSku = 1;
	}

	/* D-die */
	switch (gsku) {
	case MT7990_ADIE_677:
		/* adie 0 */
		adie[0] = &adie_7976_EFUSE_PROTE[0];
		adie_size[0] = sizeof(adie_7976_EFUSE_PROTE)/sizeof(UINT32);
		break;

	case MT7990_ADIE_577:
		adie[0] = &adie_7975_EFUSE_PROTE[0];
		adie_size[0] = sizeof(adie_7975_EFUSE_PROTE)/sizeof(UINT32);
		break;

	case MT7990_ADIE_607:
		adie[1] = NULL;
		adie_size[1] = 0;
		break;

	case MT7990_ADIE_507:
		adie[0] = &adie_7975_EFUSE_PROTE[0];
		adie_size[0] = sizeof(adie_7975_EFUSE_PROTE)/sizeof(UINT32);
		adie[1] = NULL;
		adie_size[1] = 0;
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR, "invlide sku:%d\n", gsku);
		break;
	}

	for (aidx = 0; aidx < 3; aidx++) {
		if (adie[aidx] != NULL &&
			ee_offset >= adie_offset[aidx * 2] && ee_offset <= adie_offset[aidx * 2  + 1]) {
			ptr = adie[aidx];
			for (i = 0; i < adie_size[aidx]; i += 2) {
				if (ee_offset >= (ptr[i] + adie_offset[aidx * 2])
					&& ee_offset <= (ptr[i+1] + adie_offset[aidx * 2])) {
					MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
						"invalid Adie:%d 0x%x~%x, a_ofs:%x, ee_offset: 0x%x\n",
						aidx, ptr[i], ptr[i+1], adie_offset[aidx * 2], ee_offset);
					return FALSE;
				}
			}
		}
	...
	struct _RTMP_ADAPTER *pAd, struct test_capability *chip_cap)
{
	...
	RT28xx_EEPROM_READ16(pAd, EEPROM_NON_WIFI_5MHZ_SHIFT_OFFSET, value);
	if (value & NON_WIFI_5MHZ_SHIFT_MASK) {
		/* Non-WiFi frequency with 5MHz shift support */
		chip_cap->ext_cap.feature1 |= BIT11;
	}
...
#ifdef ICVERR_RESET_RRO
VOID mt7990_reset_rro_entry(
	struct _RTMP_ADAPTER *ad,
	struct _MAC_TABLE_ENTRY *entry,
	struct BA_REC_ENTRY *pBAEntry, UINT16 mld_sta_idx, UINT8 bPNChk)
{
#ifdef DOT11_EHT_BE
	struct mld_entry_t *mld_entry = NULL;
#endif
...
#ifdef DOT11_EHT_BE
	mt_rcu_read_lock();
	if (IS_ENTRY_MLO(entry) && entry->wdev) {
	...
	if (mld_entry) {
		...
		for (idx = 0; idx < MLD_LINK_MAX; idx++) {
			if (mld_entry->link_entry[idx]) {
				...
				u16 sessin_id = pBAEntry->Session_id;

#ifdef TCSUPPORT_NPU_WIFI_OFFLOAD
				if (glb_npu_en_node)
					sessin_id = (link_entry->wcid & 0x7ff) | ((pBAEntry->TID & 0xf) << 11);
#endif
				...
				if (bPNChk)
					...
					link_entry->DISABLE_PN_CHK |= BIT(pBAEntry->TID);
				hc_set_wed_pn_check(ad, link_entry->wcid, sessin_id, bPNChk);
				hc_set_particular_to_host(ad, bPNChk);
			}
		}
		if (entry->wdev->wdev_type == WDEV_TYPE_AP)
...
#ifdef TCSUPPORT_NPU_WIFI_OFFLOAD
		if (glb_npu_en_node)
			sessin_id = (entry->wcid & 0x7ff) | ((pBAEntry->TID & 0xf) << 11);
#endif

		hc_set_wed_pn_check(ad, entry->wcid, sessin_id, bPNChk);
		hc_set_particular_to_host(ad, bPNChk);
	...
	MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO,
		"%s; Wcid=%d TID=%d DisPnChk=%d RRO_RESET_CNT=%ld flag_err_cnt=%ld\n",
		bPNChk?"EXIT":"RESET", entry->wcid, pBAEntry->TID, entry->DISABLE_PN_CHK, pBAEntry->RRO_RESET_CNT, pBAEntry->flag_err_cnt);

}
#endif /* ICVERR_RESET_RRO */
	...
	ULONG *data_len)
{
	...
	if (action & CHIP_EXTRA_ACTION_ICV_ERROR) {
		...
		rec_idx = entry->ba_info.RecWcidArray[pRxBlk->TID];
		...
		if (IS_ENTRY_MLO(entry)) {
			...
			rec_idx = mld_entry->ba_info.RecWcidArray[pRxBlk->TID];
		...
		pBAEntry = &ba_ctl->BARecEntry[rec_idx];
		...
		if (RX_BLK_TEST_FLAG(pRxBlk, fRX_ICV_ERR)) {
			...
			MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO,
				"Reset! wcid=%d TID=%u SN=%u PN=%llx ErrFlags=0x%x Flags=0x%x dispnchk=%d\n",
				pRxBlk->wcid, pRxBlk->TID, pRxBlk->SN, pRxBlk->CCMP_PN, pRxBlk->ErrFlags, pRxBlk->Flags, link_entry->DISABLE_PN_CHK);
			pBAEntry->CCMP_ICV_ERROR = FALSE;

			if (!(link_entry->DISABLE_PN_CHK & BIT(pBAEntry->TID)))
				mt7990_reset_rro_entry(ad, link_entry, pBAEntry, mld_sta_idx, 0);
		}
	...
	if (action & CHIP_EXTRA_ACTION_PN_CHK) {
		...
		for (idx = 1; VALID_UCAST_ENTRY_WCID(ad, idx); idx++) {
			...
			if (IS_ENTRY_MLO(entry)) {
				...
				mld_sta_idx = mld_entry->mld_sta_idx;
				entry = &ad->MacTab->Content[mld_entry->setup_link_wcid];
			}
#endif /* DOT11_EHT_BE */
			for (tid = 0; tid < NUM_OF_TID; tid++) {
				...
				if ((entry->DISABLE_PN_CHK & BIT(tid)) &&
					...
					if (pBAEntry->HAS_ICV_ERROR)
						...
						mt7990_reset_rro_entry(ad, entry,
							pBAEntry, mld_sta_idx, 1);
				}
				NdisReleaseSpinLock(&pBAEntry->RxReRingLock);
			}
#ifdef DOT11_EHT_BE
	...
	if (action & CHIP_EXTRA_ACTION_ICV_ERR_INIT) {
		...
		if (IS_ENTRY_MLO(entry)) {
			...
			for (idx = 0; idx < MLD_LINK_MAX; idx++) {
				if (mld_entry->link_entry[idx]) {
					...
					link_entry->DISABLE_PN_CHK &= ~(BIT(pBAEntry->TID));
					hc_set_wed_pn_check(ad, link_entry->wcid, pBAEntry->Session_id, TRUE);
					hc_set_particular_to_host(ad, TRUE);
				}
			}
		} else
#endif /* DOT11_EHT_BE */
		{
			entry->DISABLE_PN_CHK &= ~(BIT(pBAEntry->TID));
			hc_set_wed_pn_check(ad, entry->wcid, pBAEntry->Session_id, TRUE);
			hc_set_particular_to_host(ad, TRUE);
		}
#ifdef DOT11_EHT_BE
		mt_rcu_read_unlock();
...
static VOID mt7990_chipOp_init(struct _RTMP_ADAPTER *pAd, RTMP_CHIP_OP *chip_op)
{
	...
	chip_op->write_efuse_field_valide = mt7990_write_efuse_field_valide;
...
static VOID mt7990_archOp_init(RTMP_ADAPTER *ad, RTMP_ARCH_OP *arch_ops)
{
	...
	if (IS_HWIFI_INF(ad)) {
		...
		arch_ops->get_wdma_related_tx_bus_info = hwifi_get_wdma_related_tx_bus_info;
#endif
#endif /* ERR_RECOVERY */
	} else
