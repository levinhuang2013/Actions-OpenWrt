...
/* both offsets are WRITEABLE range
...
BOOLEAN mt7992_write_efuse_field_valide(RTMP_ADAPTER *pAd, UINT32 ee_offset)
{
	UINT16 i = 0;
	UINT32 *ptr = &ddie_EFUSE_PROTE[0];

	/* D-die */
	if (ee_offset < MT7992_ADIE0_OFFSET) {
...
#ifdef TCSUPPORT_NPU_WIFI_OFFLOAD
			if (!(glb_npu_en_node && (link_entry->DISABLE_PN_CHK & BIT(pBAEntry->TID))))
#endif
				mt7992_reset_rro_entry(ad, link_entry, pBAEntry, mld_sta_idx, 0);
		}
			...
			for (tid = 0; tid < NUM_OF_TID; tid++) {
				...
				if ((entry->DISABLE_PN_CHK & BIT(tid)) &&
					...
					if (pBAEntry->HAS_ICV_ERROR)
						...
						mt7992_reset_rro_entry(ad, entry,
	...
	if (action & CHIP_EXTRA_ACTION_ICV_ERR_INIT) {
			...
			for (idx = 0; idx < MLD_LINK_MAX; idx++) {
				if (mld_entry->link_entry[idx]) {
					...
					if (glb_npu_en_node)
						sessin_id = (link_entry->wcid & 0x7ff) | ((pBAEntry->TID & 0xf) << 11);
#endif
					hc_set_wed_pn_check(ad, link_entry->wcid, sessin_id, TRUE);
...
#endif /* DOT11_EHT_BE */
		{
			...
			if (glb_npu_en_node)
				sessin_id = (entry->wcid & 0x7ff) | ((pBAEntry->TID & 0xf) << 11);
#endif
			hc_set_wed_pn_check(ad, entry->wcid, sessin_id, TRUE);
			hc_set_particular_to_host(ad, TRUE);
		}
#ifdef DOT11_EHT_BE
		mt_rcu_read_unlock();
...
static VOID mt7922_update_chip_cap(RTMP_ADAPTER *pAd)
{
	struct hdev_ctrl *ctrl = pAd->hdev_ctrl;
	RTMP_CHIP_CAP *chip_cap = hc_get_chip_cap(ctrl);

	if (mt7992_get_sku_decision(pAd) == MT7992_SKU_BE7200_MT7975_MT7979)
		chip_cap->phy_caps |= fPHY_CAP_TXIBF_HE_EHT;
}
...
static VOID mt7992_chipOp_init(struct _RTMP_ADAPTER *pAd, RTMP_CHIP_OP *chip_op)
{
	...
	chip_op->write_efuse_field_valide = mt7992_write_efuse_field_valide;
	chip_op->update_chip_cap = mt7922_update_chip_cap;
}
...
static VOID mt7992_archOp_init(RTMP_ADAPTER *ad, RTMP_ARCH_OP *arch_ops)
{
	...
	arch_ops->archUniCmdSetTxCmdEDCA = MtAsicUniCmdSetTxCmdEDCA;
	arch_ops->archUniCmdSetPerStaWMM = MtAsicUniCmdSetPerStaWMM;
#endif /* WIFI_UNIFIED_COMMAND */
