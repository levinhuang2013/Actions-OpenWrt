...
static VOID chip_show_dma_info(struct hdev_ctrl *ctrl)
{
	...
	if (IS_RBUS_INF(pAd) || IS_PCI_INF(pAd) || IS_HWIFI_INF(pAd)) {
		...
		dump_dma_tx_ring_info(ctrl, "T18:TXD0(H2WA)", "AP",
			WF_WFDMA_EXT_WRAP_CSR_WED_TX0_CTRL0_ADDR);
		dump_dma_tx_ring_info(ctrl, "T19:TXD1(H2WA)", "AP",
			WF_WFDMA_EXT_WRAP_CSR_WED_TX1_CTRL0_ADDR);
#else
		dump_dma_tx_ring_info(ctrl, "T18:TXD0(H2WA)", "AP",
...
static INT32 chip_show_cmd_timeout_info(struct _RTMP_ADAPTER *pAd)
{
	...
	/*consider the invalid ctrl_info_ram_addr case*/
	if ((ctrl_info_ram_addr == 0) || ((ctrl_info_ram_addr % 4) != 0)) {
		MTWF_PRINT("invalid ctrl_info_ram_addr= 0x%08x, return\n", ctrl_info_ram_addr);
		return false;
	}
