...
void mt7992_ser_1_0_v1(struct ser_cb_t *ser_cb)
{
	...
	case SER_LV_1_0_STATE_STOP_PDMA0:
		if ((event & SER_LV_1_0_EVT_RESET_DONE) == SER_LV_1_0_EVT_RESET_DONE) {
			...
			/* case 2 reset ignore DMA_TK reset fail, notify FE reset */
			if (atomic_read(&pErrRecoveryCtrl->notify_from_fe)) {
				struct _RTMP_ADAPTER *pAd2;

				MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_INFO,
					"notify_from_fe ignore DMA_TK reset fail, ret=%d\n", ret);

				MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_INFO,
					"ack FE WIFI_RESET_DONE!!!\n");
			...
			/*  DMA_TK reset fail */
			if (ret) {
				/* consys reset fail (case3 reset) notify to FE */
				atomic_set(&pErrRecoveryCtrl->notify_to_fe, 1);
				ser_cb->need_reset = TRUE;
				MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_INFO,
					"DMA_TK reset fail need case3 reset!!\n");
			}
#endif

			ser_tans_ser_state(ser_cb, SER_LV_1_0_STATE_STOP_PDMA0_P2);
			/*re-call for change status to stop pdma0_p2*/
			mt7992_ser_1_0_v1(ser_cb);
			break;
	...
	case SER_LV_1_0_STATE_STOP_PDMA0_P2:
...
#ifdef MTK_FE_RESET_RECOVER
		atomic_set(&pErrRecoveryCtrl->notify_from_fe, 0);
		if (ser_cb->need_reset) {
			ser_cb->need_reset = FALSE;
			if (atomic_read(&pErrRecoveryCtrl->notify_to_fe)) {
				atomic_set(&pErrRecoveryCtrl->notify_to_fe, 0);
...
void mt7992_fe_reset(struct ser_cb_t *ser_cb)
{
	...
	if (event == SER_FE_STOP) {
		if (asic_ser_handler(pAd, SER_ACTION_SET_TRAFFIC, 0)) {
			...
			MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_ERROR,
				"SER ack event = %lx failed stop\n",
				FE_WIFI_TARFFIC_DONE_FAIL);
		} else
