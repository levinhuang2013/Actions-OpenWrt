...
)
{
...
#endif
	UCHAR *pHeader802_3 = NULL;
	UCHAR *exclude_source_mac_addr = NULL;
	...
	if ((a4_get_entry_count(adapter, if_index) > 0)) {
		...
		pHeader802_3 = GET_OS_PKT_DATAPTR(pkt);
		exclude_source_mac_addr = pHeader802_3 + MAC_ADDR_LEN;

		if (exclude_source_mac_addr) {
			routing_entry = RoutingTabLookup(adapter, if_index, exclude_source_mac_addr, FALSE, &wcid);
		...
		DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List) {
			if (a4_entry->valid && IS_WCID_VALID(adapter, a4_entry->wcid)) {
				...
				RTMP_SET_PACKET_SN_VLD(pkt_clone, 0);
