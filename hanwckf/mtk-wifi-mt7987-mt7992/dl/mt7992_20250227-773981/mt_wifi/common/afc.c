...
static VOID afc_set_wdev_to_hold(struct _RTMP_ADAPTER *pAd);
...
static INT afc_show_afc_params(IN struct _RTMP_ADAPTER *pAd)
{
	...
	for (unii_index = BAND_6G_UNII_5; unii_index < BAND_6G_UNII_NUM; unii_index++) {
		...
		MTWF_DBG_NP(DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_NOTICE,
			 "\nEIRP 320 :");
		max_index = max_afc_chan_idx[unii_index][AFC_BW_320];
		for (index = 0; index < max_index; index++)
			MTWF_DBG_NP(DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_NOTICE,
				" %d", pwr_info->max_eirp_bw320[index]);
...
VOID afc_disconnect_all_sta(RTMP_ADAPTER *pAd, UCHAR apidx)
{
	...
	APMlmeKickOutAllSta(pAd, apidx, REASON_DISASSOC_STA_LEAVING);
...
VOID afc_set_wdev_to_hold(RTMP_ADAPTER *pAd)
{
	BCN_BUF_STRUCT *bcn_info = NULL;
	struct wifi_dev *wdev = NULL;
	INT8 Bss_Idx;
...
INT afc_get_6g_interface_count(RTMP_ADAPTER *pAd)
{
	INT8 Bss_Idx, Bss_Idx_cnt;
	struct wifi_dev *wdev = NULL;
...
VOID afc_enable_tx(RTMP_ADAPTER *pAd)
{
	UINT16 u2ResponseCode = pAd->afc_response_data.response_code;
	struct wifi_dev *wdev = NULL;
	INT8 Bss_Idx, interface_cnt;
	BCN_BUF_STRUCT *bcn_info = NULL;

	pAd->afc_ctrl.AfcStateMachine.CurrState = AFC_RUN;
	interface_cnt = afc_get_6g_interface_count(pAd);
	if (pAd->CommonCfg.AfcDeviceType == AFC_LP_SP_COEX)
...
INT afc_test_mode(struct _RTMP_ADAPTER *pAd, char *arg)
{
	...
	if (paramref == AFC_TEST_DISABLE) {
	...
	} else if (paramref == AFC_TEST_REG_0) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		"Tx Reg 0 Beacon (Chagne devce type = COEX + hold state).\n");
		pAd->CommonCfg.AfcDeviceType = AFC_LP_SP_COEX;
		pAd->afc_ctrl.AfcStateMachine.CurrState = AFC_HOLD;
		wdev->bAllowBeaconing = TRUE;
		pAd->afc_ctrl.bAfcBeaconInit = TRUE;
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		 "paramref is %d\n", paramref);
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		 "iwpriv rax0 set AFCtest=0 => fall back to hold state.\n");
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		 "iwpriv rax0 set AFCtest=5 => Change device to COEX mode + hold state.\n");
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_AFC, DBG_LVL_ERROR,
		 "Others are forbiden.\n");
		return FALSE;
	}

	if (paramref != AFC_TEST_DISABLE && paramref != AFC_TEST_REG_0) {
