...
INT set_ba_iot_test_action(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (enable)
		tr_ctl->en_ba_iot_test = TRUE;
	else
		tr_ctl->en_ba_iot_test = FALSE;

	MTWF_PRINT("%s(): (EN = %d)\n", __func__, tr_ctl->en_ba_iot_test);
	return TRUE;
}
...
VOID ba_ori_session_start(RTMP_ADAPTER *pAd, UINT16 wcid, UINT8 UPriority)
{
	...
	if ((tr_entry && tr_entry->EntryType != ENTRY_CAT_MCAST && VALID_UCAST_ENTRY_WCID(pAd, wcid)) &&
		...
		if (((tr_entry->PortSecured == WPA_802_1X_PORT_SECURED) || tr_ctl->en_ba_iot_test)
			&& (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))) {
