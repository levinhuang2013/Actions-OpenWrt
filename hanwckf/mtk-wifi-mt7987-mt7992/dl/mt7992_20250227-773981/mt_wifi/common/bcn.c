...
/*
    ==========================================================================
    Description:
	Used to check whether any wdev on the band can send Beancon.
    return value
	FALSE: mean no necessary.
	TRUE: mean need to send Beacon on the band.
    ==========================================================================
*/
BOOLEAN BeaconTransmitRequiredOnBand(RTMP_ADAPTER *pAd)
{
	int apidx = 0;
	BSS_STRUCT *pMbss;
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		if (WDEV_WITH_BCN_ABILITY(&pMbss->wdev)
			&& BeaconTransmitRequired(pAd, &pMbss->wdev))
			return TRUE;
	}
		...
		if (bw >= BW_40) {
			...
			*(ptr + 1) = sizeof(WIDE_BW_CH_SWITCH_ELEMENT);
			...
			case BW_40:
				...
				wb_info.center_freq_1 = vht_cent_ch_freq_40mhz(wdev, channel, VHT_BW_2040, ch_band);
...
VOID MakeHTIe(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, ULONG *pFrameLen, UCHAR *pBeaconFrame)
{
	...
	wlan_operate_update_ht_bw_cap(wdev);
...
#ifdef DOT11_EHT_BE
		for (i = 0; i < ARRAY_SIZE(wdev->bcn_buf.csa.ml_csa_ie_offset); i++) {
			wdev->bcn_buf.csa.ml_csa_ie_offset[i] = 0;
			wdev->bcn_buf.csa.ml_ie_offset[i] = 0;
		}
...
/*Reduced neighbor report IE should be final IE to be added, so we can determine the maximum length of Beacon*/
	...
	if (WMODE_CAP_BE(wdev->PhyMode) &&
		...
		if (PD_CEHCK_MLO_V1_ENABLE(pAd->physical_dev)) {
			...
			if (bcn) {
				for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
					if (i < ARRAY_SIZE(wdev->bcn_buf.csa.ml_ie_offset))
						wdev->bcn_buf.csa.ml_ie_offset[i] = FrameLen;
				}
			}
			/* add MTK MLO V1 IE */
		...
		if (bcn) {
			for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
				if (i < ARRAY_SIZE(wdev->bcn_buf.csa.ml_csa_ie_offset))
					wdev->bcn_buf.csa.ml_csa_ie_offset[i] = 0;

				if (i < ARRAY_SIZE(wdev->bcn_buf.csa.ml_csa_ie_offset) && wdev->bcn_buf.csa.csa_ie_offset_in_ml[i]) {
					wdev->bcn_buf.csa.ml_csa_ie_offset[i] = wdev->bcn_buf.csa.ml_ie_offset[i]
														+ wdev->bcn_buf.csa.csa_ie_offset_in_ml[i];
					MTWF_DBG(NULL, DBG_CAT_AP, CATAP_BCN, DBG_LVL_NOTICE,
						"ml_ie_offset(%d), csa_ie_offset_in_ml(%d), csa_bss_idx(%d).\n",
						wdev->bcn_buf.csa.ml_ie_offset[i],
						wdev->bcn_buf.csa.csa_ie_offset_in_ml[i],
						wdev->bcn_buf.csa.csa_bss_idx[i]);
				}
			}

			MTWF_DBG(NULL, DBG_CAT_AP, CATAP_BCN, DBG_LVL_INFO,
				"BSS(%d), CSA IE offset = %d\n", wdev->bss_info_argument.ucBssIndex, wdev->bcn_buf.CsaIELocationInBeacon);
		}
	}
#endif /* DOT11_EHT_BE */
	...
	BCN_UPDATE_REASON reason)
{
	...
	if (pDot11h) {
		...
		if (pDot11h->csa_ap_bitmap != 0
			&& !BCN_REASON_EQUAL(reason, BCN_UPDATE_INIT)
	...
	UCHAR UpdateReason)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_INFO,
			 "bMakeBeacon: %d, UpdateReason: %d\n", bMakeBeacon, UpdateReason);
