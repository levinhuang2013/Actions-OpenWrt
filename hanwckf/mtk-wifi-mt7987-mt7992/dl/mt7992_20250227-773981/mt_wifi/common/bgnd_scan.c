...
{
#define INVALID_BANDWIDTH 0xffffffff

#define SET_CHANNEL 1
#define SET_CHANNEL_BANDWIDTH 2
	...
	if (arg) {
		...
		recv = sscanf(arg, "%d:%d", &(ch), &(bw));
		if (recv == SET_CHANNEL_BANDWIDTH) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
				"IPI Scan control ch:%d and bw:%d\n", ch, bw);
		} else if (recv == SET_CHANNEL) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
				"IPI Scan control ch:%d\n", ch);
		} else {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
				"Format Error! Please enter in the following format\n"
				"set IPIScanCtrl=ch:bw(optional)\n"
				"\tbw: 2G(0: 20MHz, 1: 40MHz), 5G(0: 20MHz/40MHz, 1: 80MHz, 2:160MHz)\n");
			return TRUE;
		}

		/* Optional: Change the bw */
		if (bw != INVALID_BANDWIDTH && bw <= VHT_BW_160 && bw >= VHT_BW_2040) {
#define UPDATE_BANDWIDTH(wdev, ch, bw) \
	do { \
		if (bw == BW_40) { \
		...
		if (wdev->channel == ch) { \
			...
			if (WMODE_CAP_AC(tgt_wdev->PhyMode)) {
				...
				for (i = 0; i < WDEV_NUM_MAX; i++) {
					...
					wdev = pAd->wdev_list[i];
					if (wdev && (band_idx == HcGetBandByWdev(wdev))) {
						UPDATE_BANDWIDTH(wdev, ch, bw);
					}
				}
			}

#undef UPDATE_BANDWIDTH
		}
...
#undef INVALID_BANDWIDTH

#undef SET_CHANNEL
#undef SET_CHANNEL_BANDWIDTH
...
static UCHAR dfs_pre_cac_get_next_detect_channel(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev)
{
	...
	for (i = 0; i < pChCtrl->ChListNum; i++) {
		...
		if ((DfsPrimToCent(pChCtrl->ChList[i].Channel, pDfsParam->band_bw)
			== DfsPrimToCent(inband_ch, pDfsParam->band_bw)) && (pChCtrl->ChList[i].cac_done_timestamp != 0))
			continue;
...
VOID dfs_pre_cac_detect_next_channel(IN RTMP_ADAPTER * pAd, IN struct wifi_dev *wdev)
{
	...
	if (pDfsParam->bPreCacEn
		...
		if (next_ch == pDfsParam->OutBandCh && *ch_stat != DFS_INB_CH_INIT)
		...
		if (next_ch != 0) {
			...
			case DFS_INB_DFS_OUTB_CH_CAC_DONE:
				/*Use BW supported by channel and dedicated RX*/
				ch_bw_cap = get_channel_bw_cap(wdev, next_ch);

				if (DfsDedicatedCheckChBwValid(pAd, next_ch, ch_bw_cap, band_idx))
					pDfsParam->pre_cac_BW = ch_bw_cap;
				else if (DfsDedicatedCheckChBwValid(pAd, next_ch, pDfsParam->OutBandBw, band_idx))
					pDfsParam->pre_cac_BW = pDfsParam->OutBandBw;
				else
					pDfsParam->pre_cac_BW = 0;

				if (!RadarChannelCheck(pAd, next_ch) && ((pDfsParam->pre_cac_BW == BW_160) && IS_CH_BETWEEN(next_ch, 36, 64)))
					pDfsParam->pre_cac_BW = BW_160;
				else if (pDfsParam->BW80DedicatedZWSupport && pDfsParam->pre_cac_BW > BW_80)
					pDfsParam->pre_cac_BW = BW_80;
