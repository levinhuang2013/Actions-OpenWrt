...
static bool alloc_bcn_appl_elem_per_sta_pf(struct bmgr_entry *entry)
{
	...
	for (repting_link = 0; repting_link < BSS_MNGR_MAX_BAND_NUM; repting_link++) {
		...
		per_sta_pf->len = 0;
		entry->csa.repted_csa_offset_in_per_sta_pf[repting_link] = 0;
		entry->csa.repting_csa_offset_in_per_sta_pf[repting_link] = 0;
...
static void free_bcn_appl_elem_per_sta_pf(struct bmgr_entry *entry)
{
	...
	for (link = 0; link < BSS_MNGR_MAX_BAND_NUM; link++) {
		if (entry->bcn_appl_elem_per_sta_pf[link].buf) {
			...
			entry->csa.repted_csa_offset_in_per_sta_pf[link] = 0;
			entry->csa.repted_csa_bss_idx[link] = 0;

			entry->csa.repting_csa_offset_in_per_sta_pf[link] = 0;
			entry->csa.repting_csa_bss_idx[link] = 0;
		}
	}
}
	...
	struct bmgr_entry *repting_entry, struct bmgr_entry *repted_entry)
{
	...
	if (repting_entry->entry_info.disc_param.rnr_6g_rule == RNR_REPORTING_NONE) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_INFO,
...
int bss_mngr_con_mld_group_addr_set(
	...
	IN u8 *mld_addr)
{
	...
	u16 i = 0;
	struct bmgr_mlo_dev *mld = NULL;
	struct bmgr_mlo_dev mld2;
	struct bmgr_entry *mbss_entry;
	u8 bss_idx_mld;
	u8 mld_grp_idx2;
	...
	memcpy(&mld2, mld, sizeof(mld2));

	for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
		bss_idx_mld = mld2.bss_idx_mld[i];

		mbss_entry = GET_BSS_ENTRY_BY_IDX(bss_idx_mld);
		if (!BMGR_VALID_BSS_ENTRY(mbss_entry))
			continue;

		ret = bss_mngr_con_mld_group_del_link(mbss_entry->pNetDev);
		if (ret != NDIS_STATUS_SUCCESS) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
				"Error: del link failed, mld_grp_idx:%u wdev:%p\n",
				mld_grp_idx, mbss_entry->pwdev);
			return ret;
		}
	}
	ret = bss_mngr_con_mld_group_destroy(wdev->if_dev, mld_grp_idx);
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			"Error: destroy mld group failed, mld_grp_idx:%u\n",
			mld_grp_idx);
		return ret;
	}

	mld_grp_idx2 = mld_grp_idx;
	ret = bss_mngr_con_mld_group_create(wdev->if_dev, &mld_grp_idx2, mld_addr, &mld2.attr, BMGR_MLD_TYPE_MULTI);
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			"Error: create mld group failed, mld_grp_idx:%u\n",
			mld_grp_idx2);
		return ret;
	}
	...
	for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
		...
		ret = bss_mngr_con_mld_group_add_link(mbss_entry->pNetDev, mld_grp_idx);
		if (ret != NDIS_STATUS_SUCCESS) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
				"Error: add link failed, mld_grp_idx:%u wdev:%p\n",
				mld_grp_idx, mbss_entry->pwdev);
			return ret;
		}
	}
...
static int bss_mngr_update_iob_disc(void)
{
	...
	for (i = 0; i < BSS_MNGR_ENTRY_CNT; i++) {
		...
		if (BMGR_VALID_BSS_ENTRY(entry)) {
			...
			by_cfg = entry->entry_info.disc_param.iob_by_cfg;
			if (by_cfg) {
				MTWF_DBG(NULL, DBG_CAT_AP, CATAP_MBSS, DBG_LVL_INFO,
			...
			if (WMODE_CAP_6G(phy_mode)) {
				...
				iob_interval = entry->entry_info.disc_param.iob_interval;
				iob_mode = entry->entry_info.disc_param.iob_mode;
			...
			entry->entry_info.disc_param.iob_type = iob_type;
			entry->entry_info.disc_param.iob_interval = iob_interval;
			entry->entry_info.disc_param.iob_mode = iob_mode;

			event_ops->bmgr_event_update_iob_disc(entry->pNetDev,
				&entry->entry_info.disc_param);
		}
	}
...
NDIS_STATUS bss_mngr_con_update_he_bss_discov(
	IN PNET_DEV pNetDev,
	IN u8 update_type,
	IN struct bmgr_disc_param *disc_param)
{
	struct bmgr_entry *entry = get_bss_entry_by_netdev(pNetDev);
	bool is_updated;
	...
	if (BMGR_VALID_BSS_ENTRY(entry)) {
		switch (update_type) {
		case BSS_IOB_DISCOV_UPDATE:
			is_updated = (entry->entry_info.disc_param.iob_type
				!= disc_param->iob_type) ? TRUE : FALSE;

			entry->entry_info.disc_param.iob_by_cfg = disc_param->iob_by_cfg;
			entry->entry_info.disc_param.iob_type = disc_param->iob_type;
			entry->entry_info.disc_param.iob_interval = disc_param->iob_interval;
			entry->entry_info.disc_param.iob_mode = disc_param->iob_mode;
		...
		case BSS_OOB_DISCOV_UPDATE:
			is_updated = (entry->entry_info.disc_param.rnr_6g_rule
				!= disc_param->rnr_6g_rule) ? TRUE : FALSE;

			entry->entry_info.disc_param.rnr_6g_rule = disc_param->rnr_6g_rule;
...
static int bss_mngr_con_info_show(u8 type)
{
	...
	for (i = 0; i < BSS_MNGR_ENTRY_CNT; i++) {
		...
		MTWF_PRINT("\t\tAuthMode:%s, Cipher(P:%s/G:%s), MFPC=%d/MFPR=%d/RsnCap=%02x%02x, 6G-RNR:(%s)",
			GetAuthModeStr(reg_info->auth_mode),
			GetEncryModeStr(reg_info->PairwiseCipher),
			GetEncryModeStr(reg_info->GroupCipher),
			reg_info->MFPC, reg_info->MFPR, reg_info->RsnCap[0], reg_info->RsnCap[1],
			(reg_info->disc_param.rnr_6g_rule < RNR_REPORTING_INVLD) ?
				rnr_rule_str[reg_info->disc_param.rnr_6g_rule] : "INVLD");

		MTWF_PRINT(", IOB:T(%d)/Tu(%d)/M(%d)\n",
			reg_info->disc_param.iob_type,
			reg_info->disc_param.iob_interval,
			reg_info->disc_param.iob_mode);
...
)
{
	int ret = NDIS_STATUS_FAILURE;
	struct bmgr_entry *entry;
	u8 query_type = mld_query->query_type, bss_idx, link_id;
	struct bmgr_mlo_dev *mld = NULL;
	struct ml_ie_info *ml_info = NULL;
	...
	case BMGR_QUERY_ML_IE_BCN:			/* mld common info */
		...
		for (link_id = 0; link_id < BSS_MNGR_MAX_BAND_NUM; link_id++) {
			if (entry->csa.repting_csa_offset_in_per_sta_pf[link_id] != 0) {
				mld_query->ie.csa_ie.csa_offset_in_ml_ie[link_id] =
					entry->csa.repting_csa_offset_in_per_sta_pf[link_id] + mld_query->ie.f_len;
				mld_query->ie.csa_ie.csa_bss_idx[link_id] = entry->csa.repting_csa_bss_idx[link_id];
			}

			if (mld_query->ie.csa_ie.csa_offset_in_ml_ie[link_id] != 0)
				MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_NOTICE,
					"\tlink_id=%d, reported link_id=%d, csa_offset_in_per_sta=%d, csa_offset_in_ml=%d\n",
					entry->link_id, link_id, entry->csa.repting_csa_offset_in_per_sta_pf[link_id],
					mld_query->ie.csa_ie.csa_offset_in_ml_ie[link_id]);
		}
...
)
{
	...
	if (entry) {
		...
		if (mld_conn->ml_ie) {
			...
			NdisZeroMemory(ml_info, sizeof(struct ml_ie_info));
			if (parse_multi_link_ie(mld_conn->ml_ie, mld_conn->ml_frag_info,
						ml_info) < 0) {
				MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
...
struct bss_mngr_ops bmgr_ops = {
...
#ifdef CONFIG_6G_SUPPORT
	.bmgr_update_he_bss_discov = bss_mngr_con_update_he_bss_discov,
#endif
	...
	.bmgr_mld_group_addr_set = bss_mngr_con_mld_group_addr_set,
#endif
};
