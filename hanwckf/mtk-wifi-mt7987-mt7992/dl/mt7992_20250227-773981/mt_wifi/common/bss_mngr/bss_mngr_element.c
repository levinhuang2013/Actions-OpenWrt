	...
	u8 **cap_info_ptr)
{
	...
	if (tx_entry) {
		if ((entry->entry_info.auth_mode != tx_entry->entry_info.auth_mode)
			/* Extra Compare the RSN IE but in TKIP cases */
			|| (entry->entry_info.PairwiseCipher != tx_entry->entry_info.PairwiseCipher)
			|| (entry->entry_info.GroupCipher != tx_entry->entry_info.GroupCipher)
			/* RSN Cap Need to compare */
			|| !NdisEqualMemory(entry->entry_info.RsnCap, tx_entry->entry_info.RsnCap, 2)
			/* PMF need to Compare */
			|| (entry->entry_info.MFPC != tx_entry->entry_info.MFPC)
			|| (entry->entry_info.MFPR != tx_entry->entry_info.MFPR)
			) {
...
u8 *build_rnr_ie(struct bmgr_entry *repting_entry, struct bmgr_entry *repted_entry, u8 *f_buf, u8 mld_id)
{
	...
	if (eid) {
		...
		if (bmap->bss_param) {
			...
			if (repted_info->disc_param.iob_type == UNSOLICIT_TX_PROBE_RSP)
				bss_param |= TBTT_INFO_BSS_PARAM_UNSOL_PROBE_RSP;
	...
	*pos_info_len = (pos - pos_info_len);
	...
	if (sta_profile && sta_profile->buf && sta_profile->len) {
		if ((pos - f_buf + sta_profile->len) < BSS_MNGR_MAX_PER_STA_PROFILE_LEN) {
			if (frame_mld_type == BMGR_MLD_APPL_ELEM_STA_PROFILE) {
				entry->csa.repted_csa_offset_in_per_sta_pf[repting_link_id] =
					pos - f_buf + PER_STA_PROILE_CAP_INFO_SIZE;
				MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_NOTICE,
					"Prepare csa.repted_csa_offset_in_per_sta_pf[repting_link_id](%d):%d\n",
					repting_link_id, entry->csa.repted_csa_offset_in_per_sta_pf[repting_link_id]);
				}
...
u8 *build_multi_link_ie(struct bmgr_entry *entry, u8 *f_buf, u8 frame_mld_type, u16 mld_sta_idx)
{
	...
	if (eid) {
		...
		if (present_bmap & DOT11BE_ML_BASIC_PRESENCE_MLD_ID) {
			u8 mld_id = entry->dot11be_mld_id;
	...
	u8 *f_buf)
{
	...
	for (repted_link = 0; repted_link < BSS_MNGR_MAX_BAND_NUM; repted_link++) {
		...
		repting_entry->csa.repting_csa_offset_in_per_sta_pf[repted_link] = 0;
			...
			if (repted_entry && repted_entry->valid && repted_entry->mlo_en) {
				...
				if (per_sta_pf.buf && per_sta_pf.len) {
					if (query_type == BMGR_QUERY_ML_IE_BCN) {
						...
						repting_entry->csa.repting_csa_offset_in_per_sta_pf[repted_entry->link_id] =
							repted_entry->csa.repted_csa_offset_in_per_sta_pf[repting_entry->link_id]
							+ (pos - f_buf);
						repting_entry->csa.repting_csa_bss_idx[repted_entry->link_id] = repted_entry->entry_info.hw_bss_index;
						MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_NOTICE,
							"repting_entry[%d]->csa_offset_in_per_sta_pf[%d]=%d, repting_entry->csa_bss_idx[%d]=%d\n",
							repting_entry->link_id,
							repted_entry->link_id,
							repting_entry->csa.repting_csa_offset_in_per_sta_pf[repted_entry->link_id],
							repted_entry->link_id,
							repting_entry->csa.repting_csa_bss_idx[repted_entry->link_id]);
					}
					NdisMoveMemory(pos, per_sta_pf.buf, per_sta_pf.len);
					pos += per_sta_pf.len;
					MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_NOTICE,
...
int parse_tid_to_link_map_ie(u8 *t2l_ie, struct tid2lnk_ie_info *t2l_info)
{
	...
	end = pos + 2 + t2l_ie_len;

	if ((pos + sizeof(struct _EID_STRUCT) + sizeof(t2l_ctrl)) > end) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
				 "incorrect length\n");
		ret = -1;
		goto err;
	...
	t2l_info->ctrl_mst = GET_DOT11BE_TID2LNK_MAP_CTRL_MST(t2l_ctrl);
	t2l_info->ctrl_ed = GET_DOT11BE_TID2LNK_MAP_CTRL_ED(t2l_ctrl);
	t2l_info->ctrl_size = GET_DOT11BE_TID2LNK_MAP_CTRL_LINK_MAP_SIZE(t2l_ctrl);
	t2l_info->ctrl_map_pres = GET_DOT11BE_TID2LNK_MAP_CTRL_LNK_MAP_PRES(t2l_ctrl);

	if (t2l_info->ctrl_mst) {
		if (pos + sizeof(t2l_info->mst_tsf) > end) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			 "incorrect length\n");
			ret = -1;
			goto err;
		}
		NdisMoveMemory(&t2l_info->mst_tsf, (u8 *)pos, sizeof(t2l_info->mst_tsf));
		t2l_info->mst_tsf = cpu_to_le16(t2l_info->mst_tsf);
		pos += sizeof(t2l_info->mst_tsf);
	}
	if (t2l_info->ctrl_ed) {
		if (pos + 3 > end) {  /*expected duration is 3 octets defined by spec*/
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			 "incorrect length\n");
			ret = -1;
			goto err;
		}
		NdisMoveMemory(&t2l_info->ed_tsf, (u8 *)pos, 3);
		t2l_info->ed_tsf = cpu_to_le32(t2l_info->ed_tsf);
		pos += 3;
	}

	if (t2l_info->ctrl_map_pres != 0xff) { /*all of the tids need mapping to one link at least*/
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
	...
	if (t2l_info->ctrl_size == 0) { /*2 octets*/
		if ((pos + MAX_TID_MAPPING_NUM * 2) > end) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			 "incorrect length\n");
			ret = -1;
			goto err;
		...
		if ((pos + MAX_TID_MAPPING_NUM) > end) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
			 "incorrect length\n");
			ret = -1;
			goto err;
	...
	tid_map = &t2l_info->tid_map[0];
	for (tid = 0 ; tid < MAX_TID_MAPPING_NUM; tid++) {
		if (t2l_info->ctrl_size == 0) { /*2 octets*/
			NdisMoveMemory(&link_map, pos, 2);
			link_map = cpu_to_le16(link_map);
			pos += 2;
		} else { /*1 octet*/
			NdisMoveMemory(&link_map, pos, 1);
			pos += 1;
		}
	...
	hex_dump_with_cat_and_lvl("T2L IE:", (u8 *)t2l_ie, (pos - t2l_ie),
		DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_DEBUG);
	MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_INFO,
		"\t mst=%d, ed=%d, mst_tsf=%d, ed_tsf=%d, bcn_tsf=%d\n", t2l_info->ctrl_mst, t2l_info->ctrl_ed,
		t2l_info->mst_tsf, t2l_info->ed_tsf, t2l_info->bcn_tsf);
