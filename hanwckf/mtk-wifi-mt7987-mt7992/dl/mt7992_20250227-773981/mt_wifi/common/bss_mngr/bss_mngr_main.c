...
u8 bss_mngr_event_update_iob_disc(PNET_DEV pNetDev, struct bmgr_disc_param *disc_param)
{
	struct wifi_dev *wdev = RtmpOsGetNetDevWdev(pNetDev);

	MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_INFO,
		"iob type %d, interval %d, mode %d\n",
		disc_param->iob_type, disc_param->iob_interval, disc_param->iob_mode);

	return in_band_discovery_update_oper(wdev,
		disc_param->iob_type, disc_param->iob_interval, disc_param->iob_mode);
}
...
#ifdef CONFIG_6G_SUPPORT
	.bmgr_event_update_iob_disc = bss_mngr_event_update_iob_disc,
#endif
#ifdef DOT11_EHT_BE
...
)
{
...
#ifdef CONFIG_AP_SUPPORT
#ifdef CONFIG_6G_SUPPORT
	reg_info->disc_param.iob_by_cfg = wlan_config_get_unsolicit_tx_by_cfg(wdev);
	reg_info->disc_param.iob_type = wlan_operate_get_unsolicit_tx_type(wdev);
	reg_info->disc_param.iob_interval = wlan_operate_get_unsolicit_tx_tu(wdev);
	reg_info->disc_param.iob_mode = wlan_operate_get_unsolicit_tx_mode(wdev);
	reg_info->disc_param.rnr_6g_rule = wlan_config_get_rnr_in_probe_rsp(wdev);
#endif /* CONFIG_6G_SUPPORT */
	...
	reg_info->MFPC			= pSecConfig->PmfCfg.MFPC;
	reg_info->MFPR			= pSecConfig->PmfCfg.MFPR;
#endif
#ifdef DISABLE_HOSTAPD_BEACON
	reg_info->RsnCap[0]		= pSecConfig->RsnCap[0];
	reg_info->RsnCap[1]		= pSecConfig->RsnCap[1];
...
#ifdef CONFIG_6G_SUPPORT
/**
 * API that update the parameters of in-of-band/out-of-band discovery for HE-capable BSS.
 *
 * @wdev: HE-capable BSS
 * @update_type: indicate iob (BSS_IOB_DISCOV_UPDATE) or oob (BSS_OOB_DISCOV_UPDATE)
 */
NDIS_STATUS bss_mngr_update_he_bss_discov(struct wifi_dev *wdev, u8 update_type)
{
	int ret = NDIS_STATUS_FAILURE;
	struct bmgr_disc_param disc_param = {0};

	if (wdev) {
		if (wdev->wdev_type == WDEV_TYPE_AP) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_INFO,
					"wdev(%d), update %d\n", wdev->wdev_idx, update_type);

			disc_param.iob_by_cfg = wlan_config_get_unsolicit_tx_by_cfg(wdev);
			disc_param.iob_type = wlan_operate_get_unsolicit_tx_type(wdev);
			disc_param.iob_interval = wlan_operate_get_unsolicit_tx_tu(wdev);
			disc_param.iob_mode = wlan_operate_get_unsolicit_tx_mode(wdev);
			disc_param.rnr_6g_rule = wlan_config_get_rnr_in_probe_rsp(wdev);

			ret = bmgr_ops.bmgr_update_he_bss_discov(wdev->if_dev, update_type, &disc_param);
		}
	}
...
NDIS_STATUS bss_mngr_mld_group_addr_set(
	struct wifi_dev *wdev,
	u8 mld_grp,
	u8 *mld_addr)
{
	return bmgr_ops.bmgr_mld_group_addr_set(wdev, mld_grp, mld_addr);
}
