...
{
	if (mode < MODE_HTMIX || mode == MODE_UNKNOWN)
		return "NA";
	...
	IN	ULONG					Data)
{
	...
	case CMD_RTPRIV_IOCTL_BIND_VLAN: {
		MAC_TABLE_ENTRY *pEntry = (MAC_TABLE_ENTRY *)pData;

		pEntry->vlan_id = (UINT32)Data;
#ifdef WHNAT_SUPPORT
		if (PD_GET_WHNAT_ENABLE(pAd->physical_dev)) {
			...
			strec->EnableFeature |= STA_REC_TX_PROC_FEATURE;
	...
	case CMD_RTPRIV_IOCTL_FREQ_LIST_GET: {
		...
		for (i = 1; i <= pChCtrl->ChListNum; i++) {
			m = 2412000;
			m = channel_to_frequency(pAd->BandSel, pChCtrl->ChList[i - 1].Channel);
			m = m * 1000;
			(*pFreq) = m;
			pFreq++;
		}
...
IOCTL_ATE_ERROR:
		...
		/* handle for SIOCGIWRATEQ */
	{
		...
		if (WMODE_CAP_BE(wdev->PhyMode)) {
			...
			MCS = 13;
			gi = wlan_config_get_he_gi(wdev);

			RtmpDrvMaxRateGet(pAd, MODE_EHT, gi,
				  BW, MCS,
				  Antenna,
				  &pRate->BitRate);
			break;
...
#define SER_FE_CASE3_FORCE_L1_FAIL		(990)
#endif
...
VOID RoamCalibData(RTMP_ADAPTER *pAd, UCHAR *macAddr, UCHAR *Bssid, char *desc, const char *func_name)
{
	INT i = 0;

	if (!NdisCmpMemory(macAddr, ZERO_MAC_ADDR, MAC_ADDR_LEN)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
			"\tError: Addr is zero MAC\n");
		return;
	}

	for (i = 0; i < ROAM_STA_CNT; i++) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_DEBUG,
			"Roaming addr:%pM, cmd addr %pM\n",
			pAd->RoamingSta[i].StaAddr, macAddr);
		if (NdisCmpMemory(pAd->RoamingSta[i].StaAddr, macAddr, MAC_ADDR_LEN) == 0) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_DEBUG,
				"Current data cnt = %d\n", pAd->RoamingSta[i].data_cnt);
			if (Bssid) {
				NdisCopyMemory(pAd->RoamingSta[i].Bssid, Bssid, MAC_ADDR_LEN);
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_DEBUG,
					"Roaming Copy Bssid Mac addr %02X:%02X:%02X:%02X:%02X:%02X\n",
					PRINT_MAC(pAd->RoamingSta[i].Bssid));
			}
			if (pAd->RoamingSta[i].data_cnt < ROAM_DATA_CNT) {
				NdisCopyMemory(pAd->RoamingSta[i].data[pAd->RoamingSta[i].data_cnt].functionName, func_name, ROAM_FUNC_LEN);
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_DEBUG,
					"Roaming OS TRace caller:%s\n", pAd->RoamingSta[i].data[pAd->RoamingSta[i].data_cnt].functionName);
				NdisCopyMemory(pAd->RoamingSta[i].data[pAd->RoamingSta[i].data_cnt].desc, desc, ROAM_DESC_LEN);
				ktime_get_real_ts64(&pAd->RoamingSta[i].data[pAd->RoamingSta[i].data_cnt].tv);
				pAd->RoamingSta[i].data_cnt++;
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_DEBUG,
					"New data cnt = %d\n", pAd->RoamingSta[i].data_cnt);
			} else {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
					"Max data cnt reached= %d\n", pAd->RoamingSta[i].data_cnt);
			}
			return;
		}
	}
...
INT Set_AMPDU_Retry_Count(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MtCmdCr4Set(pAd, WA_SET_OPTION_PER_BSS_RETRY, (UINT32)bss_idx, count);
	if (count == AMPDU_MAX_REMAIN_TX_COUNT)
		MTWF_PRINT("%s(): Unlimited retry!\n", __func__);
	else if (count == 0)
		MTWF_PRINT("%s(): Auto retry control!\n", __func__);
...
INT Set_PsFlush_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	RTMP_STRING *str1  = NULL;
	RTMP_STRING *str2  = NULL;
	RTMP_STRING *str3  = NULL;
	UCHAR PsFlushPeriod;
	UCHAR PsFlushSTAThreshold;
	UCHAR PsFlushTokenThreshold;
	UCHAR PsFlushPLEThreshold;

	MTWF_PRINT("%s(): before [Period]-[STAThld]-[TokenThld]-[PLEThld] = %d-%d-%d-%d\n",
		__func__, pAd->Mlme.PsFlushPeriod,
		pAd->Mlme.PsFlushSTAThreshold,
		pAd->Mlme.PsFlushTokenThreshold,
		pAd->Mlme.PsFlushPLEThreshold);
	if (arg == NULL || strlen(arg) == 0)
		return FALSE;
	str1 = strsep(&arg, "-");
	str2 = strsep(&arg, "-");
	str3 = strsep(&arg, "-");
	if (str1 == NULL || str2 == NULL || str3 == NULL || arg == NULL)
		return FALSE;
	PsFlushPeriod = os_str_tol(str1, 0, 10);
	PsFlushSTAThreshold = os_str_tol(str2, 0, 10);
	PsFlushTokenThreshold = os_str_tol(str3, 0, 10);
	PsFlushPLEThreshold = os_str_tol(arg, 0, 10);
	pAd->Mlme.PsFlushPeriod = PsFlushPeriod;
	pAd->Mlme.PsFlushSTAThreshold = PsFlushSTAThreshold;
	pAd->Mlme.PsFlushTokenThreshold = PsFlushTokenThreshold;
	pAd->Mlme.PsFlushPLEThreshold = PsFlushPLEThreshold;
	MTWF_PRINT("%s(): after [Period]-[STAThld]-[TokenThld]-[PLEThld] = %d-%d-%d-%d\n",
		__func__, pAd->Mlme.PsFlushPeriod,
		pAd->Mlme.PsFlushSTAThreshold,
		pAd->Mlme.PsFlushTokenThreshold,
		pAd->Mlme.PsFlushPLEThreshold);
...
INT Set_PsFlushNum_Porc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT16 ps_flush_num = 64;
	UINT BandIdx;

	if (pAd == NULL || arg == NULL || strlen(arg) == 0)
		return FALSE;

	ps_flush_num = os_str_tol(arg, 0, 10);
	if (ps_flush_num == 0) {
		MTWF_PRINT("%s(): ps flush num can't be 0!\n", __func__);
		return FALSE;
	}

	MTWF_PRINT("%s(): before [PsFlushNum] = %d\n", __func__, pAd->Mlme.PsFlushPLENum);
	pAd->Mlme.PsFlushPLENum = ps_flush_num;
	BandIdx = hc_get_hw_band_idx(pAd);
	switch (BandIdx) {
	case 0:
		RTMP_FLUSH_PS_STA_PLE_NUM(pAd, ps_flush_num, 0x1);
		break;
	case 1:
		RTMP_FLUSH_PS_STA_PLE_NUM(pAd, ps_flush_num, 0x2);
		break;
	case 2:
		RTMP_FLUSH_PS_STA_PLE_NUM(pAd, ps_flush_num, 0x4);
		break;
	default:
		break;
	}
	MTWF_PRINT("%s(): after [PsFlushNum] = %d", __func__, pAd->Mlme.PsFlushPLENum);
	return TRUE;
}
...
INT Set_TxCmdEDCA_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT i4Recv = 0, ret = NDIS_STATUS_SUCCESS;
	UCHAR idx, cwmin, cwmax, aifs;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	struct wifi_dev *wdev = NULL;
	struct _EDCA_PARM EdcaParm = {0};

	if (!arg)
		goto Error;

	i4Recv = sscanf(arg, "%hhu-%hhu-%hhu-%hhu", &(idx), &(cwmin), &(cwmax), &(aifs));
	if (i4Recv != 4)
		goto Error;
	...
	if (idx > 7) {//EDCA Table Index (0~7)
		MTWF_PRINT("invalid idx(%d)!\n", idx);
		goto Error;
	}

	if (cwmin > 0xF || cwmax > 0xF || cwmin > cwmax) {
		MTWF_PRINT("invalid cwmin/cwmax(%d/%d)!\n", cwmin, cwmax);
		goto Error;
	}
	if (aifs <= 0x0 || aifs > 0xF) {
		MTWF_PRINT("invalid aifs(%d)!\n", aifs);
		goto Error;
	}

	EdcaParm.bValid = TRUE;
	EdcaParm.Cwmin[0] = cwmin;
	EdcaParm.Cwmax[0] = cwmax;
	EdcaParm.Aifsn[0] = aifs;

#ifdef WIFI_UNIFIED_COMMAND
	ret = AsicSetTxCmdEDCA(pAd, wdev, idx, &EdcaParm);
	if (ret == NDIS_STATUS_SUCCESS) {
		MTWF_PRINT("Set TxCmd EDCA setting, idx(%d): cwmin/cwmax/aifs = %d/%d/%d\n",
			idx, cwmin, cwmax, aifs);
		return TRUE;
	}
#endif /* WIFI_UNIFIED_COMMAND */
...
Error:
	...
	MTWF_PRINT("iwpriv/mwctl ra0 set TxCmdEDCA=[idx]-[cwmin]-[cwmax]-[aifs]\n");
	MTWF_PRINT("[idx]: 0~7, TxCmd EDCA Table Index\n");
	MTWF_PRINT("[cwmin]: 0~15\n");
	MTWF_PRINT("[cwmax]: 0~15, should >= cwmin\n");
	MTWF_PRINT("[aifs]: 1~15\n");
	return FALSE;
}

INT Set_PerStaWmm_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT i4Recv = 0, ret = NDIS_STATUS_SUCCESS;
	UINT16 wcid;
	UCHAR ac, idx;
	...
	i4Recv = sscanf(arg, "%hu-%hhu-%hhu", &(wcid), &(ac), &(idx));
	if (i4Recv != 3)
		goto Error;
	...
	if (wcid == 0 || !VALID_UCAST_ENTRY_WCID(pAd, wcid)) {
		MTWF_PRINT("invalid wcid(%d)!\n", wcid);
		goto Error;
	}
	pEntry = entry_get(pAd, wcid);
	if (!pEntry || (pEntry->Sst != SST_ASSOC)) {
		MTWF_PRINT("STA(%d) is not connected!\n", wcid);
		goto Error;
	}

	if (ac >= WMM_NUM_OF_AC) {
		MTWF_PRINT("invalid ac(%d)!\n", ac);
		goto Error;
	}

	if (idx > 7) {//EDCA Table Index (0~7)
		MTWF_PRINT("invalid idx(%d) > 7, means disable EDCA setting!\n", idx);
		idx = 0x8;//invalid index
	}

#ifdef WIFI_UNIFIED_COMMAND
	ret = AsicSetPerStaWmm(pAd, wdev, wcid, ac, idx);
	if (ret == NDIS_STATUS_SUCCESS) {
		MTWF_PRINT("Set Per STA(%d) AC(%d) WMM setting (EDCA idx = %d).\n",
			wcid, ac, idx);
		return TRUE;
	}
#endif /* WIFI_UNIFIED_COMMAND */
...
Error:
	MTWF_PRINT("error input parameter!\n");
	MTWF_PRINT("iwpriv/mwctl ra0 set PerStaWmm=[wcid]-[ac]-[idx]\n");
	MTWF_PRINT("[wcid]: 1-STA1\n");
	MTWF_PRINT("[ac]: 0~3, AC0/1/2/3\n");
	MTWF_PRINT("[idx]: 0~7, > 7 means disable EDCA setting.\n");
	return FALSE;
}
...
INT32 set_datcfg_ack_cts_timeout (RTMP_ADAPTER *pAd)
{
	...
	if (pAd->CommonCfg.distance > 0) {
		...
		bCCKTimeout = DEFAULT_TMAC_CDTR_VALUE;
		...
		bOFDMTimeout = DEFAULT_TMAC_ODTR_VALUE;
		...
		bOFDMATimeout = DEFAULT_TMAC_OMDTR_VALUE;
...
INT set_dst2acktimeout_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	bTimeout = distance / LIGHT_SPEED * 2;
	if (bTimeout > MAX_ACK_TIMEOUT) {
	...
	*/
	bCckCCAtout = (DEFAULT_TMAC_CDTR_VALUE >> 16) + bTimeout;
	bCckMDRDYtout = (DEFAULT_TMAC_CDTR_VALUE & MAX_ACK_TIMEOUT) + bTimeout;
	bOfdmCCAtout = (DEFAULT_TMAC_ODTR_VALUE >> 16) + bTimeout;
	bOfdmMDRDYtout = (DEFAULT_TMAC_ODTR_VALUE & MAX_ACK_TIMEOUT) + bTimeout;
	bOfdmaCCAtout = (DEFAULT_TMAC_OMDTR_VALUE >> 16) + bTimeout;
	bOfdmaMDRDYtout = (DEFAULT_TMAC_OMDTR_VALUE & MAX_ACK_TIMEOUT) + bTimeout;
	...
	bCCKTimeout = DEFAULT_TMAC_CDTR_VALUE;
	bCCKTimeout += (bTimeout << 16) | bTimeout;
	if (set_cck_ofdm_ofdma_tout(pAd, bCCKTimeout, CCK_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
	...
	bOFDMTimeout = DEFAULT_TMAC_ODTR_VALUE;
	bOFDMTimeout += (bTimeout << 16) | bTimeout;
	if (set_cck_ofdm_ofdma_tout(pAd, bOFDMTimeout, OFDM_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
	...
	bOFDMATimeout = DEFAULT_TMAC_OMDTR_VALUE;
	bOFDMATimeout += (bTimeout << 16) | bTimeout;
	if (set_cck_ofdm_ofdma_tout(pAd, bOFDMATimeout, OFDMA_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
...
INT mtk_cfg80211_set_dst2acktimeout(RTMP_ADAPTER *pAd, UINT distance)
{
	...
	bCCKTimeout = DEFAULT_TMAC_CDTR_VALUE;
	bCCKTimeout += (bTimeout << 16) | bTimeout;
	if (set_ack_timeout_mode_byband(pAd, bCCKTimeout, band_idx, CCK_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"SET CTS/ACK CCK Timeout Fail!!\n");
		return -EFAULT;
	}
	...
	bOFDMTimeout = DEFAULT_TMAC_ODTR_VALUE;
	bOFDMTimeout += (bTimeout << 16) | bTimeout;
	if (set_ack_timeout_mode_byband(pAd, bOFDMTimeout, band_idx, OFDM_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"SET CTS/ACK OFDM Timeout Fail!!\n");
		return -EFAULT;
	}
	...
	bOFDMATimeout = DEFAULT_TMAC_OMDTR_VALUE;
	bOFDMATimeout += (bTimeout << 16) | bTimeout;
	if (set_ack_timeout_mode_byband(pAd, bOFDMATimeout, band_idx, OFDMA_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"SET CTS/ACK OFDMA Timeout Fail!!\n");
		return -EFAULT;
	}
...
INT set_cck_ack_timeout_porc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (set_cck_ofdm_ofdma_tout(pAd, bTimeout, CCK_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
			"SET CTS/ACK Timeout Fail!!\n");
		return FALSE;
	}
...
INT mtk_cfg80211_set_cck_ack_tout(RTMP_ADAPTER *pAd, UINT cck_ack_tout)
{
	...
	bCCAtout = (cck_ack_tout >> 16) & MAX_ACK_TIMEOUT;
	bMDRDYtout = cck_ack_tout & MAX_ACK_TIMEOUT;
	if ((bCCAtout <= 0 || bCCAtout > MAX_ACK_TIMEOUT)
		|| (bMDRDYtout <= 0 || bMDRDYtout > MAX_ACK_TIMEOUT)) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"CTS/ACK Timeout Range should between [0xFFFF:0)!!\n");
...
INT set_ofdm_ack_timeout_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (set_cck_ofdm_ofdma_tout(pAd, bTimeout, OFDM_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
			"SET CTS/ACK Timeout Fail!!\n");
		return FALSE;
	}
...
INT mtk_cfg80211_set_ofdm_ack_tout(RTMP_ADAPTER *pAd, UINT ofdm_ack_tout)
{
	...
	bCCAtout = (ofdm_ack_tout >> 16) & MAX_ACK_TIMEOUT;
	bMDRDYtout = ofdm_ack_tout & MAX_ACK_TIMEOUT;
	if ((bCCAtout <= 0 || bCCAtout > MAX_ACK_TIMEOUT)
		|| (bMDRDYtout <= 0 || bMDRDYtout > MAX_ACK_TIMEOUT)) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"CTS/ACK Timeout Range should between [0xFFFF:0)!!\n");
...
INT set_ofdma_ack_timeout_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (set_cck_ofdm_ofdma_tout(pAd, bTimeout, OFDMA_TIME_OUT) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_ERROR,
			"SET CTS/ACK Timeout Fail!!\n");
		return FALSE;
	}
...
INT mtk_cfg80211_set_ofdma_ack_tout(RTMP_ADAPTER *pAd, UINT ofdma_ack_tout)
{
	...
	bCCAtout = (ofdma_ack_tout >> 16) & MAX_ACK_TIMEOUT;
	bMDRDYtout = ofdma_ack_tout & MAX_ACK_TIMEOUT;
	if ((bCCAtout <= 0 || bCCAtout > MAX_ACK_TIMEOUT)
		|| (bMDRDYtout <= 0 || bMDRDYtout > MAX_ACK_TIMEOUT)) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_MAC, DBG_LVL_WARN,
			"CTS/ACK Timeout Range should between [0xFFFF:0)!!\n");
...
void Get_Gtb_Sta_Info(IN RTMP_ADAPTER * pAd, IN OUT struct wifi_sta_info *sta_info)
{
	PMAC_TABLE_ENTRY pEntry = NULL;
	struct _STA_TR_ENTRY *tr_entry;
	EXT_EVENT_TX_STATISTIC_RESULT_T rTxStatResult;
	...
	/* CAPs*/
	sta_info->XtraRange = pEntry->MlrMode;
	...
	/* Statistic */
	sta_info->TxBytes = pEntry->TxBytes;
	sta_info->RxBytes = pEntry->RxBytes;
	sta_info->AvgTxBytes = pEntry->AvgTxBytes;
	sta_info->AvgRxBytes = pEntry->AvgRxBytes;
	sta_info->OneSecTxBytes = pEntry->OneSecTxBytes;
	sta_info->OneSecRxBytes = pEntry->OneSecRxBytes;
	...
	sta_info->BW = rTxStatResult.rEntryTxRate.BW;
	sta_info->Nss = rTxStatResult.rEntryTxRate.VhtNss;
	sta_info->MCS = rTxStatResult.rEntryTxRate.MCS;

	/* Latency */
	sta_info->tx_latency_max = pEntry->tx_latency_stats.max;
	sta_info->tx_latency_min = pEntry->tx_latency_stats.min;
	sta_info->tx_latency_max_t2 = pEntry->tx_latency_stats.max_t2;
	sta_info->tx_latency_min_t2 = pEntry->tx_latency_stats.min_t2;

	/* Air Latency*/
	sta_info->air_latency_max = pEntry->tx_latency_stats.air_max;
	sta_info->air_latency_min = pEntry->tx_latency_stats.air_min;
	sta_info->air_latency_max_t2 = pEntry->tx_latency_stats.air_max_t2;
	sta_info->air_latency_min_t2 = pEntry->tx_latency_stats.air_min_t2;

	sta_info->valid = TRUE;

}

VOID OneSecTxLatencyCal(IN PMAC_TABLE_ENTRY pEntry, IN UINT32 transmitDelay, IN UINT32 airDelay, IN UINT32 txCnt)
{
	ULONG now_time;
	...
	/* one-second latency */
	if (RTMP_TIME_AFTER(now_time, pEntry->tx_latency_stats.last_calc_time + (1 * OS_HZ))) {
		pEntry->tx_latency_stats.last_calc_time = now_time;
		pEntry->tx_latency_stats.max = 0;
		pEntry->tx_latency_stats.air_max = 0;
	}

	if (pEntry->tx_latency_stats.max == 0) {
		pEntry->tx_latency_stats.min = transmitDelay;
		pEntry->tx_latency_stats.max = transmitDelay;
		pEntry->tx_latency_stats.air_min = airDelay;
		pEntry->tx_latency_stats.air_max = airDelay;
	} else {
		if (transmitDelay > 0) {
			if (pEntry->tx_latency_stats.min > transmitDelay)
				pEntry->tx_latency_stats.min = transmitDelay;

			if (pEntry->tx_latency_stats.max < transmitDelay)
				pEntry->tx_latency_stats.max = transmitDelay;

			if (pEntry->tx_latency_stats.air_min > airDelay)
				pEntry->tx_latency_stats.air_min = airDelay;

			if (pEntry->tx_latency_stats.air_max < airDelay)
				pEntry->tx_latency_stats.air_max = airDelay;
		}
	}

	/* half-sec latency */
	if (RTMP_TIME_AFTER(now_time, pEntry->tx_latency_stats.last_calc_time_t2 + (OS_HZ >> 1))) {
		pEntry->tx_latency_stats.last_calc_time_t2 = now_time;
		pEntry->tx_latency_stats.max_t2 = 0;
		pEntry->tx_latency_stats.air_max_t2 = 0;
	}

	if (pEntry->tx_latency_stats.max_t2 == 0) {
		pEntry->tx_latency_stats.min_t2 = transmitDelay;
		pEntry->tx_latency_stats.max_t2 = transmitDelay;
		pEntry->tx_latency_stats.air_min_t2 = airDelay;
		pEntry->tx_latency_stats.air_max_t2 = airDelay;
	} else {
		if (transmitDelay > 0) {
			if (pEntry->tx_latency_stats.min_t2 > transmitDelay)
				pEntry->tx_latency_stats.min_t2 = transmitDelay;

			if (pEntry->tx_latency_stats.max_t2 < transmitDelay)
				pEntry->tx_latency_stats.max_t2 = transmitDelay;

			if (pEntry->tx_latency_stats.air_min_t2 > airDelay)
				pEntry->tx_latency_stats.air_min_t2 = airDelay;

			if (pEntry->tx_latency_stats.air_max_t2 < airDelay)
				pEntry->tx_latency_stats.air_max_t2 = airDelay;
		}
	}
}
...
VOID CalStaTxLatency(IN PMAC_TABLE_ENTRY pEntry, IN UINT32 transmitDelay, IN UINT32 airDelay, IN UINT32 txCnt)
{
	...
	pEntry->tx_cnt_avg += txCnt;
#endif /* EAP_ENHANCE_STATS_SUPPORT */

	/* PKT Cnt under different air delay */
	if (airDelay < 15)
		pEntry->tx_pkt_hist[airDelay]++;
	else
		pEntry->tx_pkt_hist[15]++;

	if (txCnt <= 0xF) {
		/* PKT Cnt of Retry x, MPDU Base */
		pEntry->tx_pkt_ret[txCnt]++;

		/* Avg Latency of Retry X PKT */
		pEntry->tx_pkt_ret_dly_avg[txCnt] += transmitDelay;
	}
