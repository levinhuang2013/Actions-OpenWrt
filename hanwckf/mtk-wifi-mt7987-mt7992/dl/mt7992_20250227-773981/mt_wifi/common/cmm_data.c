	...
	if (pSrcBuf) {
		...
		if (TypeLen == ETH_TYPE_EAPOL) {
			...
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, pSrcBuf, NULL, "Send EAPOL to STA", __func__);
...
#ifdef DOT11_EHT_BE
/**
* sta_mld_rx_chk_duplicate_bmc_frame() - poll interface infos
* @pAd:	wifi adapter.
* @pRxBlk: rxblk infos.
*
* For dot11be d4.0: 10.3.2.14.3 Receiver requirements
* An MLD shall implement RC16 in Table 10-6 (Receiver caches) maintained
* by theMLD to discard duplicate group addressed Data that are delivered
* from the associated MLD. A duplicated group addressed Data frame
* received on any link shall be discarded. The method used to handle the
* sequence number wrap around for duplicate detection is implementation
* specific.
*
* return:  0  success, other fail
*/
INT sta_mld_rx_chk_duplicate_bmc_frame(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk)
{
	...
	sn = pRxBlk->SN;
	/*only mlo entry should check it*/
	entry = MacTableLookup(pAd, pRxBlk->Addr2);
	if (entry == NULL)
		return NDIS_STATUS_SUCCESS;
	...
	if (!pFmeCtrl->Retry && seq_ctrl->cacheBmcSn != sn) {
		seq_ctrl->cacheBmcSn = sn;
		MTWF_DBG(pAd, DBG_CAT_RX, CATRX_RXINFO, DBG_LVL_INFO,
		"pFrameCtrl->Retry=%d, cachebmcSn=%d, sn=%d\n",
		pFmeCtrl->Retry, seq_ctrl->cacheBmcSn, sn);
		return NDIS_STATUS_SUCCESS;
	}
...
* sequence number wrap around for duplicate detection is implementation
* specific.
* For mediatek design, only the setuplink is in the bridge, any link rx
* bmc should do the seq check and forward to the setup net dev.
*
* return:  void
*/
void sta_mld_rx_bmc_check_forward(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, struct sk_buff *pOSPkt)
{
	UCHAR *da = NULL;
	struct wifi_dev *setup_wdev = NULL, *wdev = NULL;
	struct _RTMP_ADAPTER *pad = NULL;
	MAC_TABLE_ENTRY *setup_entry = NULL;
	MAC_TABLE_ENTRY *pEntry = NULL;
	FRAME_CONTROL *fc = NULL;

	if (pAd == NULL || pRxBlk == NULL || pOSPkt == NULL) {
		if (pOSPkt)
			pOSPkt->dev = NULL;
		return;
	}
	...
	if (fc->FrDs == 1) {
		...
		if (pRxBlk->Addr1 && pRxBlk->Addr2 &&
			(pRxBlk->pRxInfo->Bcast || pRxBlk->pRxInfo->Mcast)) {
			da = pRxBlk->Addr1;
			pEntry = MacTableLookup(pAd, pRxBlk->Addr2);
			if (pEntry && pEntry->wdev && IS_BROADCAST_MAC_ADDR(da)
				&& pEntry->wdev->wdev_type == WDEV_TYPE_STA &&
				pEntry->mlo.mlo_en && pEntry->wdev->mld_dev) {
				wdev = pEntry->wdev;
				setup_wdev = get_sta_mld_setup_wdev(wdev->mld_dev);
				if (setup_wdev == NULL) {
					pOSPkt->dev = NULL;
					return;
				}
				pad = (PRTMP_ADAPTER)setup_wdev->sys_handle;
				if (pad == NULL) {
					pOSPkt->dev = NULL;
					return;
				}
				/*check duplicate*/
				if (sta_mld_rx_chk_duplicate_bmc_frame(pAd, pRxBlk) == NDIS_STATUS_FAILURE) {
					MTWF_DBG(pAd, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_INFO,
					"pentry(%pM), wdev if addr(%pM).setup_wdev if addr(%pM) Drop it\n",
					pEntry->Addr, wdev->if_addr, setup_wdev->if_addr);
					pOSPkt->dev = NULL;
					return;
				}
				setup_entry = GetAssociatedAPByWdev(pad, setup_wdev);
				if (setup_entry == NULL) {
					pOSPkt->dev = NULL;
					return;
				}
				/* covert link to setuplink */
				MTWF_DBG(pAd, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_INFO,
				"old:pentry(%pM), wdev if addr(%pM).setup_wdev if addr(%pM)\n",
				pEntry->Addr, wdev->if_addr, setup_wdev->if_addr);
				pEntry = setup_entry;
				pRxBlk->wcid = setup_entry->wcid;
				pOSPkt->dev = setup_wdev->if_dev;
				pAd = setup_wdev->sys_handle;
				RTMP_SET_PACKET_WCID(pRxBlk->pRxPacket, pRxBlk->wcid);
				MTWF_DBG(pAd, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_INFO,
				"new:pentry(%pM), wdev if addr(%pM).setup_wdev if addr(%pM)\n",
				pEntry->Addr, wdev->if_addr, setup_wdev->if_addr);
			}
		}
	}
...
VOID indicate_802_3_pkt(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx)
{
...
#ifdef DOT11_EHT_BE
	sta_mld_rx_bmc_check_forward(pAd, pRxBlk, pOSPkt);
#endif /*DOT11_EHT_BE*/
	...
	if (pOSPkt->dev == NULL) {
		...
		if (bss_mngr_is_wdev_in_mlo_group(wdev)) {
			...
			bss_mngr_query_mld_ap_basic(wdev, &bss_mld_info_basic);
			if (bss_mld_info_basic.agent_bss_wdev) {
				struct wifi_dev *agent_bss = bss_mld_info_basic.agent_bss_wdev;

				IN RTMP_ADAPTER *ad = (struct _RTMP_ADAPTER *)agent_bss->sys_handle;

				if (pRxBlk->isIgmporMld) {
					SET_OS_PKT_NETDEV((PNDIS_PACKET)pRxBlk->pRxPacket, agent_bss->if_dev);
					ad = (struct _RTMP_ADAPTER *)agent_bss->sys_handle;
					wdev = agent_bss;
				}
			}
		}
		...
		announce_or_forward_802_3_pkt(ad, pRxBlk->pRxPacket, wdev, ad->OpMode);
	}
}
	...
	IN UCHAR wdev_idx)
{
	...
	hdr_len += qos_len;
#ifdef ROAM_CALIB_SUPPORT
	if (pEntry && pAd->RoamCalibEnable)
		RoamCalibData(pAd, pEntry->Addr, NULL, "Eapol Received from STA", __func__);
#endif /* ROAM_CALIB_SUPPORT */
#ifdef CONFIG_AP_SUPPORT
	...
	if (pEntry) {
		...
		if (!RTMPCheckWPAframe(pAd, pEntry, pData, data_len, wdev_idx, eth_frame)) {
			...
			if ((pEntry->wdev->wdev_type == WDEV_TYPE_AP || pEntry->wdev->wdev_type == WDEV_TYPE_GO) &&
				...
				/*
				   If
				   1) no any EAP frame is received within 5 sec and
				...
				if (FC != NULL && FC->Wep &&
					...
					USHORT reason = REASON_DEAUTH_STA_LEAVING;

					if (IS_AKM_PSK_Entry(pEntry))
						reason = REASON_4_WAY_TIMEOUT;
#ifdef DOT1X_SUPPORT
					else if (IS_AKM_1X_Entry(pEntry) || IS_IEEE8021X_Entry(wdev))
						reason = REASON_8021X_AUTH_FAIL;
					MTWF_DBG(pAd, DBG_CAT_RX, CATRX_RXINFO, DBG_LVL_WARN,
						"==> De-Auth this STA("MACSTR"), reason=%d\n",
						MAC2STR(pEntry->Addr), reason);
					MlmeDeAuthAction(pAd, pEntry, reason, FALSE);
				}
#endif /* DOT1X_SUPPORT */
				RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
				return;
...
INT rx_chk_duplicate_frame(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, struct wifi_dev *wdev)
{
	...
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU)) {
		...
		/* Update cacheSN to previous_sn if AMSDU missing occurred */
		...
		/* Record SN & amsdu_state for next time */
		...
		/* Update cacheSN to SN at final amsdu */
...
#ifdef TXRX_STAT_SUPPORT
	if (IS_WCID_VALID(pAd, pRxBlk->wcid)) {
		MAC_TABLE_ENTRY *pEntry_RX = NULL;

		pEntry_RX = entry_get(pAd, pRxBlk->wcid);
		if ((pEntry_RX != NULL) && IS_ENTRY_CLIENT(pEntry_RX) && FC->Retry)
			INC_COUNTER64(pEntry_RX->RxRetryPktCount);
	}
#endif /* TXRX_STAT_SUPPORT*/
...
INT wdev_tx_pkts(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt, struct wifi_dev *wdev)
{
	...
	for (Index = 0; Index < pkt_cnt; Index++) {
		...
		/* packet is from wifi, not from os, so do a4 send and send data*/
		if (RTMP_GET_PACKET_WIFI_FORWARD(pPacket)) {
#ifdef A4_CONN
			UCHAR *pSrcBufVA = GET_OS_PKT_DATAPTR(pPacket);

			/* here can delete last para */
			if (MAC_ADDR_IS_GROUP(pSrcBufVA))
				a4_send_clone_pkt(pAd, wdev->func_idx, pPacket);
...
#ifdef ROAM_CALIB_SUPPORT
					if (pAd->RoamCalibEnable)
						RoamCalibData(pAd, pEntry->Addr, NULL, "Send EAPOL to STA", __func__);
#endif /* ROAM_CALIB_SUPPORT */
						MTWF_DBG(pAd,
