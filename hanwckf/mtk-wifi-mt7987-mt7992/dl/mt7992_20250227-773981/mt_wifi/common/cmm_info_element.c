	...
	IN struct wifi_dev *wdev)
{
	...
	if (pDot11h == NULL || pDfsParam == NULL)
		return;
	pIE->ID = IE_EXT_CHANNEL_SWITCH_ANNOUNCEMENT;
	pIE->Length = 4;
	if (pDfsParam->CSARadarDetect == FALSE)
		pIE->ChannelSwitchMode = 0;	/* Tx is allowed */
	else
		pIE->ChannelSwitchMode = 1;	/* Tx is not allowed */
	...
	struct MAX_CHANNEL_SWITCH_TIME_IE *pIE)
{
	...
	pIE->ID = EID_EXTENSION;
	pIE->Length = 4;
	pIE->ID_EXTENTION = EID_EXT_MAX_CH_SWITCH_TIME;
	...
	UINT16 *buf_len)
{
	...
	*(ptr + 1) = 3; /* Len = 3 */
	if (pDfsParam->CSARadarDetect == FALSE)
		*(ptr + 2) = 0;	/* Switch_mode = 0, Enable TX */
	else
