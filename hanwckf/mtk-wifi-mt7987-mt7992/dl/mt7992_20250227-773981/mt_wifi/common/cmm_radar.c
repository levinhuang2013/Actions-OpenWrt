...
BOOLEAN RadarChannelCheckforChSet(
	IN PRTMP_ADAPTER	pAd,
	IN UCHAR			Ch)
{
	INT	i;
	...
	for (i = 0; i < pChCtrl->ChListNum; i++) {
		if (Ch == pChCtrl->ChList[i].Channel) {
			...
			if (!IS_ADJ_BW_ZERO_WAIT_TX80RX80(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState) &&
				...
				result = TRUE;
#else
			if (IS_CH_BETWEEN(Ch, 36, 48) && (wlan_config_get_vht_bw(wdev) == VHT_BW_160))
				result = TRUE;
#endif
			break;
		}
...
{
	if (pAd->CommonCfg.DfsParameter.CacCtrl == NO_NEED_CAC)
		return FALSE;
	...
	struct wifi_dev *wdev)
{
	...
	&& IS_CH_BETWEEN(wdev->channel, 100, 128)
	...
	if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitSupport == TRUE) {
		...
		pDot11h->InServiceMonitorCount = 0;
		if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState != DFS_NOT_ENABLE
		&& pAd->CommonCfg.DfsParameter.CacCtrl != NO_NEED_CAC
		&& (!IS_ADJ_BW_ZERO_WAIT_TX80RX160(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState))
		&& (!IS_ADJ_BW_ZERO_WAIT_CAC_DONE(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState))) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN,
				...
				/* Setting for no available channel */
				pDot11h->ChannelMode = CHAN_SILENCE_MODE;
		} else if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState == DFS_NOT_ENABLE
		&& pAd->CommonCfg.DfsParameter.CacCtrl != NO_NEED_CAC
		&& DfsRadarChannelCheck(pAd, wdev, vht_cent2, phy_bw)) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN,
				"\x1b[1;33m SW based ZW-DFS, Change into CHAN_SILENCE_MODE \x1b[m\n");
...
*
*/
NTSTATUS APOverlappingScan(PRTMP_ADAPTER pAd, PCmdQElmt CMDQelmt)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO, "Reset ACS StatMachine State: %lu.\n",
	pAutoChCtrl->AutoChSelCtrl.AutoChScanStatMachine.CurrState);
	...
	if (pAd->ApCfg.iwpriv_event_flag) {
		...
		/*for some modules without asynchronous mode, should release ChannelOpCharge here */
		pAd->ApCfg.set_ch_async_flag = FALSE;
		ReleaseChannelOpChargeForCurrentOwner(pAd, wdev);
	}
...
NTSTATUS Dot11HCntDownTimeoutAction(PRTMP_ADAPTER pAd, PCmdQElmt CMDQelmt)
{
	...
	if (pAd->ApCfg.iwpriv_event_flag) {
		pAd->ApCfg.iwpriv_event_flag = FALSE;
		pAd->ApCfg.set_ch_async_flag = FALSE;
		ReleaseChannelOpCharge(pAd, &pMbss->wdev, CH_OP_OWNER_SET_CHN);
	} else {
...
#define CLEAR 4
	...
	UCHAR macAddr[MAC_ADDR_LEN];
	RTMP_STRING *mac_value;
	RTMP_STRING *mac;
	INT i = 0;
	RTMP_STRING *value;
	INT action;

	if (strlen(arg) > 19) {/*Mac address acceptable format 01:02:03:04:05:06 length 17 */
		MTWF_PRINT("Invalid Input\n");
		return FALSE;
	}
	...
	value = rstrtok(arg, " ");
	mac = rstrtok(NULL, " ");
	action = (INT) os_str_tol(value, 0, 10);

	if (mac && (action <= ADD_MAC)) {
		for (i = 0, mac_value = rstrtok(mac, ":"); mac_value; mac_value = rstrtok(NULL, ":")) {
			if ((strlen(mac_value) != 2) || (!isxdigit(*mac_value)) || (!isxdigit(*(mac_value+1))))
				return FALSE;  /*Invalid */
			AtoH(mac_value, (UCHAR *)&macAddr[i++], 1);
		}
	}

	switch (action) {
	case DISPLAY:
		RoamCalib_display_sta(pAd);
		return TRUE;

	case ADD_MAC:
		RoamCalib_add_sta(pAd, macAddr);
		break;

	case REMOVE_MAC:
		RoamCalib_remove_sta(pAd, macAddr);
		break;

	case ENABLE:
		if ((INT) os_str_tol(mac, 0, 10) == 1)
			pAd->RoamCalibEnable = 1;
		else
			pAd->RoamCalibEnable = 0;
		break;

	case CLEAR:
		RoamCalib_clear_data(pAd);
		return TRUE;
	default:
		MTWF_PRINT("ERROR\n");
		break;
	...
	MTWF_PRINT("current roaming sta list::[Calibration Enable Status = %d]\n", pAd->RoamCalibEnable);
	for (i = 0; i < ROAM_STA_CNT; i++) {
		MTWF_PRINT("index %d: addr:%02x:%02x:%02x:%02x:%02x:%02x\n", i,
			PRINT_MAC(pAd->RoamingSta[i].StaAddr));
	}
	return TRUE;
}
