...
static const char *nl80211_dfs_state_txt(enum nl80211_dfs_state dfs_state)
{
	switch (dfs_state) {
...
static const char *nl80211_radar_event_txt(enum nl80211_radar_event event)
{
	switch (event) {
	case NL80211_RADAR_DETECTED:
		return "NL80211_RADAR_DETECTED";
	case NL80211_RADAR_CAC_FINISHED:
		return "NL80211_RADAR_CAC_FINISHED";
	case NL80211_RADAR_CAC_ABORTED:
		return "NL80211_RADAR_CAC_ABORTED";
	case NL80211_RADAR_NOP_FINISHED:
		return "NL80211_RADAR_NOP_FINISHED";
	case NL80211_RADAR_PRE_CAC_EXPIRED:
		return "NL80211_RADAR_PRE_CAC_EXPIRED";
	case NL80211_RADAR_CAC_STARTED:
		return "NL80211_RADAR_CAC_STARTED";
	default:
		return "NULL";
	}
}
...
const char *dfs_inb_ch_switch_stat_txt(ENUM_DFS_INB_CH_SWITCH_STAT_T ch_stat)
{
	switch (ch_stat) {
	case DFS_INB_CH_INIT:
		return "DFS_INB_CH_INIT";
	case DFS_OUTB_CH_CAC:
		return "DFS_OUTB_CH_CAC";
	case DFS_INB_CH_SWITCH_CH:
		return "DFS_INB_CH_SWITCH_CH";
	case DFS_INB_DFS_OUTB_CH_CAC:
		return "DFS_INB_DFS_OUTB_CH_CAC";
	case DFS_INB_DFS_OUTB_CH_CAC_DONE:
		return "DFS_INB_DFS_OUTB_CH_CAC_DONE";
	case DFS_INB_DFS_RADAR_OUTB_CAC_DONE:
		return "DFS_INB_DFS_RADAR_OUTB_CAC_DONE";
	case DFS_INB_DFS_RADAR_OUTB_CAC_DONE_QUICK:
		return "DFS_INB_DFS_RADAR_OUTB_CAC_DONE_QUICK";
	default:
		return "NULL";
	}
}

static const char *chan_mode_txt(int mode)
{
	switch (mode) {
	case CHAN_NORMAL_MODE:
		return "CHAN_NORMAL_MODE";
	case CHAN_SWITCHING_MODE:
		return "CHAN_SWITCHING_MODE";
	case CHAN_SILENCE_MODE:
		return "CHAN_SILENCE_MODE";
	case CHAN_NOP_MODE:
		return "CHAN_NOP_MODE";
	default:
		return "NULL";
	}
}
...
/**
 * @brief  Checks the status of the Channel Availability Check (CAC) by bandwidth.
 *
 * This function performs a CAC status check for a given channel and bandwidth.
 *
 * @param pAd         Pointer to the RTMP_ADAPTER structure.
 * @param CacChannel  The channel to check for CAC status.
 * @param BW          The bandwidth in which to check the CAC status (20, 40, 80, or 160 MHz).
 * @return            TRUE if the CAC aleady done, FALSE otherwise.
 */
BOOLEAN dfs_cac_check_by_bw(
	...
	UCHAR BW)
{
	...
	int ch_idx = 0;
	BOOLEAN status = TRUE;
	enum _CAC_OP op = CAC_DONE_CHECK;

	if (!pAd) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR, "Null pAd!!!\n");
		return FALSE;
	}
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "CacChannel=%u, BW=%u, status = %u\n", CacChannel, BW, status);
	return status;
	...
	UCHAR CacChannel)
{
	...
	/* calculate bw */
	if (pDfsParam->bDedicatedZeroWaitDefault  && CacChannel == pDfsParam->OutBandCh && pDfsParam->OutBandBw >= pDfsParam->band_bw) {
		if (op == CAC_DONE_UPDATE)
	...
	enum ch_flag_op op)
{
	...
	UCHAR ch_idx = 0;
	UCHAR bw_cap, bw = 0, vht_bw, ext_ch, channel = 0;
	CHANNEL_CTRL *pChCtrl = NULL;
	struct _DFS_PARAM *pDfsParam = &pAd->CommonCfg.DfsParameter;
...
)
{
	...
	if (pChCtrl == NULL) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR,
			"\x1b[1;33m[RDM] pChCtrl is NULL \x1b[m\n");
		return;
	}
	...
	if (ChIdx >= pChCtrl->ChListNum) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR, "\x1b[1;33m[RDM] Channel: %u is not found in channel list. \x1b[m\n", Channel);
		return;
	}
	...
	/* Send CAC done event if CAC requested by MAP on channel is already CAC done by PreCac*/
	if ((wdev != NULL) && (pAd->CommonCfg.DfsParameter.CACMemoEn && dfs_cac_op(pAd, wdev, CAC_DONE_CHECK, Channel))) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "\x1b[1;33m[RDM] Channel: %u has done cac \x1b[m\n", Channel);
		wapp_send_cac_stop(pAd, RtmpOsGetNetIfIndex(wdev->if_dev), Channel, TRUE);
		return;
	}
...
#if (DFS_ZEROWAIT_DEFAULT_FLOW == 1)
/*
 adjust ch\bw based on target ch and bw
*/
UCHAR zw_dfs_adjust(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	...
	)
{
	...
	if (RadarChannelCheckforChSet(pAd, final_ch)
		|| (pAd->CommonCfg.DfsParameter.CacCtrl == NEED_RESTART && !IS_MAP_TURNKEY_ENABLE(pAd))) {
		/*if it is DFS ch performing ZW DFS process*/
		if (pAd->CommonCfg.DfsParameter.CACMemoEn
			&& dfs_cac_op(pAd, wdev, CAC_DONE_CHECK, final_ch)) {

			if (pAd->CommonCfg.DfsParameter.bPreCacEn) {
				if (pAd->CommonCfg.DfsParameter.inband_ch_stat != DFS_INB_DFS_OUTB_CH_CAC) {
		...
		} else if ((pAd->CommonCfg.DfsParameter.BW80DedicatedZWSupport) && (IS_CH_BETWEEN(DfsPrimToCent(final_ch, BW_80), 100, 128)) && (dfs_cac_check_by_bw(pAd, final_ch, BW_80))) {
			/*if ch100BW80 cac done and directly switch without cac again.*/
			/*Adjust Bw*/
			DfsAdjustOpBwSetting(pAd, wdev, pAd->CommonCfg.DfsParameter.band_bw, BW_80);
			if (pAd->CommonCfg.DfsParameter.bPreCacEn) {
			...
			if (pAd->CommonCfg.DfsParameter.inband_ch_stat != DFS_INB_DFS_RADAR_OUTB_CAC_DONE &&
				...
				DfsDedicatedExamineSetNewCh(pAd, wdev, final_ch);
				DedicatedZeroWaitStop(pAd, TRUE);
			}

			/* CAC of new DFS ch X will be checked by dedicated RX */
			zero_wait_dfs_update_ch(pAd, wdev, cur_ch, &final_ch);
		}
	}  else {
		if (pAd->CommonCfg.DfsParameter.bPreCacEn) {
			if (pAd->CommonCfg.DfsParameter.inband_ch_stat != DFS_INB_DFS_OUTB_CH_CAC) {
...
)
{
	...
	struct _CHANNEL_CTRL *pChCtrl = NULL;
	...
	)
{
	...
	&& *new_bw == BW_160) {
		pDfsParam->band_orich = *ch;
		DfsAdjustOpBwSetting(pAd, wdev, pDfsParam->band_bw, BW_160);
	...
	&& *new_bw == BW_80) {
		pDfsParam->band_orich = *ch;
		DfsAdjustOpBwSetting(pAd, wdev, pDfsParam->band_bw, BW_80);
		pDfsParam->band_bw = BW_80;
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	pDfsParam->CSARadarDetect = FALSE;
	...
	IN RTMP_STRING * arg)
{
	...
	char *pos = arg;
	char *end = pos + MTK_NL80211_VENDOR_SUBCMD_VENDOR_SHOW_BUFFER_SIZE;
	int ret;
#ifdef RT_CFG80211_SUPPORT
	struct wiphy *wiphy;
	struct ieee80211_supported_band *ptr_band;
	struct ieee80211_channel *channel;
	enum nl80211_band band_id = 0;
	UINT32 chan_idx = 0, chan_id = 0, chan_num = 0, cent_ch = 0;
	int bw = 0, remain_time = 0;
	bool inband_in_cac = FALSE;
	bool dedicated_in_cac = FALSE;
#endif /* RT_CFG80211_SUPPORT */
	struct Ch_Op_Info *pChOpInfo = NULL;
	...
	pChCtrl = hc_get_channel_ctrl(pAd->hdev_ctrl);
	pChOpInfo = hc_get_ch_op_info(pAd->hdev_ctrl);
	...
	ret = snprintf(pos, end-pos,
			"[RDM]: DfsNopExpireSetChPolicy=%d, Cfg Channel=%d, Bandwidth=%d\n",
			pDfsParam->DfsNopExpireSetChPolicy, pChOpInfo->target_ch,
			pChOpInfo->target_bw);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;

	ret = snprintf(pos, end-pos, "[RDM]: DFS channel info, ch_stat=%s\n",
			dfs_inb_ch_switch_stat_txt(*ch_stat));
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;

	ret = snprintf(pos, end-pos, "[RDM]: ChannelMode=%d, bDfsWaitCfgChNop=%d\n",
			pDot11h->ChannelMode, pDfsParam->bDfsWaitCfgChNop);
	if (os_snprintf_error(end-pos, ret))
		return FALSE;
	pos += ret;

	ret = snprintf(pos, end-pos, "[RDM]: DfsChSelPrefer=%d, SWZeroWait=%d\n",
	...
	ret = snprintf(pos, end-pos, "CH: %d,\tBW: %d,\tCAC cnt: %d,\tCAC: %d\n",
			pDfsParam->band_ch,
			pDfsParam->band_bw,
			pDot11h->RDCount,
			pDot11h->cac_time);
	...
	if ((pDfsParam->bDedicatedZeroWaitSupport == TRUE &&
		...
		ret = snprintf(pos, end-pos, "dedicated RX:\n");
		if (os_snprintf_error(end-pos, ret))
			return FALSE;
		pos += ret;
		ret = snprintf(pos, end-pos, "CH: %d,\tBW: %d,\tCAC cnt: %d,\tCAC: %d\n",
	...
	if (pDot11h->ChannelMode == CHAN_SILENCE_MODE) {
		inband_in_cac = TRUE;
		cent_ch = DfsPrimToCent(pDfsParam->band_ch, pDfsParam->band_bw);
		bw = phy_bw_2_nl_bw(pDfsParam->band_bw);
		remain_time = (int)pDot11h->cac_time - (int)pDot11h->RDCount;
	} else if (pDfsParam->bDedicatedZeroWaitSupport == TRUE &&
			 pDfsParam->OutBandCh != 0 &&
			 (pDfsParam->inband_ch_stat == DFS_OUTB_CH_CAC ||
			  pDfsParam->inband_ch_stat == DFS_INB_DFS_OUTB_CH_CAC)) {
		dedicated_in_cac = TRUE;
		cent_ch = DfsPrimToCent(pDfsParam->OutBandCh, pDfsParam->OutBandBw);
		bw = phy_bw_2_nl_bw(pDfsParam->OutBandBw);
		remain_time = (int)pDfsParam->DedicatedOutBandCacTime - (int)pDfsParam->DedicatedOutBandCacCount;
	}

	if (remain_time < 0)
		remain_time = 0;

	wiphy = pAd->pCfg80211_CB->pCfg80211_Wdev->wiphy;
	band_id = CFG80211_Get_BandId(pAd);
	chan_num = pAd->pCfg80211_CB->Cfg80211_bands[band_id].n_channels;
	ptr_band = &pAd->pCfg80211_CB->Cfg80211_bands[band_id];

	for (chan_idx = 0; chan_idx < chan_num; chan_idx++) {
		channel = &ptr_band->channels[chan_idx];
		if (!(channel->flags & IEEE80211_CHAN_RADAR))
			continue;

		chan_id = ieee80211_frequency_to_channel(channel->center_freq);
		ret = snprintf(pos, end-pos, "Channel = %d, DFS_state = %s",
						chan_id,
						nl80211_dfs_state_txt(channel->dfs_state));
		if (os_snprintf_error(end-pos, ret))
			return FALSE;
		pos += ret;

		if (inband_in_cac &&
			CFG80211_IsSubChannel(wiphy, cent_ch, bw, chan_id)) {
			ret = snprintf(pos, end-pos,
						", CAC Remain Time = %2d / %2d [sec]\n",
						remain_time, pDot11h->cac_time);
		} else if (dedicated_in_cac &&
				   CFG80211_IsSubChannel(wiphy, cent_ch, bw, chan_id)) {
			ret = snprintf(pos, end-pos,
						", CAC Remain Time = %2d / %2d [sec] (Dedicated)\n",
						remain_time, pDfsParam->DedicatedOutBandCacTime);
		} else {
			ret = snprintf(pos, end-pos, "\n");
		}
		if (os_snprintf_error(end-pos, ret))
...
BOOLEAN DfsZwBypassCac(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR OriChannel, UCHAR NewChannel)
{
...
#if defined(BACKGROUND_SCAN_SUPPORT)
	UCHAR wdev_bw = 0;
#endif
	UCHAR ch_band = 0, i;
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	...
	if (pDfsParam->bDedicatedZeroWaitSupport == TRUE &&
		...
		if (wdev_bw == VHT_BW_160) {
			if ((vht_cent_ch_freq(OriChannel, wdev_bw, ch_band) ==
			     vht_cent_ch_freq(NewChannel, wdev_bw, ch_band)) &&
...
BOOLEAN DfsSetZeroWaitInitMbssCh(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR Ch, UCHAR Bw)
{
	INT	i;
...
VOID Set_Zwdfs_Proc(RTMP_ADAPTER *ad, struct wifi_dev *wdev)
{
	...
	if (WMODE_CAP_5G(wdev->PhyMode)) {
		...
		if (ad->CommonCfg.DfsParameter.bDedicatedZeroWaitDefault == 0
			...
			if ((RadarChBwCheck(ad, wdev->channel, ad->CommonCfg.DfsParameter.band_bw)
				|| DfsSetZeroWaitInitMbssCh(ad, wdev, wdev->channel, ad->CommonCfg.DfsParameter.band_bw))
...
#ifdef RT_CFG80211_SUPPORT
	DfsNotifyInbandNL80211RadarEvent(pAd, NL80211_RADAR_CAC_FINISHED);
#endif
	...
	case DFS_INB_DFS_OUTB_CH_CAC_DONE:
		...
		*ch_stat = DFS_INB_DFS_OUTB_CH_CAC_DONE;
		...
		if (pAd->CommonCfg.DfsParameter.CACMemoEn
			...
			&& pChOpInfo->target_bw == pDfsParam->band_bw
			&& pChOpInfo->target_ch == pDfsParam->band_ch) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
				"zw dfs complete, pre cac start.\n");
			BandIdx = hc_get_hw_band_idx(pAd);
			for (BssIdx = 0; BssIdx < pAd->ApCfg.BssidNum; BssIdx++) {
				wdev = &pAd->ApCfg.MBSSID[BssIdx].wdev;
				if (BandIdx == HcGetBandByWdev(wdev))
	...
	UCHAR CompareMode)
{
	...
	if ((pDot11h->ChannelMode == CHAN_SILENCE_MODE) && (CompareMode == CHAN_SILENCE_MODE)) {
		...
		mtRddControl(pAd, CAC_START, RddIdx, 0, 0);
#ifdef RT_CFG80211_SUPPORT
		DfsNotifyInbandNL80211RadarEvent(pAd, NL80211_RADAR_CAC_STARTED);
#endif
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (ch_idx = 0; ch_idx < pChCtrl->ChListNum; ch_idx++) {
		if (pChCtrl->ChList[ch_idx].NonOccupancy > 0) {
			...
			if (pChCtrl->ChList[ch_idx].NonOccupancy == 0) {
#ifdef RT_CFG80211_SUPPORT
				DfsNotifyNL80211NopFinished(pAd, pChCtrl->ChList[ch_idx].Channel, BW_20);
#endif
...
)
{
	...
	/* Set NOP for channel if radar is detected */
...
)
{
	...
	reg_cap_bw = get_channel_bw_cap(wdev, target_ch);
	if (target_bw > reg_cap_bw) {
		if (!(target_bw == BW_8080 && (reg_cap_bw == BW_80 || reg_cap_bw == BW_160))) {
			/*bw capability check of target_ch*/
			target_bw = reg_cap_bw;
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN,
				"target_bw is reconfigured to be %d\n", reg_cap_bw);
		}
	}
	...
	IN PRTMP_ADAPTER pAd, UCHAR ucRddIdx)
{
...
#ifdef RT_CFG80211_SUPPORT
		DfsNotifyNL80211RadarDetect(pAd, pDfsParam->PrimCh, pDfsParam->band_bw, false);
#endif
	}
	...
	|| pDfsParam->bOcacEnable == 2) {
		...
		if (pDfsParam->RadarDetected[RDD_DEDICATED_IDX] == TRUE) {
			...
			pDfsParam->RadarDetected[RDD_DEDICATED_IDX] = FALSE;
#ifdef RT_CFG80211_SUPPORT
			DfsNotifyNL80211RadarDetect(pAd, pDfsParam->OutBandCh, pDfsParam->OutBandBw, true);
#endif
#ifdef DFS_CAC_R2
...
/*Link down only after sending the radar detect notification*/
		if (IS_MAP_ENABLE(pAd)) {
			...
			for (i = 0; i < MAX_APCLI_NUM; i++) {
				...
				if(wdev->quick_ch_change != QUICK_CH_SWICH_DISABLE)
					wapp_send_radar_detect_notif(pAd, wdev, wdev->map_radar_channel, wdev->map_radar_bw, 0);
#if defined(EM_PLUS_SUPPORT) && defined(CONFIG_RCSA_SUPPORT)
				if (pDfsParam->bRCSAEn != TRUE)
#endif
				{
					if (sta_wdev->channel == wdev->channel)
						pAd->StaCfg[i].ApcliInfStat.Enable = FALSE;
		...
		if (dot11h_param->ChannelMode == CHAN_NORMAL_MODE) {
			...
			if (ret == NDIS_STATUS_SUCCESS) {
				RTMP_MLME_HANDLER(pAd);
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
					"<%s>:The WLAN channel is being dynamically switched from channel %d to channel %d as result of a DFS scan\n",
					get_band_str(wlan_config_get_ch_band(wdev)), wdev->channel, NextCh);
#ifdef TR181_SUPPORT
					...
					if (pAd->CommonCfg.DfsParameter.bPreCacEn)
						...
						*ch_stat = DFS_INB_CH_INIT;
						...
						if (!pDfsParam->bDfsDedicatedRxPreselectCh &&
							pDfsParam->bDedicatedZeroWaitSupport &&
...
#if defined(EM_PLUS_SUPPORT) && defined(CONFIG_RCSA_SUPPORT)
		if (!((pAd->CommonCfg.DfsParameter.bRCSAEn) && (DfsChSelPrefer == RadarDetectSelectNonDFS)))
...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "[RDM]: OutBandCh:%d, band_ch:%d\n",
		pDfsParam->OutBandCh,
		pDfsParam->band_ch);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
...
#ifdef RT_CFG80211_SUPPORT
		DfsNotifyOutbandNL80211RadarEvent(pAd, NL80211_RADAR_CAC_STARTED);
#endif
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN,
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/*if no new out_band ch for pre-cac, keep use the current*/
	if ((pDfsParam->OutBandCh != 0) && (bw_ch == 0)
		&& (pAd->CommonCfg.DfsParameter.bPreCacEn)
		&& (pAd->CommonCfg.DfsParameter.inband_ch_stat == DFS_INB_DFS_OUTB_CH_CAC)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
			"no new out_band ch for pre-cac, keep using this ch:%d\n",
			pDfsParam->OutBandCh);
		return;
	}
...
#ifdef RT_CFG80211_SUPPORT
		DfsNotifyOutbandNL80211RadarEvent(pAd, NL80211_RADAR_CAC_FINISHED);
#endif
#ifdef DFS_CAC_R2
		if (IS_MAP_ENABLE(pAd) || IS_MAP_TURNKEY_ENABLE(pAd)) {
		...
		if (pDfsParam->SetOutBandChStat == OUTB_SET_CH_DEFAULT) {
			if (*ch_stat == DFS_INB_DFS_OUTB_CH_CAC_DONE &&
				...
				*ch_stat = DFS_INB_DFS_OUTB_CH_CAC;
				...
				/* For DFS Pre-CAC support
					Check if channel is already CAC done,
					then no need to set it to Outband.
				*/
				BandIdx = hc_get_hw_band_idx(pAd);
				pChCtrl = hc_get_channel_ctrl(pAd->hdev_ctrl);
				...
				if (ChIdx >= pChCtrl->ChListNum) {
					MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR,
						"\x1b[1;33m[RDM] Channel: %u is not found in channel list. \x1b[m\n", Channel);
					return;
				}

				/* channel is already CAC done by PreCac*/
				if ((pAd->CommonCfg.DfsParameter.CACMemoEn && pAd->CommonCfg.DfsParameter.bPreCacEn &&
					dfs_cac_op(pAd, wdev, CAC_DONE_CHECK, Channel))) {
					MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR,
						"\x1b[1;33m xflu:: [RDM] Channel: %u is already done cac \x1b[m\n", Channel);
					return;
				}
	...
	IN PRTMP_ADAPTER pAd, UCHAR Channel, UCHAR Bw, BOOLEAN doCAC, UCHAR ucRddIdx)
{
	...
	if (wdev && !IS_APCLI_DISABLE_MLO(wdev)
			&& INFRA_ON(&pAd->StaCfg[MAIN_MSTA_ID])) {
		if (Bw > wlan_operate_get_bw(wdev) &&
				((Bw > BW_80) && (vht_cent_ch_freq(wdev->channel, VHT_BW_160, CMD_CH_BAND_5G) ==
					vht_cent_ch_freq(Channel, VHT_BW_160, CMD_CH_BAND_5G)))) {
			Channel = wdev->channel;
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
					"OUT band CAC end on ch %d, update BW\n", Channel);
		} else {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
...
#if defined(DFS_CAC_R2) && !defined(EM_PLUS_SUPPORT)
		if (IS_MAP_TURNKEY_ENABLE(pAd)) {
		...
		and no radar is detected , no need for this step at all .*/
			...
			if (dot11h_param->ChannelMode != CHAN_NOP_MODE) {
				...
				*ch_stat = DFS_OUTB_CH_CAC;
				...
				for (BssIdx = 0; BssIdx < pAd->ApCfg.BssidNum; BssIdx++) {
					wdev = &pAd->ApCfg.MBSSID[BssIdx].wdev;
					if (wdev && (BandIdx == HcGetBandByWdev(wdev)))
						break;
				}
	...
	IN UCHAR band_idx)
{
	...
	if (Channel != 0) {
		if (pDfsParam->BW80DedicatedZWSupport && ((Bw == BW_160) && IS_CH_BETWEEN(Channel, 36, 64)))
			Channel = 52;

		pDfsParam->OutBandCh = Channel;
		if (pDfsParam->BW80DedicatedZWSupport && pDfsParam->OutBandBw == BW_160)
			Bw = BW_80;
		pDfsParam->OutBandBw = Bw;
	} else {
...
VOID update_cac_ctrl_status(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR old_ch, UCHAR old_bw, UCHAR new_ch, UCHAR new_bw)
{
	...
	else if (dfs_range_change == DFS_RANGE_NULL)
		pAd->CommonCfg.DfsParameter.CacCtrl = NO_NEED_CAC;
	else
		pAd->CommonCfg.DfsParameter.CacCtrl = KEEP_CAC_STATE;
}
...
#ifdef RT_CFG80211_SUPPORT
struct DfsVendorEventInfo {
	enum nl80211_radar_event event;
	UCHAR chan;
	UCHAR bw;
	bool update_dfs_state;
	enum nl80211_dfs_state dfs_state;
};

NTSTATUS DfsSendVendorEvent(
	PRTMP_ADAPTER pAd, PCmdQElmt CMDQelmt)
{
	struct wiphy *wiphy;
	struct DfsVendorEventInfo EvtInfo;
	...
	NdisMoveMemory(&EvtInfo, CMDQelmt->buffer, sizeof(struct DfsVendorEventInfo));
	event_name = nl80211_radar_event_txt(EvtInfo.event);

	if (!pAd->pCfg80211_CB->pCfg80211_Wdev) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
				"Not notify upstream for !pAd->pCfg80211_CB->pCfg80211_Wdev");
		return -1;
	}
	wiphy = pAd->pCfg80211_CB->pCfg80211_Wdev->wiphy;

#if defined(CFG_CFG80211_VERSION) && (KERNEL_VERSION(6, 1, 0) <= CFG_CFG80211_VERSION)
	chandef = wdev_chandef(pAd->pCfg80211_CB->pCfg80211_Wdev, 0);
#else
	chandef = &pAd->pCfg80211_CB->pCfg80211_Wdev->chandef;
#endif
	if (!chandef) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "chandef is NULL\n");
		return -1;
	}

	p_chan = (struct ieee80211_channel *)CFG80211_FindChan(pAd, EvtInfo.chan);
	if (!p_chan) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "p_chan NULL!\n");
		return -1;
	}

	band_idx = CFG80211_Get_BandId(pAd);
	cent_ch = DfsPrimToCent(EvtInfo.chan, EvtInfo.bw);
	chandef->chan = p_chan;
	chandef->center_freq1 = ieee80211_channel_to_frequency(cent_ch, band_idx);
	chandef->width = phy_bw_2_nl_bw(EvtInfo.bw);

	if (!cfg80211_chandef_valid(chandef)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
				"cfg80211_chandef_valid failed!\n");
		return -1;
	}

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
			"Notify upstream, %s freq=%d, cf1=%d, cf2=%d, width=%d, freq_offset=%d.\n",
			event_name, chandef->chan->center_freq, chandef->center_freq1,
			chandef->center_freq2, chandef->width, chandef->chan->freq_offset);

	if (EvtInfo.update_dfs_state)
		CFG80211_SetDfsState(wiphy, chandef, EvtInfo.dfs_state);
	nl80211_send_event_radar_notify(wiphy, chandef, EvtInfo.event);

	return 0;
}

VOID DfsNotifyInbandNL80211RadarEvent(PRTMP_ADAPTER pAd, enum nl80211_radar_event event)
{
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	struct DfsVendorEventInfo vendor_event_info;

	if (event != NL80211_RADAR_CAC_STARTED &&
		event != NL80211_RADAR_CAC_ABORTED &&
		event != NL80211_RADAR_CAC_FINISHED) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
				"Not notify upstream for (event %s) is not supported",
				nl80211_radar_event_txt(event));
		return;
	}

	if (!pAd->CommonCfg.bIEEE80211H) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
				"Not notify upstream for (!pAd->CommonCfg.bIEEE80211H)");
		return;
	}

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
			"chan_mode=%s, ch_stat=%s",
			chan_mode_txt(pAd->Dot11_H.ChannelMode),
			dfs_inb_ch_switch_stat_txt(pDfsParam->inband_ch_stat));

	memset(&vendor_event_info, 0, sizeof(vendor_event_info));
	vendor_event_info.event = event;
	vendor_event_info.chan = pDfsParam->PrimCh;
	vendor_event_info.bw = pDfsParam->band_bw;

	/* Reference: cfg80211_cac_event;
	 * For the cfg80211 APIs will start queue_works, we mapping the nl80211_radar_event to
	 * mtk_nl80211_vendor_events, only update the dfs status and send mtk_vendor_events.
	 */
	switch (event) {
	case NL80211_RADAR_CAC_STARTED:
	case NL80211_RADAR_CAC_ABORTED:
		break;
	case NL80211_RADAR_CAC_FINISHED:
		vendor_event_info.update_dfs_state = TRUE;
		vendor_event_info.dfs_state = NL80211_DFS_AVAILABLE;
		break;
	default:
		return;
	}

	RTEnqueueInternalCmd(pAd, CMDTHREAD_DFS_SEND_VENDOR_EVENT,
						&vendor_event_info, sizeof(vendor_event_info));
}

VOID DfsNotifyOutbandNL80211RadarEvent(PRTMP_ADAPTER pAd, enum nl80211_radar_event event)
{
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	struct DfsVendorEventInfo vendor_event_info;
	...
	memset(&vendor_event_info, 0, sizeof(vendor_event_info));
	vendor_event_info.event = event;
	vendor_event_info.chan = pDfsParam->OutBandCh;
	vendor_event_info.bw = pDfsParam->OutBandBw;

	/* Reference: __cfg80211_background_cac_event;
	 * For the cfg80211 APIs will start queue_works, we mapping the nl80211_radar_event to
	 * mtk_nl80211_vendor_events, only update the dfs status and send mtk_vendor_events.
	 */
	switch (event) {
...
VOID DfsNotifyNL80211RadarDetect(PRTMP_ADAPTER pAd, UCHAR chan, UCHAR bw, bool is_background)
{
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	struct DfsVendorEventInfo vendor_event_info;
	enum nl80211_radar_event event = NL80211_RADAR_DETECTED;
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
			"chan_mode=%s, ch_stat=%s, is_background=%d",
			chan_mode_txt(pAd->Dot11_H.ChannelMode),
			dfs_inb_ch_switch_stat_txt(pDfsParam->inband_ch_stat),
			is_background);
	...
	memset(&vendor_event_info, 0, sizeof(vendor_event_info));
	vendor_event_info.event = event;
	vendor_event_info.chan = chan;
	vendor_event_info.bw = bw;
	/* Referece: __cfg80211_radar_event, cfg80211_radar_event,cfg80211_background_radar_event
	 * For the cfg80211 APIs will start queue_works, we mapping the nl80211_radar_event to
	 * mtk_nl80211_vendor_events, only update the dfs status and send mtk_vendor_events.
	 */
	vendor_event_info.update_dfs_state = TRUE;
	vendor_event_info.dfs_state = NL80211_DFS_UNAVAILABLE;

	RTEnqueueInternalCmd(pAd, CMDTHREAD_DFS_SEND_VENDOR_EVENT,
						&vendor_event_info, sizeof(vendor_event_info));
}

VOID DfsNotifyNL80211NopFinished(PRTMP_ADAPTER pAd, UCHAR chan, UCHAR bw)
{
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	struct DfsVendorEventInfo vendor_event_info;
	enum nl80211_radar_event event = NL80211_RADAR_NOP_FINISHED;
	...
	vendor_event_info.chan = chan;
	vendor_event_info.bw = bw;
	/* Reference: cfg80211_dfs_channels_update_work
	 * For the cfg80211 APIs will start queue_works, we mapping the nl80211_radar_event to
	 * mtk_nl80211_vendor_events, only update the dfs status and send mtk_vendor_events.
	 */
	vendor_event_info.update_dfs_state = TRUE;
	vendor_event_info.dfs_state = NL80211_DFS_USABLE;
