	...
	IN VOID * ie_list)
{
	...
	if (rnr_ie->Len >= 4) {
		...
		do {
			...
			tmp_ml_param.channel = nbr_ap_info->ch_num;
		...
		*/
				...
				tmp_ml_param.link_disable_ind = tmp_val.dis_link_ind;
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		*/
		...
		case IE_WLAN_EXTENSION:
			...
			/*parse t2lm ie*/
			if (HAS_EHT_ML_T2LM_EXIST(ie_list->cmm_ies.ie_exists)) {
				if (parse_tid_to_link_map_ie((UINT8 *)pEid, &ie_list->t2lm_info) < 0) {
					if (pPeerWscIe)
						os_free_mem(pPeerWscIe);
					return FALSE;
				}
	...
	IN UCHAR channel)
{
	...
	for (i = 0; (i < pChCtrl->FreqListNum) && (pChCtrl->FreqListNum <= MAX_NUM_OF_CHANNELS); i++) {
		if (channel == frequency_to_channel(pChCtrl->FreqList[i]))
			return TRUE;
	}
