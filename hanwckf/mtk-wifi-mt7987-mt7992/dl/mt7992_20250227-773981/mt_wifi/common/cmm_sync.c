...
static VOID channel_related_info_backup(
	...
	struct _CHANNEL_CTRL *origChCtrl,
	struct _CHANNEL_CTRL *BackUpChCtrl)
{
	UCHAR ChIdx;

	if (origChCtrl->ChListNum > MAX_NUM_OF_CHANNELS) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
			"ChListNum = %d, out of [0-%d] range.\n",
			origChCtrl->ChListNum, MAX_NUM_OF_CHANNELS);
		return;
	}

	if (origChCtrl->ChListNum == 0) {
	...
	NdisAcquireSpinLock(&origChCtrl->ChPrioOpLock);

	os_move_mem(BackUpChCtrl, origChCtrl, sizeof(struct _CHANNEL_CTRL));

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"=================================================\n");
	for (ChIdx = 0; ChIdx < BackUpChCtrl->ChListNum; ChIdx++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"channel = %d, priority = %d, cac done timestamp = %d.\n",
			BackUpChCtrl->ChList[ChIdx].Channel,
			BackUpChCtrl->ChList[ChIdx].Priority,
			BackUpChCtrl->ChList[ChIdx].cac_done_timestamp);
	}
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"owner is %d\n", BackUpChCtrl->CurChPrioOpOwner);

	/*unlock channel priority operation lock*/
	NdisReleaseSpinLock(&origChCtrl->ChPrioOpLock);
...
static VOID channel_related_info_restore(
	struct _RTMP_ADAPTER *pAd,
	struct _CHANNEL_CTRL *CurrentChCtrl,
	struct _CHANNEL_CTRL *BackUpChCtrl)
{
	UCHAR ChIdx, BackUpChCtrlChIdx, DFSBackupChIdx;
	NOP_LIST *pNopList = (NOP_LIST *)pAd->NopListBk;

	if (CurrentChCtrl->ChListNum > MAX_NUM_OF_CHANNELS
		|| CurrentChCtrl->ChListNum == 0) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
			"ChListNum = %d, out of (0-%d] range.\n",
			CurrentChCtrl->ChListNum, MAX_NUM_OF_CHANNELS);
		return;
	}


	for (ChIdx = 0; ChIdx < CurrentChCtrl->ChListNum; ChIdx++) {
		for (BackUpChCtrlChIdx = 0;
			BackUpChCtrlChIdx < BackUpChCtrl->ChListNum; BackUpChCtrlChIdx++) {
			if (CurrentChCtrl->ChList[ChIdx].Channel
				== BackUpChCtrl->ChList[BackUpChCtrlChIdx].Channel) {
				/*restore Pre CAC info*/
				if (BackUpChCtrl->ChList[BackUpChCtrlChIdx].Flags & CHANNEL_CAC_DONE) {
					CurrentChCtrl->ChList[ChIdx].Flags |= CHANNEL_CAC_DONE;
					CurrentChCtrl->ChList[ChIdx].cac_done_timestamp
						= BackUpChCtrl->ChList[BackUpChCtrlChIdx].cac_done_timestamp;
				}
			}
		}
	}
	...
	if (pAd->NopListBk) {
		...
		for (DFSBackupChIdx = 0; DFSBackupChIdx < pNopList->ChListNum; DFSBackupChIdx++) {
			for (ChIdx = 0; ChIdx < CurrentChCtrl->ChListNum; ChIdx++) {
				if (CurrentChCtrl->ChList[ChIdx].Channel == pNopList->DfsChList[DFSBackupChIdx].Channel) {
					CurrentChCtrl->ChList[ChIdx].NonOccupancy = pNopList->DfsChList[DFSBackupChIdx].NonOccupancy;
					CurrentChCtrl->ChList[ChIdx].NOPClrCnt = pNopList->DfsChList[DFSBackupChIdx].NOPClrCnt;
					CurrentChCtrl->ChList[ChIdx].NOPSetByBw = pNopList->DfsChList[DFSBackupChIdx].NOPSetByBw;
					CurrentChCtrl->ChList[ChIdx].NOPSaveForClear = pNopList->DfsChList[DFSBackupChIdx].NOPSaveForClear;
					CurrentChCtrl->ChList[ChIdx].SupportBwBitMap = pNopList->DfsChList[DFSBackupChIdx].SupportBwBitMap;
					CurrentChCtrl->ChList[ChIdx].IsOutBandCacDone = pNopList->DfsChList[DFSBackupChIdx].IsOutBandCacDone;
				}
			}
		}
	...
	CurrentChCtrl->MaxChPrio = BackUpChCtrl->MaxChPrio;
	CurrentChCtrl->CurChPrioOpOwner = BackUpChCtrl->CurChPrioOpOwner;

	if (CurrentChCtrl->ChListNum != BackUpChCtrl->ChListNum) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"ChList has been changed, no need to restore, update from cfg.\n");
		if (!LoadCfgChprio(CurrentChCtrl))
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
	...
	/*get lock,to avoid being interrupted by other processes*/
	NdisAcquireSpinLock(&CurrentChCtrl->ChPrioOpLock);
	/* Restore channel related info */
	if (CurrentChCtrl->NeedResetToCfgChPrio) {
		/*unlock channel priority operation lock*/
		NdisReleaseSpinLock(&CurrentChCtrl->ChPrioOpLock);
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
		"-----------------------------\n");
	for (ChIdx = 0; ChIdx < CurrentChCtrl->ChListNum; ChIdx++) {
		if (CurrentChCtrl->ChList[ChIdx].Channel != BackUpChCtrl->ChList[ChIdx].Channel) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
		...
		CurrentChCtrl->ChList[ChIdx].Priority = BackUpChCtrl->ChList[ChIdx].Priority;
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
				"channel %d, priority %d\n", CurrentChCtrl->ChList[ChIdx].Channel, CurrentChCtrl->ChList[ChIdx].Priority);
	}
...
VOID BuildChannelList(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	struct _CHANNEL_CTRL *pChCtrlBackUp = NULL;
	...
	os_alloc_mem(NULL, (UCHAR **)&pChCtrlBackUp, sizeof(struct _CHANNEL_CTRL));
	if (!pChCtrlBackUp) {
		MTWF_DBG(NULL, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_WARN,
			"Fail to alloc memory for pChCtrlBackUp\n");
		return;
	}
	os_zero_mem(pChCtrlBackUp, sizeof(struct _CHANNEL_CTRL));

	/* Backup channel related information */
	channel_related_info_backup(pAd, pChCtrl, pChCtrlBackUp);
	...
	/*restore channel related info*/
	channel_related_info_restore(pAd, pChCtrl, pChCtrlBackUp);
	os_free_mem(pChCtrlBackUp);
...
BOOLEAN AdjustBwToSyncAp(RTMP_ADAPTER *pAd, BCN_IE_LIST *ie_list, struct wifi_dev *wdev)
{
	...
	if (WMODE_CAP_AC(wdev->PhyMode) && (pStaCfg->StaActive.SupportedPhyInfo.bVhtEnable == TRUE)) {
		if (HAS_VHT_OP_EXIST(cmm_ies->ie_exists) && HAS_HT_OP_EXIST(cmm_ies->ie_exists)) {
			...
			channel_bw_cap = get_channel_bw_cap(wdev, wdev->channel);
			current_operating_bw = min(channel_bw_cap, current_operating_bw);
				...
				* to 0(20MHZ) and HT info IE set to 1
				...
				if ((cfg_ht_bw >= cmm_ies->ht_op.AddHtInfo.RecomWidth) ||
					...
					if (cmm_ies->ht_op.AddHtInfo.RecomWidth > cmm_ies->ht_cap.HtCapInfo.ChannelWidth) {
						new_ht_bw = cmm_ies->ht_cap.HtCapInfo.ChannelWidth;
						new_ext_ch = EXTCHA_NONE;
					} else {
						new_ht_bw = pStaCfg->MlmeAux.AddHtInfo.AddHtInfo.RecomWidth;
						new_ext_ch = pStaCfg->MlmeAux.AddHtInfo.AddHtInfo.ExtChanOffset;
					}

					if (new_ht_bw != op_ht_bw) {
						wlan_operate_set_ht_bw(wdev, new_ht_bw, new_ext_ch);
						pEntry->HTPhyMode.field.BW = new_ht_bw;
					}
				}
			...
			wlan_operate_set_vht_bw(&pStaCfg->wdev, rf_bw_2_vht_bw(current_operating_bw));
