	...
	IN BOOLEAN isReassoc)
{
	...
	if (pEntry->FlgIs11bSta && (wlan_config_get_ch_band(wdev) == CMD_CH_BAND_24G) && wdev->IsNoBMode
		&& !HAS_HT_CAPS_EXIST(ie_list->cmm_ies.ie_exists) && !HAS_HE_CAPS_EXIST(ie_list->cmm_ies.ie_exists)
		&& !HAS_EHT_CAPS_EXIST(ie_list->cmm_ies.ie_exists)) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_WARN,
			"The STA only support b Mode, but the AP does not support b mode. MLME_ASSOC_REJ_DATA_RATE\n");
		return MLME_ASSOC_REJ_DATA_RATE;
	}
#ifdef DOT11_N_SUPPORT
	...
	IN BOOLEAN isReassoc)
{
...
#if defined(CONFIG_MAP_SUPPORT) && defined(EM_PLUS_SUPPORT) && defined(MAP_R6)
	pEntry = MacTableLookup(pAd, ie_list->Addr2);
	if (!pEntry) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
				 "NoAuth MAC - "MACSTR"\n",
	...
	if (is_mlo_setup_link) {
		if (MAC_ADDR_EQUAL(pEntry->current_assoc_mac, ZERO_MAC_ADDR)) {
			NdisGetSystemUpTime(&pEntry->current_assoc_time);
			memcpy(pEntry->current_assoc_mac, ie_list->Addr2, MAC_ADDR_LEN);
		} else if (MAC_ADDR_EQUAL(pEntry->current_assoc_mac, ie_list->Addr2)) {
			NdisGetSystemUpTime(&TNow);
			if (TNow < (pEntry->current_assoc_time + (OS_HZ)))
				goto LabelOK;
			else
				NdisGetSystemUpTime(&pEntry->current_assoc_time);
		} else {
		...
		memcpy(pEntry->current_assoc_mac, ZERO_MAC_ADDR, MAC_ADDR_LEN);
		pEntry->current_assoc_time = 0;
	}
#endif /* CONFIG_MAP_SUPPORT EM_PLUS_SUPPORT MAP_R6*/


#ifdef ROAM_CALIB_SUPPORT
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, ie_list->Addr2, NULL, "Assoc Request Received", __func__);
	...
	if (pEntry->wdev) {
		if (pEntry->wdev != wdev) {
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
					"Auth & assoc in different bssid, not allowed,auth: "MACSTR" - assoc: "MACSTR"\n",
					MAC2STR(pEntry->wdev->if_addr), MAC2STR(ie_list->Addr1));
			goto LabelOK;
		}
	} else {
		goto LabelOK;
...
#if defined(RT_CFG80211_SUPPORT)
	{
		...
		PUINT8 AKMMap = WPA_ExtractSuiteFromRSNIE(ie_list->RSN_IE,
				ie_list->RSNIE_Len, AKM_SUITE, &akm_count);
		if (AKMMap != NULL) {
			hex_dump_with_lvl("peer AssocReq AKM", AKMMap, LEN_OUI_SUITE, DBG_LVL_INFO);
			NdisCopyMemory(&pEntry->SecConfig.AKMMap, AKMMap, LEN_OUI_SUITE);
		}
	}
#endif
...
#if defined(RT_CFG80211_SUPPORT) && defined(HOSTAPD_PMKID_IN_DRIVER_SUPPORT)
	{
		...
		if ((pPMKID != NULL) && (!IS_AKM_OWE(pEntry->SecConfig.AKMMap))) {
			...
			if (NdisCmpMemory(pEntry->PmkidByHostapd, zeroPmkid, LEN_PMKID) &&
				...
				if (!IS_AKM_WPA2PSK(pEntry->SecConfig.AKMMap)
#ifdef MTK_MLO_MAP_SUPPORT
					&& (!IS_MAP_ENABLE(pAd))
#endif /* MTK_MLO_MAP_SUPPORT */
					&& (!is_owe)
...
SendAssocResponse:
	...
	if (StatusCode != MLME_SUCCESS)
		record_connect_info(pAd, ie_list->Addr2, StatusCode, TRUE);
...
#ifdef CFG_RSNO_SUPPORT
		/* MRSNO: Need to add RSNEX to assoc response if RSNEXO exists. */
		if (rsnxe_ie_len == 0)
			rsnxe_ie_len = build_rsnxe_ie(wdev, &wdev->SecConfig_ext, rsnxe_ie);
#endif /* CFG_RSNO_SUPPORT */
	...
	if (check_rsnxe_install) {
		...
		if (wpa3_test_ctrl == 9) {
			len = build_rsnxe_ie(
				wdev, &wdev->SecConfig, (UCHAR *)pOutBuffer + FrameLen);
#ifdef CFG_RSNO_SUPPORT
			/* MRSNO: Need to add RSNEX to assoc response if RSNEXO exists. */
			if (len == 0)
				len = build_rsnxe_ie(
					wdev, &wdev->SecConfig_ext, (UCHAR *)pOutBuffer + FrameLen);
#endif /* CFG_RSNO_SUPPORT */
			if (len != 0) {
				UCHAR cap;
...
#ifdef ROAM_CALIB_SUPPORT
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, ie_list->Addr2, NULL, "RE-ASSOC Resp to STA", __func__);
#endif /* ROAM_CALIB_SUPPORT */
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
...
#ifdef ROAM_CALIB_SUPPORT
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, ie_list->Addr2, NULL, "ASSOC Resp to STA", __func__);
#endif /* ROAM_CALIB_SUPPORT */
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
...
static VOID ap_peer_disassoc_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	record_connect_info(pAd, Addr2, Reason, FALSE);
...
VOID APMlmeKickOutSta(RTMP_ADAPTER *pAd, UCHAR *pStaAddr, UINT16 Wcid, USHORT Reason)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Wcid)) {
		...
		record_connect_info(pAd, pStaAddr, Reason, FALSE);
