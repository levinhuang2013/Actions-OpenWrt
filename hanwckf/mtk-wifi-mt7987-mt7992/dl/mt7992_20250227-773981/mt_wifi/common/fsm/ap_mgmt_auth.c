	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	if (IS_VALID_ENTRY(pEntry) && IS_ENTRY_CLIENT(pEntry) &&
		...
		record_connect_info(pAd, pInfo->addr, pInfo->reason, FALSE);
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid)) {
		...
		if (pEntry && !IS_ENTRY_CLIENT(pEntry))
			...
			if (IS_MAP_ENABLE(pAd) && IS_MAP_R2_ENABLE(pAd)) {
				...
				if (mld_entry) {
					for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
						...
						if (!link_entry) {
							MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_INFO,
							"link_entry is null continue");
							continue;
						}
						link_entry->DisconnectReason = Reason;
					}
				} else {
			...
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, Addr2, NULL, "Deauth Received from STA", __func__);
...
#ifdef ROAM_CALIB_SUPPORT
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, pAuth_info->addr2, wdev->bssid, "Auth Response Send", __func__);
#endif /* ROAM_CALIB_SUPPORT */
...
VOID ap_peer_auth_req_at_idle_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#ifdef ROAM_CALIB_SUPPORT
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, pAuth_info->addr2, wdev->bssid, "Auth Req Received", __func__);
#endif /* ROAM_CALIB_SUPPORT */
	...
	MacTableAuthReqClearMloEntry(pAd, wdev, pAuth_info->addr2, pAuth_info->cmm_ies.ml_ie,
...
#endif
#endif
			record_connect_info(pAd, pAuth_info->addr2, REASON_DECLINED, TRUE);
...
#ifdef RADIUS_MAC_AUTH_SUPPORT
	freq = channel_to_frequency(pAd->BandSel, pAd->LatchRfRegs.Channel);
#endif
...
#define	RADIUS_ACL_ACCEPT_TIMEOUT  3
	...
	if ((pAuth_info->auth_alg == AUTH_MODE_SAE) &&
		(IS_AKM_WPA3PSK(pMbss->wdev.SecConfig.AKMMap))) {
		...
		if ((pAuth_info->auth_alg == AUTH_MODE_OPEN) &&
			...
			if (pEntry) {
				...
				if (wdev->radius_mac_auth_enable)
					CFG80211OS_RxMgmt(wdev->if_dev, freq, Elem->Msg, Elem->MsgLen, 0);
#endif
		} else {/* MAC table full*/
#ifdef WIFI_DIAG
				diag_conn_error(pAd, apidx, pAuth_info->addr2, DIAG_CONN_STA_LIM, 0);
				...
				ApSendConnFailMsg(pAd,
					...
					REASON_DISASSPC_AP_UNABLE);
#endif
				record_connect_info(pAd, pAuth_info->addr2, REASON_DISASSPC_AP_UNABLE, TRUE);
			}
			...
			if (pEntry) {
				...
				ApSendConnFailMsg(pAd,
					...
					REASON_UNSPECIFY);
#endif
				record_connect_info(pAd, pAuth_info->addr2, REASON_UNSPECIFY, TRUE);
#ifdef CUSTOMISED_MGMT_TRANS_SUPPORT
					FailConnReason = MTK_NL80211_VENDOR_CONN_FAIL_MLME_NO_RESOURCE;
#endif
...
#endif
			record_connect_info(pAd, pAuth_info->addr2, REASON_AKMP_NOT_VALID, TRUE);
			/* If this STA exists, delete it. */
			if (pEntry)
				MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
...
#ifdef ROAM_CALIB_SUPPORT
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, auth_info->addr2, wdev->bssid, "Auth Req Received", __func__);
#endif /* ROAM_CALIB_SUPPORT */
