	...
	struct _IE_lists *ie_list)
{
	...
	pEntry = MacTableLookup(mac_ad, ie_list->Addr2);
	if (!pEntry) {
		MTWF_DBG(mac_ad, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
				"NoAuth MAC - "MACSTR"\n",
...
/*	UINT8 wapp_cnnct_stage = WAPP_ASSOC; */
	...
	if (pEntry->ie_list && pEntry->ie_list->ecdh_ie.length) {
		...
		if (pAd->RoamCalibEnable)
			RoamCalibData(pAd, ie_list->Addr2, wdev->bssid, "Send Assoc to Hostapd", __func__);
	...
	if (IS_MAP_ENABLE(pAd)) {
		...
		if (DataSize > ASSOC_REQ_LEN)
			pEntry->assoc_req_len = ASSOC_REQ_LEN;
		else
			pEntry->assoc_req_len = DataSize;
		NdisMoveMemory(pEntry->assoc_req_frame, (VOID *)FC, pEntry->assoc_req_len);
	}
#endif
...
#ifdef ROAM_CALIB_SUPPORT
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, pEntry->Addr, wdev->bssid, "Send Re-Assoc to CFG", __func__);
#endif /* ROAM_CALIB_SUPPORT */

			CFG80211OS_RxMgmt(pNetDev, freq, FC, DataSize, 0);
...
#ifdef ROAM_CALIB_SUPPORT
		if (pAd->RoamCalibEnable)
			RoamCalibData(pAd, pEntry->Addr, wdev->bssid, "Send OWE Re-Assoc to Hostapd", __func__);
#endif /* ROAM_CALIB_SUPPORT */
		CFG80211OS_RxMgmt(pNetDev, freq, FC, DataSize, 0);
	} else {
...
#ifdef ROAM_CALIB_SUPPORT
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, ie_list->Addr2, NULL, "Send Assoc Resp to STA", __func__);
#endif /* ROAM_CALIB_SUPPORT */
		...
		if (pPmkid != NULL) {
			...
			if (IS_AKM_WPA3PSK(pEntry->SecConfig.AKMMap) &&
			   !is_pmkid_cache_in_sec_config(&pEntry->SecConfig)) {
				...
				MlmeDeAuthAction(pAd, pEntry, REASON_INVALID_PMKID, FALSE);
				goto LabelOK;
			}
		}
#ifdef WSC_AP_SUPPORT
	...
	*/
	...
	if ((pFtCfg != NULL) && (pFtCfg->FtCapFlag.Dot11rFtEnable) && isReassoc
		...
		if (info) {
			...
			if (wdev->SecConfig.gtk_Reinstall) {
				os_zero_mem(info, sizeof(ASIC_SEC_INFO));
...
#ifdef DOT11W_PMF_SUPPORT
			/* Set key material to Asic */
			if (wdev->SecConfig.PmfCfg.MFPC && wdev->SecConfig.igtk_Reinstall) {
				os_zero_mem(info, sizeof(ASIC_SEC_INFO));
				info->Operation = SEC_ASIC_ADD_GROUP_KEY;
				info->Direction = SEC_ASIC_KEY_TX;
				info->Wcid = Wcid;
