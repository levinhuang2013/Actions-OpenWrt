...
static VOID hostapd_auth_req(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (Fr && MsgLen) {
		...
		if (pAd->RoamCalibEnable)
			RoamCalibData(pAd, Fr->Hdr.Addr2, NULL, "Send FT Auth to Hostapd", __func__);
...
SendAuth:
	...
	if (mgmt->u.auth.auth_alg == AUTH_MODE_SAE
		...
		if (mgmt->u.auth.auth_transaction == 1) {
			wdev->do_not_send_sta_del_event_flag = 1;
			MacTableAuthReqClearMloEntry(pAd, wdev, mgmt->da, ml_ie,
	...
	if ((mgmt->u.auth.auth_alg == AUTH_MODE_OPEN) &&
		...
		if (pEntry) {
			...
			if (pEntry->mlo.mlo_en) {
				wapp_send_sta_connect_rejected(pAd, pEntry->wdev,
				pEntry->mlo.mld_addr,
				pEntry->bssid,
