	...
	*/
	...
	if (MlmeScanReqSanity(pAd, Elem->Msg, Elem->MsgLen, &BssType, (PCHAR)Ssid, &SsidLen, &ScanType)) {
		...
		if ((ScanCtrl->state == OFFCHANNEL_SCAN_START)
			...
			/*disable fw log at default when off-ch scan running*/
			if (ScanCtrl->state == OFFCHANNEL_SCAN_START)
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
					"Disable fw log when off-ch starts!!\n");
			else
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
					"Disable fw log when MAP scan starts!!\n");
...
#ifdef RT_CFG80211_SUPPORT
#ifdef SCAN_SUPPORT
			if ((wdev->wdev_type == WDEV_TYPE_STA) && (pAd->CommonCfg.bIEEE80211H == 1)
				&& RadarChannelCheck(pAd, ScanCtrl->Channel) && (wdev->FireProbe_on_DFS == FALSE)
				&& CFG80211DRV_OpsScanRunning(pAd)) {
				PSTA_ADMIN_CONFIG pApCliEntry = &pAd->StaCfg[wdev->func_idx];

				wdev->FireProbe_on_DFS = TRUE;
				if (pApCliEntry->CfgSsidLen > 0) {
					scan_extra_probe_req(pAd, OPMODE_STA, SCAN_ACTIVE, wdev,
	...
	*/
	...
	if (pStaCfg) {
		...
		if (!HAS_EHT_MLD_EXIST(ie_list->cmm_ies.ie_exists))
			CLR_EHT_MLD_EXIST(pStaCfg->MlmeAux.ie_exists);

		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_SYNC, DBG_LVL_INFO,
			"RootAP EHT_CAP(%d), EHT_MLD(%d)\n",
			HAS_EHT_CAPS_EXIST(pStaCfg->MlmeAux.ie_exists) ? 1 : 0,
			HAS_EHT_MLD_EXIST(pStaCfg->MlmeAux.ie_exists) ? 1 : 0);
