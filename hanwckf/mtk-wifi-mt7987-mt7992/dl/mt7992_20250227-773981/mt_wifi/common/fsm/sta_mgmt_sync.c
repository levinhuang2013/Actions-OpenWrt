		...
		NDIS_802_11_VARIABLE_IEs *pVIE, USHORT LenVIE)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid)) {
		...
		if (SSID_EQUAL(pStaCfg->Ssid, pStaCfg->SsidLen, bcn_ie_list->Ssid, bcn_ie_list->SsidLen)) {
			if (at2lm_mld_sta_handle(pAd, wdev, pEntry->wcid, bcn_ie_list) < 0)
		...
		if (pEntry) {
			...
			|| (IS_AKM_DPP(pEntry_SecConfig->AKMMap))
				...
				if ((IS_MAP_ENABLE(pAd) && (IS_AKM_WPA3PSK(pEntry_SecConfig->AKMMap) ||
...
#endif
						/* Set MFPC/MFPR according to the AKM setting */
						...
						if (IS_AKM_FORCE_PMF_REQUIRED(pEntry_SecConfig->AKMMap))
