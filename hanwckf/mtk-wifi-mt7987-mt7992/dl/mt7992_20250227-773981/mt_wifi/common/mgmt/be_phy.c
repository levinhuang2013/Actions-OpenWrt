...
BOOLEAN phy_freq_adjust(struct wifi_dev *wdev, struct freq_cfg *cfg, struct freq_oper *op)
{
	...
	if (!is_testmode_wdev(wdev->wdev_type)) {
		...
		/* Use original operate BW stored in wdev,when scan restore */
		if (wdev->bRestore == TRUE  && wlan_operate_get_bw(wdev) != op->bw) {
			op->bw = wlan_operate_get_bw(wdev);
			phy_ht_vht_bw_adjust(op->bw, &op->ht_bw, &op->vht_bw);
			MTWF_DBG(NULL, DBG_CAT_HW, CATHW_PHY, DBG_LVL_NOTICE,
				"Scan Restore the original BW %d\n", op->bw);
		} else if (((pAd->CommonCfg.DfsParameter.bDedicatedZeroWaitSupport &&
		...
		pAd->CommonCfg.DfsParameter.ZwAdjBwFlag) {
			if (pAd->CommonCfg.DfsParameter.CACMemoEn && dfs_cac_op(pAd, wdev, CAC_DONE_CHECK, op->prim_ch)) {
				pAd->CommonCfg.DfsParameter.ZwAdjBw = op->bw;
			} else if (pAd->CommonCfg.DfsParameter.ZwAdjBw < op->bw) {
			...
			if (pAd->CommonCfg.DfsParameter.CACMemoEn && !dfs_cac_op(pAd, wdev, CAC_DONE_CHECK, op->prim_ch)) {
				...
				while (op->bw) {
					if (dfs_cac_check_by_bw(pAd, op->prim_ch, op->bw))
						break;
					op->bw = op->bw - 1;
				}
	...
	/* Always reset the scan restore flag after bandwidth adjustment */
	wdev->bRestore = FALSE;
...
VOID operate_loader_phy(struct wifi_dev *wdev, struct freq_cfg *cfg)
{
	...
	if ((GetCurrentChannelOpOwner(ad, wdev) == CH_OP_OWNER_SCAN) ||
		(GetCurrentChannelOpOwner(ad, wdev) == CH_OP_OWNER_PARTIAL_SCAN)) {
		cfg->ht_bw = wlan_operate_get_ht_bw(wdev);
		cfg->vht_bw = wlan_operate_get_vht_bw(wdev);
		cfg->eht_bw = wlan_operate_get_he_bw(wdev);
	}
	...
	if (IS_MAP_TURNKEY_ENABLE(ad) || IS_MAP_BS_ENABLE(ad)) {
		for (i = 0; i < WDEV_NUM_MAX; i++) {
			...
			if (WDEV_BSS_STATE(tdev) >= BSS_ACTIVE) {
				/* set flag for mesh when ch/bw change */
	...
	UCHAR config_bw)
{
	...
	ch_bw_cap = get_channel_bw_cap(wdev, channel);
	//todo: add ext_ch into target bw calculation

	return ch_bw_cap > config_bw ? config_bw : ch_bw_cap;
}
/*
* export function
*/
...
BOOLEAN wlan_operate_full_bw_scan(struct wifi_dev *wdev, UCHAR prim_ch)
{
	...
	case BW_40:
		...
		oper.ht_bw = HT_BW_40;
		oper.vht_bw = VHT_BW_2040;
		oper.cen_ch_1 = vht_cent_ch_freq_40mhz(wdev, prim_ch, VHT_BW_2040, oper.ch_band);
		break;
	case BW_80:
		oper.bw = BW_80;
		oper.ht_bw = HT_BW_40;
