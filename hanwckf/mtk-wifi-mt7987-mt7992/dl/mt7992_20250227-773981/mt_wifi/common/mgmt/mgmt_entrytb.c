...
extern UINT8 vow_infra_bwcg_mapping(PRTMP_ADAPTER pad, MAC_TABLE_ENTRY * pEntry);
...
VOID TRTableInsertMcastEntry(RTMP_ADAPTER *pAd, UINT16 hw_wcid, struct wifi_dev *wdev, BOOLEAN isVlan)
{
	...
	if (!IS_WCID_VALID(pAd, wdev->tr_tb_idx))
		return;
	if (isVlan)
		tr_entry = tr_entry_get(pAd, hw_wcid);
	else
...
VOID MgmtTableSetMcastEntry(RTMP_ADAPTER *pAd, UINT16 hw_wcid, struct wifi_dev *wdev, BOOLEAN isVlan)
{
...
#endif /* CONFIG_6G_SUPPORT */
	if (hw_wcid)
		TRTableInsertMcastEntry(pAd, hw_wcid, wdev, isVlan);
}
	...
	if ((!(pAd->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG)) || pAd->vow_cfg.en_bw_ctrl) {
		if (VOW_IS_ENABLED(pAd)) {
			...
			if (pAd->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG
				...
				new_group_id = vow_infra_bwcg_mapping(pAd, pEntry);
	...
	IN UCHAR *mld_addr)
{
	...
	UINT16 i;
	MAC_TABLE_ENTRY *pEntry = NULL, *pCurrEntry, *pSearchEntry;
	struct _STA_TR_ENTRY *tr_entry = NULL;
	...
	if (WDEV_BSS_STATE(wdev) != BSS_READY && wdev->wdev_type == WDEV_TYPE_AP) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR,
		"Return NULL, wdev state(%d) is not BSS_READY\n", WDEV_BSS_STATE(wdev));
...
#ifdef ROAM_CALIB_SUPPORT
	snprintf(desr, 200, "Create MacTable Entry (caller:%pS)", OS_TRACE);
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, pAddr, NULL, desr, __func__);
#endif /* ROAM_CALIB_SUPPORT */
	...
	NdisAcquireSpinLock(pAd->MacTabLock);
	pSearchEntry = MacTableLookup(pAd, pAddr);
	if (pSearchEntry) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR,
		"Return NULL, Sta already exist in mactable, addr="MACSTR", pEntry=%p\n", pAddr, pSearchEntry);
		NdisReleaseSpinLock(pAd->MacTabLock);
		...
		do {
			...
			if (ent_type == ENTRY_CLIENT) {
				...
				if (pAd->vow_gen.VOW_GEN >= VOW_GEN_BESRA) {
					...
					if ((ph_dev && ph_dev->bw_en_mask) || vow_watf_is_enabled(pAd))
						RTMP_SET_STA_DWRR(pAd, pEntry);
				} else {
...
uint8_t _MacTableDeleteEntry(struct _RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr)
{
	...
	if (bDeleteEntry) {
		...
		*/
		...
		if (bIndicateSendEvent && pEntry && wdev && IS_MBO_ENABLE(wdev)) {
...
/*optimizing events to daemon when all the stations are disconnected from the driver*/
			if (bIndicateSendEvent && pEntry && IS_ENTRY_CLIENT(pEntry) && wdev && wdev->if_dev && pAd->disconnect_all_sta == 1) {
				wapp_send_cli_all_leave_event(pAd, RtmpOsGetNetIfIndex(wdev->if_dev), TmpAddrForIndicate, pEntry);
				pAd->disconnect_all_sta = 2;
			}
		} else {
#endif
			if (bIndicateSendEvent && pEntry && IS_ENTRY_CLIENT(pEntry) && wdev && wdev->if_dev)
...
VOID MacTableResetWdev(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (!OPSTATUS_TEST_FLAG_WDEV(wdev, fOP_AP_STATUS_MEDIA_STATE_CONNECTED)) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "wdev %d not ready !!!\n",
			wdev->wdev_idx);
		return;
	}
...
{
	struct _MAC_TABLE_ENTRY *pLinkEntry, *pMldEntry;
	struct ml_ie_info *ml_info = NULL;

	/* sanity the peer's mlo-capability if peer existing, we need to check
	1, link addr 2, mld addr if is_mlo_auth */
	pLinkEntry = MacTableLookupForTx(pAd, link_addr, NULL);
	if (pLinkEntry && IS_ENTRY_CLIENT(pLinkEntry)) {
		/* (mlo -> non-mlo/mlo) || (non-mlo -> mlo) */
		if (IS_ENTRY_MLO(pLinkEntry) || is_mlo_auth) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_ENTRY, DBG_LVL_ERROR,
				"clear peers, mac=%pM, wcid=%d, is_mlo=%d, request_mlo=%d\n",
				pLinkEntry->Addr, pLinkEntry->wcid, IS_ENTRY_MLO(pLinkEntry), is_mlo_auth);
			MacTableDeleteEntry(pAd, pLinkEntry->wcid, pLinkEntry->Addr);
		}
	}

	if (ml_ie && is_mlo_auth) {
		...
		eht_parse_multi_link_ie(wdev, ml_ie, NULL, ml_info);

		pMldEntry = MacTableLookupForTx(pAd, ml_info->mld_addr, NULL);
		/*is_mlo_auth, but has pEntry of this mld address, so clear it*/
		if (pMldEntry && IS_ENTRY_CLIENT(pMldEntry)) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_ENTRY, DBG_LVL_ERROR,
				"clear non mlo peers, mac=%pM, wcid=%d, is_mlo=%d, request_mlo=%d\n",
				pMldEntry->Addr, pMldEntry->wcid, IS_ENTRY_MLO(pMldEntry), is_mlo_auth);
			MacTableDeleteEntry(pAd, pMldEntry->wcid, pMldEntry->Addr);
		}
	}
end:
	if (ml_info)
