	...
	while ((Length + 2 + sta_pf_eid->Len) <= sta_profile_len) {
		...
		case IE_WLAN_EXTENSION: {
			...
			if (max_ch_swich_len > 0) {
				...
				if (*subelement == EID_EXT_MAX_CH_SWITCH_TIME) {
					subelement += 1;
					...
					"%s() - wrong EID_EXT_MAX_CH_SWITCH_TIME in STA profile\n",
					__func__);
				}
			}
			break;
...
int sta_mld_link_sync(struct wifi_dev *wdev_src, MAC_TABLE_ENTRY *pEntry_src)
{
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		if (mld->peer_mld.single_link[i].active && mld->mld_own_links[i].used) {
			...
			pStaCfg_dst->ApcliInfStat.Valid = pStaCfg_src->ApcliInfStat.Valid;
			...
			if (pStaCfg_dst->ApcliInfStat.Enable == TRUE) {
				...
				pStaCfg_dst->ApcliInfStat.ApCliLinkUpTime = pStaCfg_src->ApcliInfStat.ApCliLinkUpTime;
			} else {
				NdisZeroMemory(pStaCfg_dst->Bssid, MAC_ADDR_LEN);
				NdisZeroMemory(pStaCfg_dst->MlmeAux.Bssid, MAC_ADDR_LEN);
				pStaCfg_dst->ApcliInfStat.ApCliLinkUpTime = 0;
			}
			...
			if (pStaCfg_src->SsidLen <= MAX_LEN_OF_SSID + 1) {
				NdisMoveMemory(pStaCfg_dst->Ssid, pStaCfg_src->Ssid, pStaCfg_src->SsidLen);
...
int sta_mld_update_rx_bcn_time(struct wifi_dev *pwdev, uint8_t rx_bcn_type)
{
	...
	if (setup_wdev && setup_wdev->sys_handle && pwdev->mld_dev) {
		mld = pwdev->mld_dev;
		for (idx = 0; idx < MLD_LINK_MAX; idx++) {
			if (mld->peer_mld.single_link[idx].active && mld->mld_own_links[idx].used) {
				wdev = mld->mld_own_links[idx].wdev;
				if (!wdev)
					continue;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		for (j = 0; j < BSS_MNGR_MAX_BAND_NUM; j++) {
			...
			if (sta_profile_len <= 4) {
				...
				/*update HT cap from common ies*/
				if (HAS_HT_CAPS_EXIST(comm_ies->ie_exists)
					&& WMODE_CAP_N(non_setup_link_pEntry->wdev->PhyMode)) {
					SET_HT_CAPS_EXIST(ie_list->cmm_ies.ie_exists);
					NdisMoveMemory(&ie_list->cmm_ies.ht_cap, &comm_ies->ht_cap, sizeof(comm_ies->ht_cap));
				}
...
int init_sta_mld_link_mgr(void)
{
	...
	mutex_init(&mld_device.peer_mld.at2lm_contract.op_mutex);
...
VOID show_sta_mlo_link_info(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		if (mld->mld_own_links[i].used == 1) {
			...
			pad = (PRTMP_ADAPTER) pCurwdev->sys_handle;
			if (pad == NULL) {
				MTWF_DBG(pAd, DBG_CAT_MLO, CATMLO_BMGR, DBG_LVL_ERROR,
					"pad not found\n");
				return;
			...
			pentry = GetAssociatedAPByWdev(pad, pCurwdev);
			if (pentry && pentry->mlo.mlo_en &&
				pentry->mlo.link_info.link_id < MLD_LINK_MAX)
				MTWF_PRINT("\tlinkid:%d, valid.\n", pentry->mlo.link_info.link_id);
			else
				MTWF_PRINT("\tlinkid:NA, invalid.\n");
		...
		if (add_link_rnr_info) {
			...
			for (link = 0; link < MLD_LINK_MAX; link++) {
				...
				if (add_link_rnr_info->channel_band == channel_band &&
					IsValidChannel(temp_pAd, add_link_rnr_info->channel, tmp_wdev))
					break;
			}
			if (link < MLD_LINK_MAX)
				add_link = TRUE;
		...
		for (link_id = 0; link_id < BSS_MNGR_MAX_BAND_NUM; link_id++) {
			...
			if (ml_link->active) {
				...
				for (i = 0; i < MLD_LINK_MAX; i++) {
					...
					tmp_entry = peer_link->priv_ptr;
					if (!tmp_entry || !tmp_entry->wdev)
						continue;
					tmp_wdev = tmp_entry->wdev;
				...
				link_id_bss_info_idx[ml_link->link_id] =
					tmp_wdev->bss_info_argument.ucBssIndex;
