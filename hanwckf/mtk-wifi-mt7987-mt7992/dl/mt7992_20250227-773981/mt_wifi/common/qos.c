	...
	UCHAR *srcAddr, UCHAR *buf, UCHAR buflen)
{
	...
	while ((*ptr) == EID_EXTENSION && (*(ptr + 2)) == EID_EXT_TCLAS_MASK) {
		subielen = *(ptr + 1);
		ptr += 2;
		leftlen -= 2;
		if (subielen > leftlen)
...
void QoS_PeerRobust_AV_Action(struct _RTMP_ADAPTER *pAd, struct _MLME_QUEUE_ELEM *Elem)
{
	...
	case ACT_MSCS_REQ:
		...
		if  (mscs_desc->ElementID == EID_EXTENSION &&
				mscs_desc->ElementID_Ext == EID_EXT_MSCS_DESC) {
	...
	UINT8 reasoncode, struct wapp_vend_spec_classifier_para_report *classifier_para)
{
	...
	if (classifier_para) {
		...
		mscs_desc->ElementID = EID_EXTENSION;
		...
		ptclas_elem->ElementID = EID_EXTENSION;/*fill tclas ie*/
		ptclas_elem->Length =
			sizeof(TCLAS_ELEM) + sizeof(FRAME_CLASS_TYPE_4_ipv4) - 2; /*len*/
		ptclas_elem->UP_or_ElementID_Ext = EID_EXTENSION;
	...
	struct wifi_dev *wdev, UCHAR *srcAddr, UCHAR *buf, ULONG len, UINT8 *reasoncode)
{
	...
	if ((ptclas_elem->ElementID != EID_EXTENSION) ||
		(ptclas_elem->UP_or_ElementID_Ext) != EID_EXT_TCLAS_MASK) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_QOS_MAP, DBG_LVL_ERROR,
			"tclas ie error\n");
		return FALSE;
	}
