...
VOID UserCfgInit(RTMP_ADAPTER *pAd)
{
	...
	*/
	...
	if (hc_get_tx_sw_token_nums_all_band(pAd, pAd->MacTab->MaxToken, MAX_BAND_NUM))
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"get tx sw token num fail!\n");
...
#ifdef CONFIG_AP_SUPPORT
#ifdef DOT11_HE_AX
	/* init bss color per band variables */
	pAd->ApCfg.bss_color_cfg.collision_ignore = FALSE;
#endif /* DOT11_HE_AX */
#endif /* CONFIG_AP_SUPPORT */
