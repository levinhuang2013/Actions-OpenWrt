...
int mt_wifi_init(VOID *pAdSrc, RTMP_STRING *pDefaultMac, RTMP_STRING *pHostName)
{
	...
	if (chip_ops && chip_ops->update_chip_cap)
		chip_ops->update_chip_cap(pAd);
...
VOID RTMPDrvOpen(VOID *pAdSrc)
{
	...
	*/
	...
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_OFFLOAD)) {
		for (i = 0; i < TxQ_AC_NUM; i++) {
			MtCmdCr4Set(pAd, WA_SET_OPTION_PER_AC_RETRY, i, 0);
			if (pAd->retrylimit[i] > 0) {
				MtCmdCr4Set(pAd, WA_SET_OPTION_PER_AC_RETRY, i, pAd->retrylimit[i]);
			}
		}
	}
