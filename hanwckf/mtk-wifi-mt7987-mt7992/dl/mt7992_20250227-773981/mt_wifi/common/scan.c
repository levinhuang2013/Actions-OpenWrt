...
INT scan_ch_restore(RTMP_ADAPTER *pAd, UCHAR OpMode, struct wifi_dev *pwdev)
{
	...
	/*For adjusting bw when scan restore*/
	wdev->bRestore = TRUE;
...
#endif /* MBO_SUPPORT */
#ifdef DOT11_EHT_BE
	/*Add MLO IE in probe request if not use Wildcard SSID*/
	if (ScanCtrl->SsidLen != 0) {
		ie_info.frame_buf = (UCHAR *)(frm_buf + FrameLen);
...
BOOLEAN scan_active_probe_disallowed(RTMP_ADAPTER *pAd, UCHAR channel)
{
	...
	/*for dfs ch, allow to probe for supplicant connection via active scan*/
	if (channel > 14) {
		if ((pAd->CommonCfg.bIEEE80211H == 1) &&
			RadarChannelCheck(pAd, channel) &&
			!(CFG80211DRV_OpsScanRunning(pAd) && pAd->ScanCtrl.SsidLen != 0))
			bScanPassive = TRUE;
	}
...
/* TODO: Star, fix me when Scan is prepare to modify */
	...
	if ((ScanCtrl->Channel == 0) || ScanPending) {
		...
		if (IS_MAP_TURNKEY_ENABLE(pAd) && ScanType == SCAN_ACTIVE
			...
			if (strncmp(pAd->physical_dev->fw_log_ctrl.fw_log_arg, "0:0", 3)) {
#ifdef WIFI_UNIFIED_COMMAND
				if (cap->uni_cmd_support) {
		...
		if (ScanCtrl->state == OFFCHANNEL_SCAN_COMPLETE) {
			...
			/*resume fw log when off-ch scan ends*/
			if (strncmp(pAd->physical_dev->fw_log_ctrl.fw_log_arg, "0:0", 3)) {
#ifdef WIFI_UNIFIED_COMMAND
				if (cap->uni_cmd_support) {
					MtUniCmdFwLog2Host(pAd, HOST2N9, 1);
...
#ifdef CONFIG_STA_SUPPORT
			if ((wdev->wdev_type == WDEV_TYPE_STA) &&
				((ScanType == SCAN_ACTIVE) || (ScanType == SCAN_PARTIAL)) &&
				(ScanCtrl->SsidLen > 0)) {
...
VOID scan_partial_trigger_checker(RTMP_ADAPTER *pAd)
{
	...
	UINT ScanType = SCAN_TYPE_MAX;
		...
		if (ScanCtrl->PartialScan.bScanning == TRUE) {
			...
			ScanInfo = &wdev->ScanInfo;
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
					 "(%s) NumOfChannels = %u, LastScanChannel = %u, bscanwait = %u, BreakTime = %u[%d]\n",
					  wdev->if_dev->name,
					  ScanCtrl->PartialScan.NumOfChannels,
					  ScanInfo->LastScanChannel,
					  ScanCtrl->PartialScan.bscanwait,
