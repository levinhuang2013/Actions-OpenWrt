	...
	IN RTMP_STRING * arg)
{
	...
	token = rstrtok(arg, ",");
	while (token) {
		if (rtstrcasecmp(token, "OPEN") == TRUE)
			SET_AKM_OPEN(AKMMap);
		else if (rtstrcasecmp(token, "SHARED") == TRUE)
			SET_AKM_SHARED(AKMMap);
		else if (rtstrcasecmp(token, "WEPAUTO") == TRUE) {
			SET_AKM_OPEN(AKMMap);
			SET_AKM_AUTOSWITCH(AKMMap);
		} else if (rtstrcasecmp(token, "WPA") == TRUE)
			SET_AKM_WPA1(AKMMap);
		else if (rtstrcasecmp(token, "WPAPSK") == TRUE)
			SET_AKM_WPA1PSK(AKMMap);
		else if (rtstrcasecmp(token, "WPANONE") == TRUE)
			SET_AKM_WPANONE(AKMMap);
		else if (rtstrcasecmp(token, "WPA2") == TRUE)
			SET_AKM_WPA2(AKMMap);
		else if (rtstrcasecmp(token, "WPA2MIX") == TRUE) {
			SET_AKM_WPA2(AKMMap);
			SET_AKM_WPA2_SHA256(AKMMap);
		} else if (rtstrcasecmp(token, "WPA2PSK") == TRUE)
			SET_AKM_WPA2PSK(AKMMap);
		else if (rtstrcasecmp(token, "WPA2PSKMIX") == TRUE) {
			SET_AKM_WPA2PSK(AKMMap);
			SET_AKM_WPA2PSK_SHA256(AKMMap);
		} else if (rtstrcasecmp(token, "WPA3") == TRUE) {
			/* WPA3 code flow is same as WPA2, */
			/* the usage of SEC_AKM_WPA3 is to force pmf on */
			SET_AKM_WPA2(AKMMap);
			SET_AKM_WPA3(AKMMap);
		}
	#ifdef DOT11_SUITEB_SUPPORT
		else if (rtstrcasecmp(token, "WPA3-192") == TRUE)
			SET_AKM_WPA3_192BIT(AKMMap);
	#endif
	#if defined(DOT11_SAE_SUPPORT) || defined(SUPP_SAE_SUPPORT)
		else if (rtstrcasecmp(token, "WPA3PSK") == TRUE) {
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "WPA3PSK_EXT") == TRUE) {
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "WPA2PSKWPA3PSK") == TRUE) {
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "WPA2PSKMIXWPA3PSK") == TRUE) {
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
			SET_AKM_WPA2PSK_SHA256(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		...
		else if (rtstrcasecmp(token, "WPA1WPA2") == TRUE) {
			SET_AKM_WPA1(AKMMap);
			SET_AKM_WPA2(AKMMap);
		} else if (rtstrcasecmp(token, "WPAPSKWPA2PSK") == TRUE) {
			SET_AKM_WPA1PSK(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
		} else if ((rtstrcasecmp(token, "WPA_AES_WPA2_TKIPAES") == TRUE)
				   || (rtstrcasecmp(token, "WPA_AES_WPA2_TKIP") == TRUE)
				   || (rtstrcasecmp(token, "WPA_TKIP_WPA2_AES") == TRUE)
				   || (rtstrcasecmp(token, "WPA_TKIP_WPA2_TKIPAES") == TRUE)
				   || (rtstrcasecmp(token, "WPA_TKIPAES_WPA2_AES") == TRUE)
				   || (rtstrcasecmp(token, "WPA_TKIPAES_WPA2_TKIPAES") == TRUE)
				   || (rtstrcasecmp(token, "WPA_TKIPAES_WPA2_TKIP") == TRUE)) {
			SET_AKM_WPA1PSK(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
		} else if (rtstrcasecmp(token, "OWE") == TRUE) {
			SET_AKM_OWE(AKMMap);
		}
	#ifdef OCE_FILS_SUPPORT
		else if (rtstrcasecmp(token, "FILS_SHA256") == TRUE) {
			SET_AKM_WPA2(AKMMap);
			SET_AKM_FILS_SHA256(AKMMap);
		} else if (rtstrcasecmp(token, "FILS_SHA384") == TRUE) {
			SET_AKM_WPA2(AKMMap);
			SET_AKM_FILS_SHA384(AKMMap);
		}
	#endif /* OCE_FILS_SUPPORT */
			else if (rtstrcasecmp(token, "DPP") == TRUE) {
			SET_AKM_DPP(AKMMap);
		} else if (rtstrcasecmp(token, "DPPWPA2PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
		} else if (rtstrcasecmp(token, "DPPWPA3PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_SAE_SHA256(AKMMap);
		} else if (rtstrcasecmp(token, "DPPWPA3PSKWPA2PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
	#ifdef MAP_R6
		} else if (rtstrcasecmp(token, "SAE-EXTDPPWPA3PSKWPA2PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXTDPPWPA2PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXTDPPWPA3PSK") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXTWPA2PSK") == TRUE) {
			SET_AKM_SAE_EXT(AKMMap);
			SET_AKM_WPA2PSK(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXTWPA3PSK") == TRUE) {
			SET_AKM_SAE_SHA256(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXTDPP") == TRUE) {
			SET_AKM_DPP(AKMMap);
			SET_AKM_SAE_EXT(AKMMap);
		} else if (rtstrcasecmp(token, "SAE-EXT") == TRUE) {
			SET_AKM_SAE_EXT(AKMMap);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_DEBUG,
				"Only AKM24 Supported\n");
	#endif
		}
	#ifdef CONFIG_HOTSPOT_R3
		else if (rtstrcasecmp(token, "WPA2-Ent-OSEN") == TRUE) {
	...
	IN RTMP_STRING * arg)
{
	...
	token = rstrtok(arg, ",");
	while (token) {
		if (rtstrcasecmp(token, "NONE") == TRUE)
			SET_CIPHER_NONE(Cipher);
		else if (rtstrcasecmp(token, "WEP") == TRUE)
			SET_CIPHER_WEP(Cipher);
		else if (rtstrcasecmp(token, "TKIP") == TRUE)
			SET_CIPHER_TKIP(Cipher);
		else if ((rtstrcasecmp(token, "AES") == TRUE) || (rtstrcasecmp(token, "CCMP128") == TRUE))
			SET_CIPHER_CCMP128(Cipher);
		else if (rtstrcasecmp(token, "CCMP256") == TRUE)
			SET_CIPHER_CCMP256(Cipher);
		else if (rtstrcasecmp(token, "GCMP128") == TRUE)
			SET_CIPHER_GCMP128(Cipher);
		else if (rtstrcasecmp(token, "GCMP256") == TRUE)
			SET_CIPHER_GCMP256(Cipher);
		else if ((rtstrcasecmp(token, "TKIPAES") == TRUE) || (rtstrcasecmp(token, "TKIPCCMP128") == TRUE)) {
			SET_CIPHER_TKIP(Cipher);
			SET_CIPHER_CCMP128(Cipher);
		} else if ((rtstrcasecmp(token, "WPA_AES_WPA2_TKIPAES") == TRUE)
			   || (rtstrcasecmp(token, "WPA_AES_WPA2_TKIP") == TRUE)
...
void sec_update_sae_cap(
	UINT8 h2e_only,
	UINT8 sae_pk,
	struct sae_capability *sae_cap)
{
	if (h2e_only == 0x3)
		sae_cap->gen_pwe_method = PWE_HASH_ONLY;
	else if (h2e_only == 0x2)
		sae_cap->gen_pwe_method = PWE_MIXED;
	else
		sae_cap->gen_pwe_method = PWE_LOOPING_ONLY;
	if (sae_pk)
		sae_cap->sae_pk_en = 1;
	else
		sae_cap->sae_pk_en = 0;

	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_NOTICE,
		"hostapd config h2e_only:0x%x, pwe_method:%d and saepk is :%d\n",
		h2e_only, sae_cap->gen_pwe_method, sae_cap->sae_pk_en);
}
