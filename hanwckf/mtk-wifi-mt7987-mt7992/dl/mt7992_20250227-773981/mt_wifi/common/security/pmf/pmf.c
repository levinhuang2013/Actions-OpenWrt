	...
	IN PVOID SystemSpecific3)
{
	...
	if (pEntry) {
...
#endif /* CONFIG_STA_SUPPORT */
		{
			...
			{
				record_connect_info(pAd, pEntry->Addr, REASON_DISASSOC_STA_LEAVING, FALSE);
	...
	IN struct _SECURITY_CONFIG *pSecConfigEntry)
{
	...
	if (pBuf == NULL) {
		...
		pSecConfigEntry->PmfCfg.MFPC = peer_MFPC = RsnCap.field.MFPC;
		pSecConfigEntry->PmfCfg.MFPR = peer_MFPR = RsnCap.field.MFPR;
		...
		if (IS_AKM_OWE(pSecConfigEntry->AKMMap)) {
			pSecConfigEntry->PmfCfg.MFPC = TRUE;
			pSecConfigEntry->PmfCfg.MFPR = TRUE;
		}
...
BOOLEAN PMF_PerformRxFrameAction(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		case NOT_ROBUST_UNICAST_FRAME:
			/*ALLOW SA Query on APCLI WPA2PSK+PMK*/
			if (pEntry && IS_AKM_WPA2PSK(pEntry->SecConfig.AKMMap)
					&& (pEntry->wdev->wdev_type == WDEV_TYPE_AP)) {
				MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_INFO,
	...
	IN struct wifi_dev *wdev)
{
	...
	if (wpa3_test_ctrl == 7) {
		...
		bss.bcn_prot_cfg.update_bipn = 1;
	}

	AsicBssInfoUpdate(ad, &bss);
	bss.bcn_prot_cfg.update_bipn = 0;
}
#endif
#endif /* DOT11W_PMF_SUPPORT */
