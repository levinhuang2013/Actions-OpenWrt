...
extern INT update_bw_config(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT32 new_bw);
...
#elif defined(CONFIG_RCSA_SUPPORT)
	...
	CSA_IE_INFO *CsaInfo)
{
	...
	while ((Length + 1) < Elem->MsgLen) {
		...
		case EID_EXTENSION:
			/*parse EXTENSION EID*/
			extension_id = Elem->Msg[Length + 2];
			switch (extension_id) {
			case EID_EXT_BW_IND:
				RTMPMoveMemory(&CsaInfo->bw_ind_ie, &Elem->Msg[Length+3], Elem->Msg[Length + 1] - 1);
			break;
		...
		if (Length + Elem->Msg[Length+1] + 2 > 255)
			break;

		if (IE_ID == IE_CH_SWITCH_WRAPPER)
			Length += 2;
		else
	...
	IN	CSA_IE_INFO *CsaInfo)
{
	...
	// cal bw from CSA
	if (CsaInfo->wb_info.new_ch_width == 0) {
		if (CsaInfo->wb_info.center_freq_1 && CsaInfo->wb_info.center_freq_1 != CsaInfo->ChSwAnnIE.Channel)
			CsaInfo_bw = BW_40;
		else
			CsaInfo_bw = BW_20;
	} else if (CsaInfo->wb_info.new_ch_width == 1 && CsaInfo->wb_info.center_freq_1) {
		if (CsaInfo->wb_info.center_freq_2)
			CsaInfo_bw = BW_160;
		else
			CsaInfo_bw = BW_80;
	} else if (CsaInfo->wb_info.new_ch_width == 4 || CsaInfo->bw_ind_ie.control == 4) {
		CsaInfo_bw = BW_320;
	}

	opr_bw = wlan_operate_get_bw(wdev);

	if ((pAd->CommonCfg.bIEEE80211H == 1) &&
		CsaInfo->ChSwAnnIE.Channel != 0 &&
		(wdev->channel != CsaInfo->ChSwAnnIE.Channel || opr_bw != CsaInfo_bw) &&
		pDot11h->ChannelMode != CHAN_SWITCHING_MODE) {
		MTWF_DBG(NULL, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"[APCLI] Following root AP to switch channel to ch%u or switch bandwidth to BW%u\n",
					CsaInfo->ChSwAnnIE.Channel, CsaInfo_bw);
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
				"update bw, opr_bw = %d, CsaInfo_bw = %d\n", opr_bw, CsaInfo_bw);
		err_code = update_bw_config(pAd, wdev, (UINT32)CsaInfo_bw);
		if (err_code) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
				"update bw failed!\n");
		...
		if (wdev->channel != CsaInfo->ChSwAnnIE.Channel)
			rtmp_set_channel(pAd, wdev, CsaInfo->ChSwAnnIE.Channel);
		else if (opr_bw != CsaInfo_bw) {
			//set new bw
