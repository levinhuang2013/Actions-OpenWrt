		      ...
		      UCHAR mtk_vendor_ie_tlv1_type)
{
	...
	ULONG mtk_ie_len = 0, vendor_ie_len = 0, i = 0;
#ifdef DOT11_EHT_BE
	...
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE2)) {
		...
		for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
			if (i < ARRAY_SIZE(wdev->bcn_buf.csa.ml_ie_offset) && vie_frm_type == VIE_BEACON && wdev->bcn_buf.csa.ml_ie_offset[i] != 0)
				wdev->bcn_buf.csa.ml_ie_offset[i] += mtk_tlv1_type2->len;
		}
