...
static VOID update_unicast_all_info(struct wifi_dev *wdev, BOOLEAN bActive)
{
	struct _DEV_INFO_CTRL_T *devinfo = &wdev->DevInfo;
	struct _RTMP_ADAPTER *ad = wdev->sys_handle;
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		if (bActive == FALSE)
			CmdUnicastAllEnable(ad, bActive, devinfo->BandIdx, devinfo->OwnMacIdx, wdev->bss_info_argument.ucBssIndex);
	}
}
...
#ifdef IGMP_SNOOPING_OFFLOAD
	update_unicast_all_info(wdev, act);
#endif /* IGMP_SNOOPING_OFFLOAD */
#endif
#endif
	MTWF_DBG(NULL, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_INFO,
...
INT wifi_sys_bss_query_mld(struct wifi_dev *wdev, struct bss_mld_info *mld_info)
{
	...
	if (!mld_info) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_ENTRY, DBG_LVL_ERROR, "mld_info=NULL,fail");
		return NDIS_STATUS_FAILURE;
	}
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		...
		if (!WMODE_CAP_BE(wdev->PhyMode) || (mld_group_idx == 0)) {
			...
			mld_info->link_id = MLD_LINK_ID_RSVD;
		...
		if (wdev->mld_dev && wdev->mld_dev->peer_mld.valid) {
			MAC_TABLE_ENTRY *pentry = GetAssociatedAPByWdev(ad, wdev);

			if (pentry && pentry->mlo.mlo_en &&
				pentry->mlo.link_info.link_id < MLD_LINK_MAX)
				mld_info->link_id = pentry->mlo.link_info.link_id;
			else
				mld_info->link_id = MLD_LINK_ID_RSVD;
			mld_info->mld_group_idx = wdev->mld_dev->mld_grp_idx;
			NdisCopyMemory(mld_info->mld_addr, wdev->mld_dev->mld_addr, MAC_ADDR_LEN);
			MTWF_DBG(ad, DBG_CAT_MLO, CATMLO_ENTRY, DBG_LVL_INFO,
			"(sta):mld_gp_idx=%d,mld_addr=%pM, link_id(%d) wdev(%pM)\n",
			mld_info->mld_group_idx, mld_info->mld_addr, mld_info->link_id, wdev->if_addr);
		} else {
			mld_info->mld_group_idx = MLD_GROUP_NONE;
			mld_info->link_id = MLD_LINK_ID_RSVD;
			NdisCopyMemory(mld_info->mld_addr, ZERO_MAC_ADDR, MAC_ADDR_LEN);
