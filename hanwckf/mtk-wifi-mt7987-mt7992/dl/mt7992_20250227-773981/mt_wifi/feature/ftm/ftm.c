	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	/* init iSTA */
	for (idx = 0; idx < MAX_FTM_TBL_SIZE; idx++) {
		pPeerInfo = &pAd->ftm_peer[idx];

		NdisZeroMemory(pPeerInfo, sizeof(struct _FTM_PEER_INFO));
...
)
{
	...
	// if asap==1, and sb. still in burst, drop iFTMR pkt
	do {
		OS_SEM_LOCK(&pAd->ftm_lock);
		pEntry = wdev->FtmCtrl.FtmPeer + idx;

		if (pEntry->State == FTMPEER_MEASURING_IN_BURST) {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_DEBUG,
					 "FTM %d_["MACSTR"] is FTMPEER_MEASURING_IN_BURST\n",
...
)
{
	...
	if (pEntry->VerdictPeerInfo.asap == 0) {
		...
		if (pEntry->VerdictPeerInfo.partial_tsf < PartialTSF)
			pEntry->VerdictPeerInfo.partial_tsf = PartialTSF + 256;

		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_DEBUG,
				"PartialTSF: %u, new PTSF: %u\n", PartialTSF, pEntry->VerdictPeerInfo.partial_tsf);
	}
}
...
)
{
	...
	/* received FTMR
	 * [ trigger = 0 ]
	 * MAC entry=NULL, FTM entry=NULL <--- drop
	 * MAC entry=NULL, FTM entry=exist <--- X
	 * MAC entry=exist, FTM entry=NULL <--- drop
	 * MAC entry=exist, FTM entry=exist <--- send FW
	 * [ trigger = 1 ]
	 * MAC entry=NULL, FTM entry=NULL <--- create both
	 * MAC entry=NULL, FTM entry=exist <--- X
	 * MAC entry=exist, FTM entry=NULL <--- create FTM entry
	 * MAC entry=exist, FTM entry=exist <--- send FW
	 */

	if (pFtmReq->Trigger == FTM_TRIGGER_STOP && (!VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid) ||
		(VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid) && FtmEntrySearch(pAd, wdev, Fr->Hdr.Addr2) == NULL))) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_INFO,
			"pFtmReq->Trigger %d, wcid %d\n", pFtmReq->Trigger, Elem->Wcid);
		return;
	}
	...
	if (pEntry == NULL) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_WARN,
			"FTM peer measuring in burst or no available entry to handle\n");
		return;
	}

	/* get or create STAREC */
	RTMPMoveMemory(&pEntry->VerdictPeerInfo.Responder, Fr->Hdr.Addr2, MAC_ADDR_LEN);
...
VOID FtmProfileNeighborApParse(RTMP_ADAPTER *pAd, UINT NeighborIdx, RTMP_STRING *tmpbuf)
{
	...
	for (j = 0, NeighborInfo = rstrtok(tmpbuf, ";"); NeighborInfo && j < MAX_CIVIC_CA_VALUE_LENGTH; NeighborInfo = rstrtok(NULL, ";"), j++) {
		...
		if (strncmp(NeighborInfo, "Bssid:", 6) == 0) {
			arg_len = strlen(NeighborInfo);
			arg_len = (arg_len >= 1024) ? 1023 : arg_len;
			NdisZeroMemory(tmpNeighborInfo, 1024);
			NdisCopyMemory(tmpNeighborInfo, NeighborInfo, arg_len);
			tmpNeighborInfo[arg_len] = '\0';
		} else {
			CHAR *pch = NULL;
