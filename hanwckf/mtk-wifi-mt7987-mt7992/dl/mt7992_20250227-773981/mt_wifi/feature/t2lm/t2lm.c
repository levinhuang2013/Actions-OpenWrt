...
/**
 * @at2lm sanity check on apcli side.
 *
 * @param *mld
 * @param *bcn_ie_list
 */
int at2lm_mld_sta_sanity_check(struct mld_dev *mld,
	BCN_IE_LIST *bcn_ie_list)
{
	int ret = 0;
	UINT8 link_id, rnr_idx, rnr_link_id, mld_link_id;
	struct sta_mld_at2lm_link_info *link_at2lm_info = NULL;
	u16 disable_link_map = 0;
	...
	/*update rnr info to struct sta_mld_at2lm_link_info*/
	for (rnr_idx = 0; rnr_idx < MAX_MLD_GROUP_NUM; rnr_idx++) {
		...
		for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
			if (mld->peer_mld.single_link[link_id].active &&
				mld->mld_own_links[link_id].used) {
				tmp_entry = (MAC_TABLE_ENTRY *)mld->peer_mld.single_link[link_id].priv_ptr;
				mld_link_id = mld->peer_mld.single_link[link_id].link_id;
			} else
				tmp_entry = NULL;
			if (!tmp_entry)
				continue;
		...
		if (link_id < MLD_LINK_MAX) {
			link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[rnr_link_id];
			link_at2lm_info->link_dis_ind =
				bcn_ie_list->rnr_ml_parms[rnr_idx].link_disable_ind;
		}
	}

	for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
		link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[link_id];
		if (!link_at2lm_info->at2lm_ie_exist)
			continue;

		if (link_at2lm_info->dir != T2LM_BI) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_NOTICE,
				"dir check fail, dir=%d\n", link_at2lm_info->dir);
			ret = -1;
			goto err;
		}
		if (link_at2lm_info->link_disabled)
			disable_link_map |= 1 << link_id;
	}
	if (disable_link_map == 0)
		return ret;
	...
	for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
		...
		if (link_at2lm_info->tid_map[link_id] == 0) {
			if (!(disable_link_map & (1 << link_id))) {
				MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
					"disable link check fail, disable_map=%d\n", disable_link_map);
				ret = -1;
				break;
			}
		} else if (link_at2lm_info->tid_map[link_id] == 0xFF) {
			if ((disable_link_map & (1 << link_id))) {
				MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
					"enable link check fail, disable_link_map=%d\n", disable_link_map);
				ret = -1;
				break;
			}
		} else {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
				"tid_map link check fail, link=%d, map=%d\n", link_id,
				link_at2lm_info->tid_map[link_id]);
			ret = -1;
			break;
		}
	}
...
/**
 * @update t2lm and send fw cmd to WM.
 *
 * @param *mld sta mld struct
 * @param *wdev target wdev for send fw cmd
 */
int at2lm_sta_mld_tid_map_update(struct mld_dev *mld,
	struct wifi_dev *wdev)
{
	int ret = 0, i;
	struct nt2lm_req_t req = {0};
	struct sta_mld_at2lm_contract *at2lm_contract = NULL;
	MAC_TABLE_ENTRY *pEntry = NULL;
	UINT8 link_id;
	...
	at2lm_contract = &mld->peer_mld.at2lm_contract;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		if (!mld->peer_mld.single_link[i].active)
			continue;
		pEntry = mld->peer_mld.single_link[i].priv_ptr;
		if (!pEntry)
			continue;
		link_id = mld->peer_mld.single_link[i].link_id;
		req.link_id_bitmap |= 1 << link_id;
		req.link_id_to_wcid[link_id] = pEntry->wcid;
		req.tid_map_dl[link_id] = at2lm_contract->tid_map[link_id];
		req.tid_map_ul[link_id] = at2lm_contract->tid_map[link_id];
	}
...
/**
 * @handle mld at2lm state in MST state. update t2lm to fw
 *
 * @param *mld sta mld struct
 */
int at2lm_mld_sta_mst_handle(struct mld_dev *mld)
{
	int ret = 0;
	UINT8 link_id;
	UINT8 cmd_to_disable_link = FALSE;
	UINT16 mst_end;
	struct sta_mld_at2lm_link_info *link_at2lm_info;
	...
	for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
		if (!mld->peer_mld.single_link[link_id].active)
			continue;
		link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[link_id];
		if (link_at2lm_info->link_dis_ind) {
			cmd_to_disable_link = TRUE;
			break;
		}
		if (link_at2lm_info->link_disabled &&
			!link_at2lm_info->link_dis_ind) {
			if (link_at2lm_info->bcn_tsf >= link_at2lm_info->mst_tsf)
				mst_end = link_at2lm_info->bcn_tsf - link_at2lm_info->mst_tsf;
			else
				mst_end = link_at2lm_info->mst_tsf - link_at2lm_info->bcn_tsf;
			if (mst_end <= link_at2lm_info->bcn_interval &&
				link_at2lm_info->dtim_bcn) {
				cmd_to_disable_link = TRUE;
				break;
			}
		}
	...
	if (cmd_to_disable_link) {
		MAC_TABLE_ENTRY *pEntry = NULL;

		NdisMoveMemory(&mld->peer_mld.at2lm_contract.tid_map,
			link_at2lm_info->tid_map, MLD_LINK_MAX);
		pEntry = mld->peer_mld.single_link[link_id].priv_ptr;
		...
		at2lm_sta_mld_tid_map_update(mld, pEntry->wdev);
		mld->peer_mld.at2lm_contract.state = AT2LM_STE_ED;
	}
...
/**
 * @handle mld at2lm state in ED state. update t2lm to fw
 *
 * @param *mld sta mld struct
 */
int at2lm_mld_sta_ed_handle(struct mld_dev *mld)
{
	int ret = 0, i;
	UINT8 link_id;
	UINT8 cmd_to_enable_link = FALSE;
	struct sta_mld_at2lm_link_info *link_at2lm_info;
	struct _MAC_TABLE_ENTRY *tmp_entry = NULL;
	struct sta_mld_at2lm_contract *at2lm_mld_info;
	...
	for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
		...
		link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[link_id];
		if (!link_at2lm_info->at2lm_ie_exist) {
			cmd_to_enable_link = TRUE;
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
				"enable link = %d\n", link_id);
			break;
		}
		if (link_at2lm_info->at2lm_ie_exist && !link_at2lm_info->link_disabled &&
			link_at2lm_info->ed_tsf == 0) {
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
				"enable link = %d, ed=%d\n", link_id, link_at2lm_info->ed_tsf);
			cmd_to_enable_link = TRUE;
			break;
		}
	}
	if (cmd_to_enable_link) {
		at2lm_mld_info = &mld->peer_mld.at2lm_contract;
		for (i = 0; i < MLD_LINK_MAX; i++)
			at2lm_mld_info->tid_map[i] = 0xff;

		tmp_entry = mld->peer_mld.single_link[link_id].priv_ptr;
		if (!tmp_entry || !tmp_entry->wdev) {
			ret = -1;
			MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_ERROR,
			 "err=%d\n", ret);
			goto err;
		}
		at2lm_sta_mld_tid_map_update(mld, tmp_entry->wdev);
		mld->peer_mld.at2lm_contract.state = AT2LM_STE_NONE;
		...
		for (link_id = 0; link_id < MLD_LINK_MAX; link_id++) {
			if (!mld->peer_mld.single_link[link_id].active)
				continue;
			link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[link_id];
			link_at2lm_info->link_disabled = 0;
			link_at2lm_info->bcn_tsf = 0;
			link_at2lm_info->ed_tsf = 0;
			link_at2lm_info->mst_tsf = 0;
			link_at2lm_info->at2lm_state = AT2LM_STE_NONE;
			link_at2lm_info->link_dis_ind = 0;
		}
	}
...
/**
 * @handle at2lm info on apcli side, called per beacon.
 *
 * @param *pAd
 * @param *wdev
 * @param wcid
 * @param *bcn_ie_list
 */
int at2lm_mld_sta_handle(struct _RTMP_ADAPTER *pAd,
		struct wifi_dev *wdev, UINT16 wcid, BCN_IE_LIST *bcn_ie_list)
{
	struct tid2lnk_ie_info *t2l_info;
	struct mld_dev *mld;
	struct sta_mld_at2lm_contract *at2lm_info;
	MAC_TABLE_ENTRY *pEntry = NULL;
	struct sta_mld_at2lm_link_info *link_at2lm_info;
	struct sta_mld_at2lm_contract *mld_at2lm_info;
	int ret = 0;
	UINT8 link_id, i;

	if (!wdev || !pAd || !bcn_ie_list ||
		!VALID_UCAST_ENTRY_WCID(pAd, wcid))
		return ret;

	pEntry = entry_get(pAd, wcid);
	if (IS_APCLI_DISABLE_MLO(wdev) || !pEntry->mlo.mlo_en)
		return ret;
	...
	if (!mld->peer_mld.valid || mld->valid_link_num <= 1) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
			"mld check fial, ret=%d\n", ret);
		return ret;
	}
	at2lm_info = &mld->peer_mld.at2lm_contract;
	t2l_info = &bcn_ie_list->t2lm_info;

	link_id = pEntry->mlo.link_info.link_id;
	if (link_id >= MLD_LINK_MAX) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
			"mld link check fail, ret=%d\n", ret);
		return ret;
	}
	if (!mld->peer_mld.single_link[link_id].active) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
			"mld link active check fail, ret=%d\n", ret);
		return ret;
	}

	/* step1 update link at2lm status */
	link_at2lm_info = &mld->peer_mld.at2lm_contract.at2lm_link_info[link_id];
	if (t2l_info->state && HAS_EHT_ML_T2LM_EXIST(bcn_ie_list->cmm_ies.ie_exists)) {
		link_at2lm_info->at2lm_ie_exist = TRUE;
		link_at2lm_info->dir = t2l_info->ctrl_dir;
		NdisMoveMemory(link_at2lm_info->tid_map, t2l_info->tid_map, MLD_LINK_MAX);
		if (bcn_ie_list->DtimCount == 0)
			link_at2lm_info->dtim_bcn = TRUE;
		else
			link_at2lm_info->dtim_bcn = FALSE;
		link_at2lm_info->bcn_interval = bcn_ie_list->BeaconPeriod;

		for (i = 0; i < MLD_LINK_MAX; i++) {  /*check disable link*/
			if (((t2l_info->tid_map[i] & 0xff) == 0) &&
				(i == link_id)) {
				link_at2lm_info->link_disabled = TRUE;
				break;
			}
		}
		link_at2lm_info->mst_tsf = t2l_info->mst_tsf;
		/*mapping switch time value bit10-bit25 of timestamp*/
		link_at2lm_info->bcn_tsf = (bcn_ie_list->TimeStamp.u.LowPart >> 10) & 0XFFFF;
		link_at2lm_info->ed_tsf = t2l_info->ed_tsf;
		if (t2l_info->ctrl_mst && t2l_info->ctrl_ed)
			link_at2lm_info->at2lm_state = AT2LM_STE_MST;
		else if (!t2l_info->ctrl_mst && t2l_info->ctrl_ed)
			link_at2lm_info->at2lm_state = AT2LM_STE_ED;
		else
			link_at2lm_info->at2lm_state = AT2LM_STE_NONE;
	} else {
		link_at2lm_info->at2lm_ie_exist = FALSE;
		link_at2lm_info->at2lm_state = AT2LM_STE_NONE;
	}

	/*step2 sanity check*/
	if (at2lm_mld_sta_sanity_check(mld, bcn_ie_list)) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_T2LM, DBG_LVL_INFO,
			"sanity check fial, ret=%d\n", ret);
		return ret;
	}

	/*step3 update to mld at2lm state: NONE --> MST --> ED*/
	mld_at2lm_info = &mld->peer_mld.at2lm_contract;
	if (link_at2lm_info->link_disabled) {
		if ((link_at2lm_info->at2lm_state == AT2LM_STE_MST) &&
			mld_at2lm_info->state == AT2LM_STE_NONE)
			mld_at2lm_info->state = AT2LM_STE_MST;
	}

	/*step4 apcli action: check mld state and update t2lm to fw*/
	mutex_lock(&mld_at2lm_info->op_mutex);
	switch (mld_at2lm_info->state) {
	case AT2LM_STE_MST:
		at2lm_mld_sta_mst_handle(mld);
		break;
	case AT2LM_STE_ED:
		at2lm_mld_sta_ed_handle(mld);
		break;
	default:
		break;
	}
	mutex_unlock(&mld_at2lm_info->op_mutex);
