...
INT twt_ctrl_itwt_dump(struct wifi_dev *wdev)
{
	...
	NdisAcquireSpinLock(&twt_ctrl->twt_rec_lock);
	for (sch_link_idx = 0; sch_link_idx < SCH_LINK_NUM; sch_link_idx++) {
		if (twt_ctrl->twt_link[sch_link_idx].Next != NULL && twt_ctrl->twt_link[sch_link_idx].Prev != NULL) {
	...
	OUT BOOLEAN *bNeedBcnUpdate)
{
	 ...
	 */
	for (sch_link_idx = 0; sch_link_idx < SCH_LINK_NUM; sch_link_idx++) {
		DlListForEach(temp_twt_node, &twt_ctrl->twt_link[sch_link_idx],
			      struct twt_link_node, list) {
			curr_twt_node = temp_twt_node;
			if (!curr_twt_node) {
				MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_TWT, DBG_LVL_ERROR,
					"twt_entry=NULL, please check\n");
			...
			if ((curr_twt_node->type == TWT_TYPE_BTWT) &&
				...
				twt_agrt_cmd_set(agrt_para, curr_twt_node,
						 TWT_AGRT_CTRL_MODIFY, CMD_TSF_TYPE_BTWT);
#ifdef DOT11_EHT_BE
				if (IS_RTWT_ID(curr_twt_node) &&
	...
	for (sch_link_idx = 0; sch_link_idx < SCH_LINK_NUM; sch_link_idx++) {
		DlListForEach(temp_twt_node, &twt_ctrl->twt_link[sch_link_idx],
			...
			if ((curr_twt_node->type == TWT_TYPE_BTWT) &&
			    ...
			    (GET_BTWT_ID(curr_twt_node) == btwt_id) &&
			    (curr_twt_node->grp_member_cnt > 0)) {
				os_move_mem(sta_list, curr_twt_node->sta_list,
					    sizeof(UINT16) * TWT_HW_BTWT_MAX_MEMBER_CNT);
				os_zero_mem(curr_twt_node->sta_list,
					    sizeof(UINT16) * TWT_HW_BTWT_MAX_MEMBER_CNT);
	...
	IN struct _MLME_QUEUE_ELEM *elem)
{
	...
	if (nego_type == TWT_CTRL_NEGO_TYPE_ITWT) {
		...
		if (!TWT_SUPPORT_ITWT(wlan_config_get_he_twt_support(wdev))) {
			MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_TWT, DBG_LVL_INFO,
					"wdev(type=%d,fun_idx=%d,wdev_idx=%d) not support itwt,return\n",
	...
	if (nego_type == TWT_CTRL_NEGO_TYPE_ITWT) {
		...
		if (!MAC_ADDR_EQUAL(msg->peer_addr, ZERO_MAC_ADDR)) {
			...
			for (i = 1; i < TWT_BTWT_ID_NUM; i++) {
				...
				if (twt_handle_one_peer_with_btwt_id_leave(wdev, entry, i,
														&agrt_para, &bNeedBcnUpdate))
					mt_asic_twt_agrt_update(wdev, &agrt_para);
			...
			for (i = 1; i < TWT_BTWT_ID_NUM; i++) {
				...
				twt_handle_all_peers_in_btwt_id_leave(wdev, i, sta_list, &sta_cnt,
													&agrt_para, &bNeedBcnUpdate);
	...
	UINT8 btwt_id)
{
	...
	/* handle twt_node, twt_link_lsit */
	twt_handle_all_peers_in_btwt_id_leave(wdev, btwt_id, sta_list, &sta_cnt,
										&agrt_para, NULL);
	...
	if ((sta_cnt > 0) && (btwt_id > 0)) {
		...
		for (i = 0; i < sta_cnt; i++) {
			...
			entry = entry_get(ad, wcid);
			if (!GET_PEER_JOIN_BTWT_ID(entry, btwt_id))
				continue;
			...
			btwt_frame->category = CATEGORY_UNPROT_S1G;
			btwt_frame->s1g_action = CATE_S1G_ACTION_TWT_TEARDOWN;
			btwt_frame->twt_flow |= SET_BTWT_FLOW_BTWT_ID(btwt_id) |
				SET_BTWT_FLOW_NEGO_TYPE(TWT_CTRL_NEGO_TYPE_BTWT_MBR_MGMT) |
	...
	twt_release_btwt_node(wdev, btwt_id);
	...
	UINT32 ie_type)
{
	...
	pParaBufNext = (UCHAR *)&btwt_element->btwt_para[0];
		...
		DlListForEach(temp_twt_node, &twt_ctrl->twt_link[sch_link_idx], struct twt_link_node, list) {
			...
			if ((curr_twt_node->type == TWT_TYPE_BTWT) &&
				...
				if (wlan_config_get_eht_restricted_twt(wdev) &&
					...
					if (curr_twt_node->grp_member_cnt > 0)
						btwt_para_fix->btwt_info |= SET_BTWT_INFO_RTWT_SCHEDULE_INFO(1);
					else
						btwt_para_fix->btwt_info |= SET_BTWT_INFO_RTWT_SCHEDULE_INFO(0);
				}
#endif /* DOT11_EHT_BE */
	...
	struct btwt_para_set *btwt_para_out)
{
	...
	if (!join) {
		...
		/* Don't care persistence value from peer's request */
		//btwt_ctrl_para.persistence = GET_BTWT_INFO_BTWT_P(btwt_para_fix_in->btwt_info);
		btwt_ctrl_para.persistence = 255;
...
out:
	...
	btwt_para_fix_out->req_type |= (join ? SET_TWT_RT_SETUP_CMD(TWT_SETUP_CMD_ACCEPT) :
										SET_TWT_RT_SETUP_CMD(TWT_SETUP_CMD_REJECT));
	btwt_para_fix_out->btwt_info |= SET_BTWT_INFO_BTWT_P(255);
}
#endif /* DOT11_EHT_BE */
