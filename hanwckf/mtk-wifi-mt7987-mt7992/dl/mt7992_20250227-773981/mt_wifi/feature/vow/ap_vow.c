...
UINT8 vow_infra_bwcg_mapping(PRTMP_ADAPTER pad, MAC_TABLE_ENTRY *pEntry)
{
	UINT_8 new_group_id = 0;

	new_group_id = pad->max_bssgroup_num - pEntry->wdev->func_idx - 1;

	return new_group_id;
}

void vow_set_bwcg_for_sta(IN  PRTMP_ADAPTER pad, unsigned short sta_id, unsigned char group_id)
{
	...
	unsigned char bwcg_group_id;
	unsigned int ret = 0;
	unsigned char band_idx = 0;
	PRTMP_ADAPTER pTargetAd = NULL;
	/* default set to each station with its correspond bwcg when sta_id == 0 */
	start_idx = 0;
	...
	for (idx = start_idx; idx <= end_idx; idx++) {
		...
		pTargetAd = physical_device_get_mac_adapter_by_band(pad->physical_dev, band_idx);
		...
		if (group_id == 0) {
			...
			bwcg_group_id = pTargetAd->bss_group.bw_group_idx[pEntry->wdev->func_idx];

			if ((pEntry->wdev->wdev_type == WDEV_TYPE_STA) && (IS_ENTRY_PEER_AP(pEntry) || IS_ENTRY_REPEATER(pEntry)))
				bwcg_group_id = vow_infra_bwcg_mapping(pad, pEntry);
		}
		/* set group for station */
		pTargetAd->vow_sta_cfg[idx].bw_group = bwcg_group_id;

		MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
			"%s:band_idx:%u,wcid:%u,bw_group:%u,func_idx:%u\n", __func__, band_idx, idx, bwcg_group_id, pEntry->wdev->func_idx);

#ifdef WIFI_UNIFIED_COMMAND
		if (pChipCap->uni_cmd_support) {
			/* update station bitmap */
			ret = uni_cmd_vow_set_sta(pTargetAd, idx, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
		} else
#endif /* WIFI_UNIFIED_COMMAND */
		{
			ret = vow_set_sta(pTargetAd, idx, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
		}
...
void vow_variable_reset(PRTMP_ADAPTER pAd)
{
	...
	/* VOW control */
	pAd->vow_at_est.at_monitor_period = 100;
	pAd->vow_cfg.en_bw_ctrl = FALSE;

	if (ph_dev) {
		UINT_32 idx = hc_get_hw_band_idx(pAd);
...
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support)
				ret = uni_cmd_vow_set_at_estimator_group(pad,
						ENUM_AT_PROC_EST_GROUP_RATIO_CTRL, i);
			else
#endif /* WIFI_UNIFIED_COMMAND */
				ret = vow_set_at_estimator_group(pad,
						ENUM_AT_PROC_EST_GROUP_RATIO_CTRL, i);
		}
	}
#ifdef WIFI_UNIFIED_COMMAND
...
VOID vow_mbss_init(PRTMP_ADAPTER pad, struct wifi_dev *wdev)
{
	...
	if (!pad || !wdev) {
		MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR, "%s:NULL input pointer.\n", __func__);
		return;
	}
	...
	if (pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG) {
		...
		if (wdev->wdev_idx)
			pad->bss_group.group_idx[wdev->func_idx] = (UINT_8)(HW_BSSID_MAX + (wdev->OmacIdx - 1) % MAX_BEACON_NUM);
		else
			pad->bss_group.group_idx[wdev->func_idx] = wdev->OmacIdx % HW_BSSID_MAX;

		if (wdev->wdev_idx)
			pad->bss_group.bw_group_idx[wdev->func_idx] = (wdev->OmacIdx) % VOW_MAX_GROUP_NUM;
		else
			pad->bss_group.bw_group_idx[wdev->func_idx] = 0;
	} else {
		...
		pad->bss_group.bw_group_idx[wdev->func_idx] = pad->bss_group.group_idx[wdev->func_idx];
	}
	...
	if (wdev->wdev_type != WDEV_TYPE_REPEATER) {
		...
		vow_set_client(pad, pad->bss_group.group_idx[wdev->func_idx],
...
VOID vow_mbss_grp_band_map(PRTMP_ADAPTER pad, struct wifi_dev *wdev)
{
	...
	if (pad->vow_gen.VOW_GEN >= VOW_GEN_BESRA) {
		MTWF_PRINT("%s not support for VOW_GEN:%d\n",
				__func__, pad->vow_gen.VOW_GEN);
		return; /* not support */
	}
#ifdef WIFI_UNIFIED_COMMAND
	pChipCap = hc_get_chip_cap(pad->hdev_ctrl);
...
UINT16 vow_convert_rate_token(PRTMP_ADAPTER pad, UINT8 type, UINT8 group_id)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
			 "%s: group %u, period %dus, rate %u, token %u\n", __func__, group_id, period, rate, token);
	return token;
}
...
UINT16 vow_convert_airtime_token(PRTMP_ADAPTER pad, UINT8 type, UINT8 group_id)
{
	...
	*/
	...
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
			 "%s: group %u, period %dus, ratio %u, available time %u, token %u\n", __func__, group_id, period, ratio, atime, token);
	return token;
}
...
VOID vow_set_client(PRTMP_ADAPTER pad, UINT8 group, UINT8 bw_group, UINT16 sta_id, UINT8 wmm_id)
{
	BOOLEAN ret;
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP *pChipCap = hc_get_chip_cap(pad->hdev_ctrl);
	...
	pad->vow_sta_cfg[sta_id].bw_group = bw_group;
	pad->vow_sta_cfg[sta_id].wmm_idx = wmm_id;
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support) {
		/* update station bitmap */
		/* don't change command sequence - STA_BSS_GROUP will refer to STA_ALL's old setting */
		ret = uni_cmd_vow_set_sta(pad, sta_id, ENUM_VOW_DRR_CTRL_FIELD_STA_ALL);
		ret = uni_cmd_vow_set_sta(pad, sta_id, ENUM_VOW_DRR_CTRL_FIELD_STA_BSS_GROUP);
		if (pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG)
			ret = uni_cmd_vow_set_sta(pad, sta_id, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
...
#endif /* WIFI_UNIFIED_COMMAND */
	{
		...
		if (pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG)
			ret = vow_set_sta(pad, sta_id, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
		/* set station pause status */
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 1) && (IS_WCID_VALID(pad, sta))) {
			...
			pad->vow_sta_cfg[sta].bw_group = val;
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support) {
				if (pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG) {
					pad->vow_sta_cfg[sta].bw_group = val;
					ret = uni_cmd_vow_set_sta(pad, sta, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
				} else {
				...
				if (pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG) {
					pad->vow_sta_cfg[sta].bw_group = val;
					ret = vow_set_sta(pad, sta, ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP);
				} else {
					pad->vow_sta_cfg[sta].group = val;
					pad->vow_sta_cfg[sta].bw_group = val;
