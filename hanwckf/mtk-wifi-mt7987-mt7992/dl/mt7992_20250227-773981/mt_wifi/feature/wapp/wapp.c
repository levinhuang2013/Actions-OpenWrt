...
#ifdef EM_PLUS_SUPPORT
				if (((Get_My_Tx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US) > 255)
					radio_metric->cu_tx = 255;
				else
#endif
					radio_metric->cu_tx =
						(Get_My_Tx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
#ifdef EM_PLUS_SUPPORT
				if (((Get_My_Rx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US) > 255)
					radio_metric->cu_rx = 255;
				else
#endif
					radio_metric->cu_rx =
					(Get_My_Rx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
#ifdef EM_PLUS_SUPPORT
				if (((Get_OBSS_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US) > 255)
					radio_metric->cu_other = 255;
				else
#endif
					radio_metric->cu_other =
					(Get_OBSS_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
#ifdef EM_PLUS_SUPPORT
				if (((Get_EDCCA_Time(pAd, band_idx)*255)/ONE_SEC_2_US) > 255)
					radio_metric->edcca = 255;
				else
#endif
					radio_metric->edcca =
						(Get_EDCCA_Time(pAd, band_idx)*255)/ONE_SEC_2_US;

#ifdef EM_PLUS_SUPPORT
				sum = radio_metric->cu_tx + radio_metric->cu_rx + radio_metric->cu_other + radio_metric->edcca;
				if (sum > 255) {
					radio_metric->edcca = radio_metric->cu_tx + radio_metric->cu_rx
						+ radio_metric->cu_other > 255 ? 0 :
						(255 - radio_metric->cu_tx + radio_metric->cu_rx + radio_metric->cu_other);
				}
#endif
				wapp_send_wapp_qry_rsp(pAd, &event);
			}
...
INT wapp_send_ssid_change_rsp(
	PRTMP_ADAPTER pAd,
	struct wifi_dev *wdev,
	u8 *ssid, UCHAR ssid_len)
{
	struct wapp_event event;

	if (wdev->if_dev) {
		event.event_id = WAPP_SSID_CHANGE;
		event.ifindex = RtmpOsGetNetIfIndex(wdev->if_dev);
		event.data.SSID.SsidLength = ssid_len;
		os_move_mem(event.data.SSID.Ssid, ssid, ssid_len);
		wapp_send_wapp_qry_rsp(pAd, &event);
	}
	return 0;
}
	...
	USHORT reason_code)
{
	...
	if (wdev && wdev->if_dev && bssid && sta_mac_addr) {
