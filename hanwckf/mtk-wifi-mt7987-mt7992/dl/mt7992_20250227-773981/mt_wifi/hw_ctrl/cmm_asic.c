...
INT AsicSetTxCmdEDCA(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT8 idx, PEDCA_PARM pEdcaParm)
{
	struct _RTMP_ARCH_OP *arch_ops = hc_get_arch_ops(pAd->hdev_ctrl);
	...
	if (pChipCap->uni_cmd_support) {
		if (arch_ops->archUniCmdSetTxCmdEDCA)
			return arch_ops->archUniCmdSetTxCmdEDCA(pAd, wdev, idx, pEdcaParm);
	}
#endif /* WIFI_UNIFIED_COMMAND */
...
INT AsicSetPerStaWmm(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT16 wcid, UINT8 ac, UINT8 idx)
{
	struct _RTMP_ARCH_OP *arch_ops = hc_get_arch_ops(pAd->hdev_ctrl);
	...
	if (pChipCap->uni_cmd_support) {
		if (arch_ops->archUniCmdSetPerStaWMM)
			return arch_ops->archUniCmdSetPerStaWMM(pAd, wdev, wcid, ac, idx);
	}
#endif /* WIFI_UNIFIED_COMMAND */
...
static UCHAR *phy_band_str[] = {"2.4Ghz", "5Ghz", "6Ghz"};
char *get_band_str(UCHAR band_idx)
{
	if (band_idx <= WIFI_CH_BAND_6G)
		return phy_band_str[band_idx];
	else
		return "N/A";
}
	...
	if (IS_HIF_TYPE(ad, HIF_MT)) {
		...
		if (cap->uni_cmd_support) {
			...
			if (ret == NDIS_STATUS_SUCCESS) {
				mt_rcu_read_lock();
				tmp_rsp_buf = rcu_dereference(agrt_mgmt);
				mt_rcu_read_unlock();
				if (tmp_rsp_buf)
					os_move_mem((UINT8 *)tmp_rsp_buf,
						(UINT8 *)&_agrt_mgmt,
...
BOOLEAN asic_get_wdma_related_tx_bus_info(struct _RTMP_ADAPTER *ad, u32 wdma_idx)
{
	RTMP_ARCH_OP *arch_ops = hc_get_arch_ops(ad->hdev_ctrl);

	if (arch_ops->get_wdma_related_tx_bus_info)
		return arch_ops->get_wdma_related_tx_bus_info(wdma_idx);
