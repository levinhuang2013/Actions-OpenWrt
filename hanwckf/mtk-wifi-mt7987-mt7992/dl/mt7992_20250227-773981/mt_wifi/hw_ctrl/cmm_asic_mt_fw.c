	...
	IN struct _ASIC_SEC_INFO *pInfo)
{
	...
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, pInfo->PeerAddr, NULL, "Key Install/Remove Start", __func__);
...
#ifdef ROAM_CALIB_SUPPORT
			if (pAd->RoamCalibEnable)
				RoamCalibData(pAd, pInfo->PeerAddr, NULL, "Key Install/Remove Completed", __func__);
#endif /* ROAM_CALIB_SUPPORT */
			return;
		}
#endif /* WIFI_UNIFIED_COMMAND */
	...
	UINT32 EdcaValue)
{
	...
	os_zero_mem(&EdcaParam, sizeof(MT_EDCA_CTRL_T));
	EdcaParam.u8EnableFeature = 1 << UNI_CMD_EDCA_AC_PARM;
	EdcaParam.ucTotalNum = 1;
	EdcaParam.ucAction = EDCA_ACT_SET;
	pAcParam = &EdcaParam.rAcParam[0];
...
INT MtAsicUniCmdSetTxCmdEDCA(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT8 idx, PEDCA_PARM pEdcaParm)
{
	MT_EDCA_CTRL_T EdcaParam;
	P_TX_AC_PARAM_T pAcParam;
	...
	if ((pEdcaParm != NULL) && (pEdcaParm->bValid != FALSE)) {
		EdcaParam.u8EnableFeature = 1 << UNI_CMD_EDCA_TXCMD_EDCA_TBL;
		EdcaParam.ucEdcaTblIdx = idx;

		pAcParam = &EdcaParam.rAcParam[0];
		pAcParam->ucAifs = pEdcaParm->Aifsn[0];
		pAcParam->ucWinMin = pEdcaParm->Cwmin[0];
		pAcParam->u2WinMax = pEdcaParm->Cwmax[0];
	}

	return MtUniCmdEdcaParameterSet(pAd, wdev, &EdcaParam);
}

INT MtAsicUniCmdSetPerStaWMM(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT16 wcid, UINT8 ac, UINT8 idx)
{
	MT_EDCA_CTRL_T EdcaParam;

	os_zero_mem(&EdcaParam, sizeof(MT_EDCA_CTRL_T));

	EdcaParam.u8EnableFeature = 1 << UNI_CMD_EDCA_SU_DL_TXCMD;
	EdcaParam.u2WlanIdx = wcid;
	EdcaParam.ucAc = ac;
	EdcaParam.ucEdcaTblIdx = idx;
