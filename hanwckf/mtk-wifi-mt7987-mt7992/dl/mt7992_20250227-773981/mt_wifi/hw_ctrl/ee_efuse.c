	...
	for (offset = 0; offset < length; offset += EFUSE_BLOCK_SIZE) {
		if (memcmp(ptr, all_ff, EFUSE_BLOCK_SIZE)) {
			...
			for (index = 0; index < EFUSE_BLOCK_SIZE; index++) {
				if (ops->write_efuse_field_valide) {
					fIsValid = ops->write_efuse_field_valide(pAd, (offset + index));
					if (!fIsValid)
						MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
							"offset: %x is invalid now\n", (offset + index));
				}

				if (fIsValid && (block[index] != ptr[index]))
					NeedWrite = TRUE;
				else
					continue;
...
INT set_eFuseWrite_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (!chip_ops->write_efuse_field_valide(pAd, offset)) {
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_ERROR,
		"offset: %x is invalid now\n", offset);
		return FALSE;
	}
