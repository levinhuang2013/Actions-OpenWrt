...
int hc_get_tx_sw_token_nums_all_band(PRTMP_ADAPTER ad, u16 *max_tk, u8 array_size)
{
	struct hdev_ctrl *ctrl = NULL;
	...
	ctrl = (struct hdev_ctrl *)ad->hdev_ctrl;
	if (ctrl->hdev_ops && ctrl->hdev_ops->get_tx_sw_token_nums_all_band)
		return ctrl->hdev_ops->get_tx_sw_token_nums_all_band(ad, max_tk, array_size);
	else
		return -1;
}

void hc_get_tx_sw_used_token_by_band(PRTMP_ADAPTER ad, u16 *used_tk)
{
	struct hdev_ctrl *ctrl = NULL;
	...
	if (!ad)
		return;
		 ctrl = (struct hdev_ctrl *)ad->hdev_ctrl;
	if (ctrl->hdev_ops && ctrl->hdev_ops->get_tx_sw_used_token_by_band)
		ctrl->hdev_ops->get_tx_sw_used_token_by_band(ad, used_tk);
	else
		return;
}
...
int hc_set_particular_to_host(PRTMP_ADAPTER ad, bool enable)
{
	struct hdev_ctrl *ctrl = NULL;
	...
	ctrl = (struct hdev_ctrl *)ad->hdev_ctrl;
	if (ctrl->hdev_ops && ctrl->hdev_ops->set_particular_to_host)
		return ctrl->hdev_ops->set_particular_to_host(ad, enable);
	else
		return 0;
}
...
VOID HcShowChCtrlInfo(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UCHAR BandIdx, ChIdx;
	...
	if (wdev == NULL)
		...
		if (pChCtrl->ChListNum == 0) {
			ret = snprintf(pos, end-pos,
				"\x1b[1;33mBandIdx = %d\x1b[m, ChannelListNum = %d\n ",
			...
			ret = snprintf(pos, end-pos,
				"\x1b[1;33mBandIdx = %d\x1b[m\n"
				"ChannelListNum = %d\n"
				"ChGrpABandEn = %d\n"
				"Channel list information:\n"
				"Is40BWforbid = %d\n",
				BandIdx, pChCtrl->ChListNum, pChCtrl->ChGrpABandEn, pChCtrl->ch_cfg.bw40_forbid);
			if (os_snprintf_error(end-pos, ret))
				return;
			pos += ret;
...
struct Ch_Op_Info *hc_get_ch_op_info(void *hdev_ctrl)
{
	struct hdev_ctrl *ctrl = hdev_ctrl;
...
UCHAR hc_init_ACSChCtrl(RTMP_ADAPTER *pAd)
{
	...
	pAutoChCtrl->AutoChSelCtrl.ScanChIdx = DEFAULT_ACS_SCAN_CH_IDX;
	pAutoChCtrl->AutoChSelCtrl.ChListNum = DEFAULT_ACS_CH_LIST_NUM;
	pAutoChCtrl->AutoChSelCtrl.ACSChStat = DEFAULT_ACS_CH_STAT;
	pAutoChCtrl->AutoChSelCtrl.RestoreDwell = DEFAULT_ACS_RESTORE_DWELL;
	pAutoChCtrl->AutoChSelCtrl.ScanningDwell = DEFAULT_ACS_SCANNING_DWELL;
	pAutoChCtrl->AutoChSelCtrl.ScanningChNum = DEFAULT_ACS_SCANNING_CH_NUM;
	pAutoChCtrl->AutoChSelCtrl.DataRateWeight = DEFAULT_ACS_DATA_RATE_WEIGHT;
