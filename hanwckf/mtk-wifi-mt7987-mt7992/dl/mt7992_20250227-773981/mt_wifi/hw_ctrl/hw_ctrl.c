...
static NTSTATUS HwCtrlHandleUpdateBeacon(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	...
	if ((wdev != NULL) && (wdev->BcnBPCC_Init) &&
		((IS_BCN_CRIT_UPD(UpdateReason)) || (BCN_REASON_EQUAL(UpdateReason, BCN_UPDATE_IE_CHG)) || (BCN_REASON_EQUAL(UpdateReason, BCN_UPDATE_CSA))))
...
static NTSTATUS HwCtrlSetStaDWRR(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	...
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_CMD_CTRL, DBG_LVL_DEBUG,
		"group %d, bw_group %d, staid %d\n", pVoW->GroupIdx, pVoW->BW_GroupIdx, pVoW->StaIdx);
	vow_set_client(pAd, pVoW->GroupIdx, pVoW->BW_GroupIdx, pVoW->StaIdx, pVoW->WmmIdx);
	return NDIS_STATUS_SUCCESS;
}
#endif /* CONFIG_AP_SUPPORT */
#ifdef VOW_SUPPORT
...
static NTSTATUS HwCtrlGetTemperature(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	...
	VOID *tmp_rsp_buf = NULL;

	/*ActionIdx 0 means get temperature*/
	MtCmdGetThermalSensorResult(pAd, 0, uBandIdx ,&temperature);
	mt_rcu_read_lock();
	tmp_rsp_buf = rcu_dereference(CMDQelmt->RspBuffer);
	mt_rcu_read_unlock();
	if (tmp_rsp_buf == NULL)
		return NDIS_STATUS_FAILURE;

	os_move_mem(tmp_rsp_buf, &temperature, CMDQelmt->RspBufferLen);
	return NDIS_STATUS_SUCCESS;
}
...
static NTSTATUS HwCtrlGetTsf(RTMP_ADAPTER *ad, HwCmdQElmt *CMDQelmt)
{
	...
	if (ret == TRUE)
		os_move_mem(tmp_rsp_buf, &TsfResult, CMDQelmt->RspBufferLen);
	return ret;
}
...
static NTSTATUS HwCtrlGetTsfDiff(RTMP_ADAPTER *ad, HwCmdQElmt *CMDQelmt)
{
	...
	if (tmp_rsp_buf == NULL)
		return NDIS_STATUS_FAILURE;
	os_move_mem(tmp_rsp_buf, &TsfDiffResult, CMDQelmt->RspBufferLen);
...
static NTSTATUS HwCtrlGetAt2lmRes(RTMP_ADAPTER *ad, HwCmdQElmt *CMDQelmt)
{
	...
	if (tmp_rsp_buf == NULL)
		return NDIS_STATUS_FAILURE;
	os_move_mem(tmp_rsp_buf, &rsp, CMDQelmt->RspBufferLen);
...
static NTSTATUS hw_ctrl_flush_ps_sta_ple(struct _RTMP_ADAPTER *ad, HwCmdQElmt *CMDQelmt)
{
	struct _FLUSH_PS_STA_PLE *pFlushPsStaPle = (struct _FLUSH_PS_STA_PLE *)CMDQelmt->buffer;

	return MTUniCmdFlushPsStaPle(ad, pFlushPsStaPle->u2PktThres, pFlushPsStaPle->ucBandMap);
}

static NTSTATUS hw_ctrl_flush_ps_sta_ple_num(struct _RTMP_ADAPTER *ad, HwCmdQElmt *CMDQelmt)
{
	struct _FLUSH_PS_STA_PLE *pFlushPsStaPle = (struct _FLUSH_PS_STA_PLE *)CMDQelmt->buffer;

	return MTUniCmdFlushPsStaPleNum(ad, pFlushPsStaPle->u2PktThres, pFlushPsStaPle->ucBandMap);
}
...
static HW_CMD_TABLE_T HwCmdPsTable[] = {
	...
	{HWCMD_ID_FLUSH_PS_STA_PLE, hw_ctrl_flush_ps_sta_ple, 0},
	{HWCMD_ID_FLUSH_PS_STA_PLE_NUM, hw_ctrl_flush_ps_sta_ple_num, 0},
#endif /* WIFI_TWT_SUPPORT */
#endif /* DOT11_HE_AX */
	{HWCMD_ID_END, NULL, 0}
};
