...
VOID RTMP_GET_TEMPERATURE(RTMP_ADAPTER *pAd, UINT8 ucDbdcIdx, UINT32 *pTemperature)
{
	...
	HW_CTRL_TXD_RSP(pAd, sizeof(UINT32), pTemperature, MCU_CMD_TIMEOUT * 1000, HwCtrlTxd);
	ret = HwCtrlEnqueueCmd(pAd, HwCtrlTxd);
}
	...
	UINT32 *current_tsf)
{
	...
	HW_CTRL_TXD_RSP(ad, sizeof(UINT32)*2, current_tsf, MCU_CMD_TIMEOUT * 1000, HwCtrlTxd);
...
VOID RTMP_FLUSH_PS_STA_PLE(PRTMP_ADAPTER pAd, UINT16 pkt_thres, UINT8 band_map)
{
	struct _FLUSH_PS_STA_PLE flush_ps_sta_ple  = {0};
	UINT32 ret;

	flush_ps_sta_ple.u2PktThres = pkt_thres;
	flush_ps_sta_ple.ucBandMap = band_map;

	ret = HW_CTRL_BASIC_ENQ(pAd, HWCMD_TYPE_PS, HWCMD_ID_FLUSH_PS_STA_PLE, sizeof(struct _FLUSH_PS_STA_PLE),
		&flush_ps_sta_ple);
}

VOID RTMP_FLUSH_PS_STA_PLE_NUM(PRTMP_ADAPTER pAd, UINT16 flush_num, UINT8 band_map)
{
	struct _FLUSH_PS_STA_PLE flush_ps_sta_ple  = {0};
	UINT32 ret;

	flush_ps_sta_ple.u2PktThres = flush_num;
	flush_ps_sta_ple.ucBandMap = band_map;
	ret = HW_CTRL_BASIC_ENQ(pAd, HWCMD_TYPE_PS, HWCMD_ID_FLUSH_PS_STA_PLE_NUM, sizeof(struct _FLUSH_PS_STA_PLE),
			&flush_ps_sta_ple);
}
...
)
{
	...
	HW_CTRL_TXD_RSP(ad, 0, NULL, MCU_CMD_TIMEOUT * 1000, HwCtrlTxd);
...
VOID RTMP_SET_STA_DWRR(PRTMP_ADAPTER pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	VoW.BW_GroupIdx = pAd->bss_group.bw_group_idx[pEntry->wdev->func_idx];

	if ((pAd->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG) || (pAd->vow_cfg.en_bw_ctrl))
		MTWF_PRINT("%s::[vow]GroupID:%u,BW_GroupIdx:%u,func_idx:%u, func_tb_idx:%u,wcid:%u\n", __func__,
				VoW.GroupIdx, VoW.BW_GroupIdx, pEntry->wdev->func_idx, pEntry->func_tb_idx, pEntry->wcid);
