...
hwifi_start(struct _RTMP_ADAPTER *ad)
{
	...
	RTMP_STRING fw_log_arg_tmp[10] = {0};
#endif /* FW_LOG_DUMP */
	...
	if (strlen(fw_log_arg)) {
		strlcpy(fw_log_arg_tmp, fw_log_arg, strlen(fw_log_arg) + 1);
		set_fw_log(ad, fw_log_arg_tmp);
	}
#endif /* FW_LOG_DUMP */
...
static int hwifi_update_wdev(struct wifi_dev *wdev)
{
	...
	struct Ch_Op_Info *ch_op_info = NULL;

	MTWF_DBG(ad, DBG_CAT_HIF, CATHIF_HWIFI, DBG_LVL_WARN, "(caller:%pS), wdev(%d).\n",
			 OS_TRACE, wdev->wdev_idx);

	if (ad->hdev_ctrl == NULL) {
		MTWF_DBG(ad, DBG_CAT_HIF, CATHIF_HWIFI, DBG_LVL_WARN,
			"hdev_ctrl is NULL!!\n");
		return -ENOMEM;
	}
	ch_op_info = hc_get_ch_op_info(ad->hdev_ctrl);
	if (ch_op_info == NULL) {
		MTWF_DBG(ad, DBG_CAT_HIF, CATHIF_HWIFI, DBG_LVL_WARN,
				 "ch_op_info is NULL\n");
		return -ENOMEM;
	...
	/*record target ch/bw*/
	if (ch_op_info->target_ch == 0) {
		ch_op_info->target_ch = wdev->channel;
		ch_op_info->target_bw = wlan_get_target_bw(wdev, ch_op_info->target_ch, wlan_config_get_bw(wdev));
		MTWF_DBG(ad, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"target ch = %d, target bw = %d\n", ch_op_info->target_ch, ch_op_info->target_bw);
	} else {
		MTWF_DBG(ad, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
			"already have target ch = %d, target bw = %d\n", ch_op_info->target_ch, ch_op_info->target_bw);
	}
...
static int
hwifi_get_tx_sw_token_nums_all_band(struct _RTMP_ADAPTER *ad, u16 *max_tk, u8 array_size)
{
	struct mtk_mac_dev *mac_dev = pad_to_mac_dev(ad);
	struct mtk_mac_hw *hw = &mac_dev->hw;

	return mac_dev->ops->get_tx_sw_token_nums_all_band(hw, max_tk, array_size);
}

static void
hwifi_get_tx_sw_used_token_by_band(struct _RTMP_ADAPTER *ad, u16 *used_tk)
{
	struct mtk_mac_dev *mac_dev = pad_to_mac_dev(ad);
	struct mtk_mac_hw *hw = &mac_dev->hw;

	mac_dev->ops->get_tx_sw_used_token_by_band(hw, used_tk);
}
...
static int
hwifi_set_particular_to_host(struct _RTMP_ADAPTER *ad, bool enable)
{
	struct mtk_mac_dev *mac_dev = pad_to_mac_dev(ad);
	struct mtk_mac_hw *hw = &mac_dev->hw;

	return mac_dev->ops->set_particular_to_host(hw, enable);
}
...
hwifi_get_fw_info(struct _RTMP_ADAPTER *pAd, int mcu_type, char *fw_ver, char *build_date, char *fw_ver_long)
{
	...
	if (!fw_ver || !build_date || !fw_ver_long) {
		MTWF_DBG(NULL, DBG_CAT_HIF, CATHIF_HWIFI, DBG_LVL_ERROR,
			"fw_ver is %s, build_date is %s, fw_ver_long is %s",
			fw_ver ? "not null" : "NULL",
			build_date ? "not null" : "NULL",
			fw_ver_long ? "not null" : "NULL");
		return;
	}

	info.type = MAC_MCU_INFO_FW;
	info.fw_info.mcu_type = mcu_type;
	info.fw_info.fw_ver = fw_ver;
	info.fw_info.build_date = build_date;
	info.fw_info.fw_ver_long = fw_ver_long;

	if (mac_dev->ops->get_info)
		mac_dev->ops->get_info(&mac_dev->hw, MAC_INFO_MCU, &info);
}

BOOLEAN
hwifi_get_wdma_related_tx_bus_info(u32 wdma_idx)
{
	struct _RTMP_ADAPTER *pAd_list[MAX_HW_DEV_NUM] = {NULL};
	...
	info.type = MAC_BUS_INFO_TX_DATA_NUM;
	dev_num = physical_device_get_hw_dev_first_up_pad_list(pAd_list);
	...
	for (i = 0; i < dev_num; i++) {
		...
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_DEBUG,
				 "mac adapter: %p\n", pAd);
		mac_dev =  pad_to_mac_dev(pAd);
		if (mac_dev->ops->get_info)
			mac_dev->ops->get_info(&mac_dev->hw, MAC_INFO_BUS, &info);

		if (info.ring_info.ring_num > 0)
			break;
	}

	if (info.ring_info.ring_num == 0) {
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_ERROR,
			 "Can't get wdma ring info\n");
		return FALSE;
	}

	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_SER, DBG_LVL_DEBUG,
			 "Total ring num=%d\n", info.ring_info.ring_num);

	if (info.ring_info.ring_num) {
		os_alloc_mem(NULL,
					(UCHAR **)&info.ring_info.ring,
					 info.ring_info.ring_num * sizeof(struct mtk_mac_ring));

		if (info.ring_info.ring == NULL) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_SER, DBG_LVL_ERROR,
				"ring_info alloc failed!\n");
				return FALSE;
		}
		os_zero_mem(info.ring_info.ring, sizeof(struct mtk_mac_ring));

		/* Get ring info by wdma index */
		info.type = MAC_BUS_INFO_TX_DATA_BY_WDMA_IDX;
		info.wdma_idx = wdma_idx;
		mac_dev->ops->get_info(&mac_dev->hw, MAC_INFO_BUS, &info);

		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_SER, DBG_LVL_DEBUG,
				 "<< wdma_idx:%d - ring num:%d >>\n",
				 wdma_idx, info.ring_info.ring_num);
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_SER, DBG_LVL_DEBUG,
					"%10s %10s %10s %10s %10s %10s\n",
					"Base", "MaxCnt", "MagicCnt", "CIdx", "Didx", "QCnt");
		for (i = 0; i < info.ring_info.ring_num; i++) {
			struct mtk_mac_ring *ring = info.ring_info.ring + i;

			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_SER, DBG_LVL_DEBUG,
					"%10llx %10x %10x %10x %10x %10x\n",
					ring->base,
					ring->max_cnt,
					ring->magic_cnt,
					ring->cidx,
					ring->didx,
					ring->q_cnt);
			if (ring->q_cnt == ring->max_cnt && ring->q_cnt != 0)
				isBusy = TRUE;
		}

	}
	if (info.ring_info.ring)
		os_free_mem(info.ring_info.ring);
...
static struct mtk_hdev_ops hwifi_ops = {
	...
	.set_particular_to_host = hwifi_set_particular_to_host,
	.change_setup_link_sta = hwifi_change_setup_link_sta,
	.get_tx_sw_token_nums_all_band = hwifi_get_tx_sw_token_nums_all_band,
	.get_tx_sw_used_token_by_band = hwifi_get_tx_sw_used_token_by_band,
};
...
mtk_mac_tx_status(struct mtk_mac_hw *hw, struct mtk_tx_status *tx_status)
{
...
#if defined(EAP_STATS_SUPPORT) || defined(GTB_APP_SUPPORT)
...
#if defined(EAP_STATS_SUPPORT) || defined(CFG_RED_SUPPORT) || defined(DABS_QOS) || defined(GTB_APP_SUPPORT)
	if (VALID_UCAST_ENTRY_WCID(ad, tx_status->wcid)) {
		...
		OneSecTxLatencyCal(pEntry, tx_status->mac_latency,
...
#endif
#endif /* EAP_STATS_SUPPORT || GTB_APP_SUPPORT */
