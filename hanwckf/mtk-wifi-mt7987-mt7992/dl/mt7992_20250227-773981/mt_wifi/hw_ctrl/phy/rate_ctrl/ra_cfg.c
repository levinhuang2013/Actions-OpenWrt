...
INT Set_Fixed_Rate_Per_AC_Reg_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	RTMP_STRING * arg)
{
	UINT32 ret;
	INT32 i4Recv = 0;
	UINT32 u4WCID = 0, u4Mode = 0, u4Bw = 0, u4Mcs = 0, u4VhtNss = 0, u4SGI = 0;
	UINT32 u4Preamble = 0, u4STBC = 0, u4LDPC = 0, u4SpeEn = 0, u4HeLtf = 0, u4AC = 0;
	MAC_TABLE_ENTRY *pEntry = NULL;
#ifdef RACTRL_FW_OFFLOAD_SUPPORT
	CMD_STAREC_AUTO_RATE_UPDATE_T rRaParam;
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	...
	i4Recv = sscanf(arg, "%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d",
		&(u4WCID), &(u4Mode), &(u4Bw), &(u4Mcs), &(u4VhtNss), &(u4SGI),
		&(u4Preamble), &(u4STBC), &(u4LDPC), &(u4SpeEn), &(u4HeLtf), &(u4AC));
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_NOTICE,
		"\n\t\tWCID = %d, Mode = %d, BW = %d, MCS = %d, VhtNss = %d, SGI = %d,\n"
		"\t\tPreamble = %d, STBC = %d, LDPC = %d, SpeEn = %d, HeLtf = %d, AC = %d\n",
		u4WCID, u4Mode, u4Bw, u4Mcs, u4VhtNss, u4SGI,
		u4Preamble, u4STBC, u4LDPC, u4SpeEn, u4HeLtf, u4AC);
	...
	if (!VALID_UCAST_ENTRY_WCID(pAd, u4WCID)) {
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_WARN,
			"WCID(%d) exceed MaxUcastEntryNum(%d)!\n",
			u4WCID, GET_MAX_UCAST_NUM(pAd));
		goto Error;
	}

	if (u4Mode != HW_CCK_MODE && u4Mode != HW_OFDM_MODE
		&& u4Mode != HW_HTMIX_MODE && u4Mode != HW_VHT_MODE
		&& u4Mode != HW_HE_SU_MODE && u4Mode != HW_EHT_MU) {
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_WARN,
			"Unknown Mode(%d)!\n", u4Mode);
		goto Error;
	}
	...
	if ((u4Mode == HW_CCK_MODE && u4Mcs > 3) ||
		(u4Mode == HW_OFDM_MODE && u4Mcs > 7) ||
		(u4Mode == HW_HTMIX_MODE && u4Mcs > 32) ||
		(u4Mode == HW_VHT_MODE && u4Mcs > 9) ||
		(u4Mode == HW_HE_SU_MODE && u4Mcs > 11) ||
		(u4Mode == HW_EHT_MU && u4Mcs > 13)) {
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_WARN,
			"Unknown Mode(%d)/MCS(%d)!\n", u4Mode, u4Mcs);
		goto Error;
	}
	...
	u4Preamble = (u4Preamble == 0) ? LONG_PREAMBLE : SHORT_PREAMBLE;
	u4STBC = raStbcSettingCheck(u4STBC, u4Mode, u4Mcs, u4VhtNss, 0, 0);
	u4LDPC = u4LDPC ? (HT_LDPC | VHT_LDPC | HE_LDPC) : 0;
...
#ifdef DOT11_N_SUPPORT
#ifdef DOT11_VHT_AC
	if (u4Mode == MODE_VHT || u4Mode == HW_HE_SU_MODE)
		pEntry->HTPhyMode.field.MCS = (((u4VhtNss - 1) & 0x3) << 4) + u4Mcs;
	else
#endif /* DOT11_VHT_AC */
#endif /* DOT11_N_SUPPORT */
	...
	if (cap->fgRateAdaptFWOffload == TRUE) {
		...
		rRaParam.FixedRateCfg.ShortGI = u4SGI;
		rRaParam.FixedRateCfg.BW = u4Bw;
		rRaParam.FixedRateCfg.ldpc = u4LDPC;
		rRaParam.FixedRateCfg.MCS = u4Mcs;
		rRaParam.FixedRateCfg.VhtNss = u4VhtNss;
		rRaParam.FixedRateCfg.he_ltf = u4HeLtf;
		rRaParam.FixedRateCfg.u1AC = u4AC;
		rRaParam.ucShortPreamble = u4Preamble;
		rRaParam.ucSpeEn = u4SpeEn;
		rRaParam.u4Field = RA_PARAM_FIXED_RATE_AC_REG;
		RAParamUpdate(pAd, pEntry, &rRaParam);
	}
#endif /* RACTRL_FW_OFFLOAD_SUPPORT */
	RTMP_SEM_EVENT_UP(&pAd->AutoRateLock);
...
Error:
	MTWF_PRINT("\niwpriv ra0 set FixedRatePerAcReg=");
	MTWF_PRINT("[WICD]-[Mode]-[BW]-[MCS]-[VhtNss]-[SGI]");
	MTWF_PRINT("[Preamble]-[STBC]-[LDPC]-[SPE_EN]-[HE_LTF]-[AC]\n");
	MTWF_PRINT("[WCID] Wireless Client ID\n");
	MTWF_PRINT("[Mode]CCK=0, OFDM=1, HT=2, VHT=4, HE_SU=8, EHT_SU=15\n");
	MTWF_PRINT("[BW]BW20=0, BW40=1, BW80=2, BW160=3, BW320=4\n");
	MTWF_PRINT("[MCS]CCK=0~4, OFDM=0~7, HT=0~32, VHT=0~9, HE=0~11, EHT=0~13\n");
	MTWF_PRINT("[VhtNss]VHT/HE=1~4, Other=ignore\n");
	MTWF_PRINT("[SGI]HT/VHT:Long=0, Short=7; HE:SGI/MGI/LGI=0/1/2\n");
	...
	MTWF_PRINT("[SPE_EN]Disable=0, Enable=1\n");
	MTWF_PRINT("[HE_LTF]1X=0, 2X=1, 4X=2\n");
	MTWF_PRINT("[AC]AC0~3=0~3\n");

	return FALSE;
}

INT Set_Fixed_Rate_Per_AC_UnReg_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	RTMP_STRING * arg)
{
	UINT32 ret;
	...
	u4WCID = (UINT32)os_str_toul(arg, 0, 10);
	...
	if (cap->fgRateAdaptFWOffload == TRUE) {
		NdisZeroMemory(&rRaParam, sizeof(CMD_STAREC_AUTO_RATE_UPDATE_T));
		rRaParam.u4Field = RA_PARAM_FIXED_RATE_AC_UNREG;
		RAParamUpdate(pAd, pEntry, &rRaParam);
	}
#endif /* RACTRL_FW_OFFLOAD_SUPPORT */
	RTMP_SEM_EVENT_UP(&pAd->AutoRateLock);
...
Error:
	MTWF_PRINT("iwpriv ra0 set FixedRatePerAcUnReg=[WCID]");
	MTWF_PRINT("[WCID] Wireless Client ID\n");
	return FALSE;
}
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			i4Recv = sscanf(arg, "%d-%d", &(u4WCID), &(u4Max_BW));
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
			"WCID = %d, u4Max_BW = %d\n", u4WCID, u4Max_BW);
			...
			operating_bw = (peer_max_bw > u4Max_BW) ? u4Max_BW : peer_max_bw;
			...
			MTWF_PRINT("%s: operating_bw=%d\n", __func__, operating_bw);
			RTMP_SEM_EVENT_UP(&pAd->AutoRateLock);
		} while (0);
	}
