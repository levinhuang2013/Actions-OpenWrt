...
)
{
	...
	if (!IS_VALID_ENTRY(pEntry) || !pEntry->wdev)
		return;
...
)
{
	...
	switch (pRaParam->u4Field) {
#ifdef DOT11_N_SUPPORT
#ifdef DOT11_VHT_AC
	case RA_PARAM_VHT_OPERATING_MODE:
	...
	case RA_PARAM_FIXED_RATE_FALLBACK:
	...
	case RA_PARAM_FIXED_RATE_AC_REG:
		...
		pCmdStaRecAutoRateUpdate->FixedRateCfg.VhtNss = pRaParam->FixedRateCfg.VhtNss;
		pCmdStaRecAutoRateUpdate->FixedRateCfg.he_ltf = pRaParam->FixedRateCfg.he_ltf;
		pCmdStaRecAutoRateUpdate->FixedRateCfg.u1AC = pRaParam->FixedRateCfg.u1AC;
		pCmdStaRecAutoRateUpdate->ucShortPreamble = pRaParam->ucShortPreamble;
		pCmdStaRecAutoRateUpdate->ucSpeEn = pRaParam->ucSpeEn;
		break;

	case RA_PARAM_FIXED_RATE_AC_UNREG:
		pCmdStaRecAutoRateUpdate->u4Field = RA_PARAM_FIXED_RATE_AC_UNREG;
		break;
