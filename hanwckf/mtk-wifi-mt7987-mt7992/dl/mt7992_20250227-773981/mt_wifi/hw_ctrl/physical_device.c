...
u32 physical_device_get_hw_dev_first_up_pad_list(struct _RTMP_ADAPTER **ret_pAd_list)
{
	struct physical_device_node *device_node;
	struct physical_device *device;
	struct mac_ad_node *mac_ad_node;
	...
	OS_SEM_LOCK(&physical_device_list_lock);
	dev_num = physical_device_list.size;
	...
	for (device_node = (struct physical_device_node *)physical_device_list.pHead;
		...
		if (ret_pAd_list) {
			...
			/* check if the mac adapter is valid or not */
			if (mac_ad_node && pAd && i < MAX_HW_DEV_NUM) {
				MTWF_DBG(NULL, DBG_CAT_HIF, CATHIF_HWIFI, DBG_LVL_DEBUG,
						 "device idx=%d mac adapter: %p\n", i, pAd);

				/* stored first up mac adapter */
				ret_pAd_list[i] = pAd;
				i++;
			}
		}
