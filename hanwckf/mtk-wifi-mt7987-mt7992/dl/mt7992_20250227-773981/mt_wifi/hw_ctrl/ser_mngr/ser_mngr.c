	...
	struct _RTMP_ADAPTER *ad)
{
	...
	/* get L1 ser state from ser_cb */
	RTMP_SPIN_LOCK_IRQSAVE(lock, &flags);
	DlListForEach(ser_cb, &_ser_ctrl->ser_cb_list, struct ser_cb_t, list) {
		if (ser_cb->hw_id == hw_id && ser_cb->ser_level == SER_LV_1_0) {
			found = 1;
			ser_state = ser_cb->ser_state;
			break;
		}
...
static struct ser_lv_info serLvInfo[] = {
	{"SER_LV_0_0",  SER_LV_0_0},
	{"SER_LV_0_5",  SER_LV_0_5},
	{"SER_LV_1_0",  SER_LV_1_0},
#ifdef MTK_FE_RESET_RECOVER
	{"FE_RESET",  FE_RESET},
	{"FE_WDMA_RESET",  FE_WDMA_RESET},
#endif
	{"SER_LV_10_0",  SER_LV_10_0},
	{"UNKNOWN",  SER_LV_END}
};
	...
	struct ser_event_cb_t *event_cb)
{
	...
	for (i = 0; i < ARRAY_SIZE(serLvInfo); i++) {
		if (level == serLvInfo[i].level && serLvInfo[i].name != NULL) {
			MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_INFO,
...
bool mtk_check_wifi_busy_stat(u32 wdma_idx)
{
	bool isBusy = false;
	BOOLEAN bBusy = false;
	...
	if (dev_num == 0) {
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_SER, DBG_LVL_ERROR,
			 "dev_num should not be 0\n");
		return false;
	}

	if (pAd_list[0] != NULL) {
		pAd = pAd_list[0];
		bBusy = asic_get_wdma_related_tx_bus_info(pAd, wdma_idx);
		if (bBusy == TRUE)
			isBusy = true;
...
#ifdef MTK_FE_RESET_RECOVER
extern bool (*mtk_check_wifi_busy)(u32 wdma_idx);
#endif
	...
	void)
{
	...
	mtk_check_wifi_busy = mtk_check_wifi_busy_stat;
#endif
#endif
