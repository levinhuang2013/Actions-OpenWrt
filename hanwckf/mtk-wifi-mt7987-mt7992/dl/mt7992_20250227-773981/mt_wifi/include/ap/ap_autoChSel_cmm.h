...
typedef struct {
	...
	UINT32 chanbusytime_psc[MAX_NUM_OF_CHANNELS + 1];/*used for MTK Alg*/
...
typedef struct _AUTOCH_SEL_CH_LIST {
	...
	/*for Intlgnt ACS*/
	UCHAR TxPower;
	/*to retain 3 decimal places,
	each para is multiplied by 1000*/
	ULONG Score;
	ULONG PrioParam;
	ULONG DataRateParam;
	ULONG AvgDataRateParam;
	ULONG AvgScore;
	/*record calculation para for debug*/
	ULONG TxpowerDataRate;
	USHORT idletime;
...
enum _ACS_EXECUTE_TYPE {
	ACS_EXECUTE_TYPE_NONE = 0,
	ACS_EXECUTE_TYPE_NORMAL_MODE,
	ACS_EXECUTE_TYPE_EMERGENCY_MODE,
	ACS_EXECUTE_TYPE_COMMAND
};
...
typedef struct _AUTOCH_SEL_CTRL {
	...
	UCHAR CountofCh;
	BOOLEAN RestoreFlag;
	UCHAR MaxChPriority; /*the max channel priority that ACS should run*/

	/*ACS execution record*/
	ULONG LatestACSTimeNode;
	enum _ACS_EXECUTE_TYPE LatestACSType;

	/*Auto ACS*/
	UINT32 ACSCheckCount; /* if  ACSCheckCount > ACSCheckTime, then do ACS check */
	UINT AutoNormTriggerCount;
	UINT AutoEmergTriggerCount;
	UINT CmdTriggerCount;
	UINT32 NormalExceedingCount;
	UINT EmergExceedingCount;
	ULONG AutoCheckStartTime;
	UCHAR LastACSCh;
	...
	ULONG MinDataRate;
	ULONG MaxDataRate;
	ULONG MinAvgDataRate;
	ULONG MaxAvgDataRate;
