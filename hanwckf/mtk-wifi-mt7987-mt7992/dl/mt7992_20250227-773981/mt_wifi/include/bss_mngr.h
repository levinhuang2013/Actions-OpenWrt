...
/** struct bmgr_disc_param - in-of-band and out-of-band discovery parameters
 *
 * This struct maintains parameters for in-of-band and out-of-band discovery.
 * (WiFi 6, HE)
 *
 * @param iob_by_cfg indicate follow user configuration for iob disc.
 * @param iob_type frame type for iob disc.
 * @param iob_interval frame transmission interval for iob disc.
 * @param iob_mode tx mode of frame for iob disc.
 * @param rnr_6g_rule oob disc rule for 6G
 */
struct bmgr_disc_param {
	/* in-of-band discovery parameters */
	u8 iob_by_cfg;
	u8 iob_type;		/* enum unsolicit_txtype */
	u8 iob_interval;	/* UNSOLICIT_TX_INTERVAL */
	u8 iob_mode;		/* enum unsolicit_txmode */

	/* out-of-band discovery parameters */
	u8 rnr_6g_rule;		/* enum rnr_reporting_rule */
};
...
struct bmgr_reg_info {
	...
	struct bmgr_disc_param disc_param;
...
struct csa_in_ml_ie {
	u16 csa_offset_in_ml_ie[BSS_MNGR_MAX_BAND_NUM];	/* output: csa_ie offset in ml IE */
	u8 csa_bss_idx[BSS_MNGR_MAX_BAND_NUM];		/* output: bss_idx of the csa in ml IE */
};
...
/*
 * HE BSS in-of-band/out-of-band discovery update
 */
enum {
	BSS_IOB_DISCOV_UPDATE = 0,
	BSS_OOB_DISCOV_UPDATE,
};
#endif
...
struct bss_mngr_ops {
...
#ifdef CONFIG_6G_SUPPORT
	int (*bmgr_update_he_bss_discov)(PNET_DEV pNetDev, u8 update_type, struct bmgr_disc_param *disc_param);
#endif
	...
	int (*bmgr_mld_group_addr_set)(struct wifi_dev *wdev, u8 mld_grp_idx, u8 *mld_addr);
#endif
};
...
#ifdef CONFIG_6G_SUPPORT
	u8 (*bmgr_event_update_iob_disc)(PNET_DEV pNetDev, struct bmgr_disc_param *disc_param);
#endif
#ifdef DOT11_EHT_BE
...
#ifdef CONFIG_6G_SUPPORT
NDIS_STATUS bss_mngr_update_he_bss_discov(struct wifi_dev *wdev, u8 update_type);
#endif
...
NDIS_STATUS bss_mngr_mld_group_addr_set(struct wifi_dev *wdev, u8 mld_grp, u8 *mld_addr);
#endif
...
struct tid2lnk_ie_info {
	...
	u16		ctrl_mst:1;
	u16		ctrl_ed:1;
	u16		ctrl_size:1; /*0: 2 octets; 1: 1 octet*/
	u16		ctrl_reserved:2;
	u16		ctrl_map_pres:8;

	/* per-link's tid [7:0] mapping */
	u8		tid_map[MLD_LINK_MAX];
	UINT16  bcn_tsf;      /*bit10-bit25*/
	UINT16	mst_tsf;
	UINT32  ed_tsf;
};
...
struct csa_info {
	u16	repted_csa_offset_in_per_sta_pf[BSS_MNGR_MAX_BAND_NUM];
	u8 repted_csa_bss_idx[BSS_MNGR_MAX_BAND_NUM];

	u16	repting_csa_offset_in_per_sta_pf[BSS_MNGR_MAX_BAND_NUM];
	u8 repting_csa_bss_idx[BSS_MNGR_MAX_BAND_NUM];
};
