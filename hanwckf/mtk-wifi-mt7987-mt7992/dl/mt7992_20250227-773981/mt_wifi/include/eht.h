...
enum eht_crit_upd_mode {
	CRIT_UPD_MODE_FLAG_FW_CMD_ONLY = 0,
	CRIT_UPD_MODE_FLAG_FW_CMD_WITH_EVENT = 1,
};
...
struct eht_mld_affiliated_sta {
	unsigned char if_addr[MAC_ADDR_LEN];
	unsigned char link_addr[MAC_ADDR_LEN];
	unsigned char valid;
...
struct eht_mld_info {
	...
	unsigned char peer_mld_addr[MAC_ADDR_LEN];
	unsigned char peer_mld_valid;
	struct eht_mld_affiliated_aps aps[BSS_MNGR_MAX_BAND_NUM];
	struct eht_mld_affiliated_sta stas[MLD_LINK_MAX];
};
...
INT eht_ap_mld_get(struct wifi_dev *wdev, u8 mld_group_idx, struct eht_mld_info *reply_mld);
INT eht_ap_mld_get_by_mld_addr(struct wifi_dev *wdev, u8 *mld_addr, struct eht_mld_info *reply_mld);
