...
/*peer link at2lm struct*/
struct sta_mld_at2lm_link_info {
	UINT8   at2lm_ie_exist;
	UINT8   dir;
	UINT8   link_disabled;      /*disable by tid_map*/
	UINT8   link_dis_ind;       /*disable by rnr info*/
	UINT8   at2lm_state;		/*NONE, MST, ED*/
	UINT8   dtim_bcn;
	UINT16  bcn_interval;
	UINT16  bcn_tsf;      /*bit10-bit25*/
	UINT16	mst_tsf;
	UINT32  ed_tsf;
	UINT8	tid_map[MLD_LINK_MAX];		/*per link tid mapping */
};

/*sta at2lm struct*/
struct sta_mld_at2lm_contract {
	UINT8   state;		/*NONE, MST, ED*/
	UINT8   dir;
	UINT8	tid_map[MLD_LINK_MAX];		/*per link tid mapping */
	struct	mutex op_mutex;
	struct  sta_mld_at2lm_link_info at2lm_link_info[MLD_LINK_MAX];
};
...
struct peer_mld_entry {
	...
	struct sta_mld_at2lm_contract at2lm_contract;
};
