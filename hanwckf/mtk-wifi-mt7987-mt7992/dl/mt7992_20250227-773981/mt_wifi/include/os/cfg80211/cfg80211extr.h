...
#define CFG80211_FILL_TXRX_STREAM(__pAd, __wdev, __pBandInfo) \
	{\
		do {\
			(__pBandInfo)->TxStream = ((__wdev == NULL) || (__wdev->wpf_cfg == NULL)) ? __pAd->mcs_nss.max_nss : wlan_config_get_tx_stream(__wdev);	\
			(__pBandInfo)->RxStream = ((__wdev == NULL) || (__wdev->wpf_cfg == NULL)) ? __pAd->mcs_nss.max_nss : wlan_config_get_rx_stream(__wdev);	\
		} while (0);\
	}
#endif
