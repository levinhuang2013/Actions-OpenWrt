...
#define ROAM_FUNC_LEN 100
#define ROAM_DESC_LEN 200
...
struct csa_ie_info {
	UINT16 ml_ie_offset[BSS_MNGR_MAX_BAND_NUM];
	UINT16 ml_csa_ie_offset[BSS_MNGR_MAX_BAND_NUM];
	UINT16 csa_ie_offset_in_ml[BSS_MNGR_MAX_BAND_NUM];
	UINT8 csa_bss_idx[BSS_MNGR_MAX_BAND_NUM];
};
...
struct BCN_REQ_DATA {
	BOOLEAN				BcnReqScan;
	RRM_BEACON_REQ_INFO	BcnReqInfo;
	UINT8				DialogT;
	CHAR				ReqSsid[MAX_LEN_OF_SSID + 1];
	UINT8				ReqSsid_len;
	UCHAR				Addr1Req[6];
	UCHAR				Addr2Req[6];
	MEASURE_REQ_INFO	MeasureReqInfo_scan;
	UINT8				Incap;
	UINT8				Ref;
};
#endif
...
#ifdef EM_PLUS_EXT_SUPPORT
	/*Bcn Rep */
	struct BCN_REQ_DATA Bcn_Req_Data;
#endif
	/* VLAN related */
	BOOLEAN bVLAN_Tag;
	USHORT VLAN_VID;
...
/* station mld device */
	...
	UCHAR rsneo_len;
	UCHAR rsnxeo_len;
	UCHAR rsneo2_content[MAX_LEN_OF_RSNIE];
	UCHAR rsneo2_len;
#endif /* CFG_RSNO_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
#ifdef RT_CFG80211_SUPPORT
...
struct ACS_CONFIG {
	...
	UCHAR ChUtilThr;
	UCHAR ACSIceChUtilThr;
	UCHAR ACSMaxACSTimes;
	UCHAR ACSSwChThr;/*in percentage, only switch if new ch is exceed this thr*/
};
...
#ifdef ROAM_CALIB_SUPPORT
struct CAL_DATA {
	char functionName[ROAM_FUNC_LEN];
	char desc[ROAM_DESC_LEN];
...
#ifdef FTM_SUPPORT
	PFTM_CTRL pFtmCtrl;
	struct _FTM_PEER_INFO ftm_peer[MAX_FTM_TBL_SIZE];
	NDIS_SPIN_LOCK ftm_lock;
	struct loc_bias_table ftm_bias_tbl;
#endif /* FTM_SUPPORT */
...
/* For wifi and md coex in colgin project*/
	...
	CHAR max_power_5g;
#ifdef ROAM_CALIB_SUPPORT
	struct _ROAM_STA RoamingSta[ROAM_STA_CNT];
	UINT8 RoamCalibEnable;
#endif /*ROAM_CALIB_SUPPORT*/
};
...
struct wifi_sta_info {
	...
	UINT32 tx_latency_max_t2;
	UINT32 tx_latency_min_t2;
	UINT32 air_latency_max;
	UINT32 air_latency_min;
	UINT32 air_latency_max_t2;
	UINT32 air_latency_min_t2;
	BOOLEAN valid;  /* if find mac, set to true, else set to false */
};
#endif
...
INT set_ba_iot_test_action(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
int frequency_to_channel(IN USHORT freq);
...
VOID RTMPDeletePMKIDCacheForSTA(
	IN NDIS_AP_802_11_PMKID * pPMKIDCache,
	IN UCHAR *own_mld_mac,
	IN UCHAR *own_link_mac,
	IN UCHAR *peer_mac,
	IN UCHAR is_ft);
...
VOID TRTableInsertMcastEntry(RTMP_ADAPTER *pAd, UINT16 hw_wcid, struct wifi_dev *wdev, BOOLEAN isVlan);
VOID TRTableEntryDump(RTMP_ADAPTER *pAd, INT tr_idx, const RTMP_STRING *caller, INT line);
VOID MgmtTableSetMcastEntry(RTMP_ADAPTER *pAd, UINT16 wcid, struct wifi_dev *wdev, BOOLEAN isVlan);
...
enum EDCCA_REGION_T {
	...
	EDCCA_REGION_ETSI_JP      = 5,
	EDCCA_REGION_ETSI_2023_JP = 6,
...
#define DEFAULT_TMAC_CDTR_VALUE	0x003000E7
#define DEFAULT_TMAC_ODTR_VALUE	0x001C003C
#define DEFAULT_TMAC_OMDTR_VALUE	0x001C00D0
...
void MacTableAuthReqClearMloEntry(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *link_addr, u8 *ml_ie, BOOLEAN is_mlo_auth);
#endif /* DOT11_EHT_BE */
