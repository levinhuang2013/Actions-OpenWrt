	...
	IN struct cfg80211_scan_request *pRequest)
{
	...
	if (pNdev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) {
		...
		if (pAd->CommonCfg.ApcliPartialScan)
			scan_cmd.ScanType = SCAN_PARTIAL;
	...
	OUT struct survey_info *survey)
{
	...
	ch_idx = Channel2Index(pAd, ieee80211_frequency_to_channel(chan->center_freq));
	...
	survey->channel = chan;
	if (ch_idx != -1 && ch_idx < MAX_NUM_OF_CHANNELS) {
		survey->time_busy = div_u64(pAd->ScanCtrl.busytime[ch_idx], 1000);
		survey->time_rx = div_u64(pAd->ScanCtrl.rx_time[ch_idx],  1000);
#if defined(CFG_CFG80211_VERSION) && (KERNEL_VERSION(6, 1, 0) <= CFG_CFG80211_VERSION)
		survey->time_bss_rx = div_u64(pAd->ScanCtrl.bss_rx_time[ch_idx], 1000);
#endif
		survey->time_tx = div_u64(pAd->ScanCtrl.tx_time[ch_idx], 1000);
		survey->noise = pAd->ScanCtrl.ch_AvgNF[ch_idx];
		survey->time = div_u64(pAd->ScanCtrl.active_time[ch_idx], 1000);
	}
	...
	IN UINT8							KeyIdx)
{
	...
	if ((pNdev->ieee80211_ptr->iftype == RT_CMD_80211_IFTYPE_AP) ||
		(pNdev->ieee80211_ptr->iftype == RT_CMD_80211_IFTYPE_P2P_GO))
		RTMP_DRIVER_80211_AP_BEACON_KEY_DEFAULT_SET(pAd, pNdev, KeyIdx);
#endif /* CONFIG_AP_SUPPORT */
				      ...
				      struct cfg80211_chan_def *chandef)
{
	...
	if (!RTMP_TEST_FLAG(pAd->physical_dev, PH_DEV_SYSTEM_START_UP)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR, "physical_dev not ready!\n");
		return -ENODEV;
	}
...
static void CFG80211_SetChansDfsState(struct wiphy *wiphy, u32 center_freq,
					 u32 bandwidth,
					 enum nl80211_dfs_state dfs_state)
{
	struct ieee80211_channel *c;
	u32 freq;
	ULONG *__pPriv = (ULONG *)(wiphy_priv(wiphy));
	PRTMP_ADAPTER pAd = (struct _RTMP_ADAPTER *)(*__pPriv);

	for (freq = center_freq - bandwidth/2 + 10;
	     freq <= center_freq + bandwidth/2 - 10;
	     freq += 20) {
		c = ieee80211_get_channel_khz(wiphy, MHZ_TO_KHZ(freq));
		if (!c || !(c->flags & IEEE80211_CHAN_RADAR))
			continue;

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
				"ch:%d, freq:%d, set dfs_state=%d\n",
				ieee80211_frequency_to_channel(freq), freq, dfs_state);
...
void CFG80211_SetDfsState(struct wiphy *wiphy,
			    ...
			    enum nl80211_dfs_state dfs_state)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
			"chandef: cf1:%d cf2:%d width:%d(%d), dfs_state=%d\n",
			chandef->center_freq1, chandef->center_freq2, chandef->width, width, dfs_state);
	if (width < 0)
		return;

	CFG80211_SetChansDfsState(wiphy, chandef->center_freq1, width, dfs_state);

	if (!chandef->center_freq2)
		return;
	CFG80211_SetChansDfsState(wiphy, chandef->center_freq2, width, dfs_state);
}

bool CFG80211_IsSubChannel(struct wiphy *wiphy,
				UINT cent_ch,
				...
				UINT sub_ch)
{
	ULONG *__pPriv = (ULONG *)(wiphy_priv(wiphy));
	PRTMP_ADAPTER pAd = (struct _RTMP_ADAPTER *)(*__pPriv);
	UINT32 cent_freq_mhz, sub_ch_freq_mhz, bw_mhz;
	UINT32 band_id = CFG80211_Get_BandId(pAd);

	cent_freq_mhz = ieee80211_channel_to_frequency(cent_ch, band_id);
	sub_ch_freq_mhz = ieee80211_channel_to_frequency(sub_ch, band_id);
	bw_mhz = nl80211_chan_width_to_mhz(width);

	if (sub_ch_freq_mhz >= (cent_freq_mhz - bw_mhz/2 + 10) &&
		sub_ch_freq_mhz <= (cent_freq_mhz + bw_mhz/2 - 10))
		return TRUE;
...
*/
	...
	if ((params->vlan_id) || (params->vlan)) {
		...
		if (params->vlan_id) {
			wdev = RTMP_OS_NETDEV_GET_WDEV(dev);
			if (wdev == NULL)
				return 0;
			vg_info = CFG80211_GetVlanInfoByVlanid(wdev, params->vlan_id);
			if (vg_info)
				vlan_id = vg_info->vlan_id;
		} else if (params->vlan) {
		...
		vg_info = CFG80211_GetVlanInfoByVlandev(wdev, params->vlan);
			vlan_id = (vg_info) ? vg_info->vlan_id : 0;
		}
#ifdef DYNAMIC_VLAN_SUPPORT
		if (vlan_id > 0)
			RTMP_DRIVER_80211_AP_BIND_VLAN(pAd, pEntry, vlan_id);
		MTWF_PRINT("%s STA %02X:%02X:%02X:%02X:%02X:%02X iface %s vlan_id %d\n",
			__func__, PRINT_MAC(pMacAddr), dev->name, pEntry->vlan_id);
#endif
	}
#endif
	return 0;
...
static int CFG80211_GetTxPower(
	struct wiphy *wiphy,
	struct wireless_dev *wdev,
	INT32 *dbm)
{
	...
	struct vif_params *pParams)
{
	...
	if (pAd && ((PRTMP_ADAPTER)pAd)->net_dev) {
		if (!strcmp(vifInfo.vifName, RtmpOsGetNetDevName(((PRTMP_ADAPTER)pAd)->net_dev))) {
			MTWF_DBG(NULL, DBG_CAT_CFG80211, CATCFG80211_INTF, DBG_LVL_ERROR,
				"pDev %s is the primary dev\n", vifInfo.vifName);
			return ERR_PTR(-ENFILE);
		}
	}
						 ...
						 const void *data, int len)
{
	...
	nla_for_each_attr(attr, head, len, rem) {
		...
		if (nla_type(attr) == MTK_NL80211_VENDOR_ATTR_E2P_WRITE_PARAM &&
			...
			if (eepAddr > get_dev_eeprom_size(pAd)) {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
					"start addr > eeprom size\n");
				goto error;
			}
						 ...
						 const void *data, int len)
{
	...
	UINT32 max_len = MTK_NL80211_VENDOR_SUBCMD_VENDOR_SHOW_BUFFER_SIZE;
	int free_len = max_len;
	...
	while ((this_char = strsep((char **)&tmp, ",")) != NULL) {
		...
		msg_len = strlen(msg);
		if (msg_len > 0 && (value == NULL || strncmp(value, msg, msg_len) != 0)) {
			skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, msg_len + 1);
			if (!skb) {
				MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_NOTICE,
					"fail to allocate reply msg\n");
						...
						void *data, int len)
{
	...
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, len, MTK_NL80211_VENDOR_EVENT_OFFCHANNEL_INFO, flag);
	if (!skb) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
				"cfg80211_vendor_event_alloc fail!!\n");
		return -EINVAL;
	...
	cfg80211_vendor_event(skb, flag);
	return 0;
}

static enum mtk_nl80211_vendor_events radar_event_map_nl80211_to_vendor(enum nl80211_radar_event event)
{
	switch (event) {
	case NL80211_RADAR_DETECTED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_DETECTED;
	case NL80211_RADAR_CAC_FINISHED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_CAC_FINISHED;
	case NL80211_RADAR_CAC_ABORTED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_CAC_ABORTED;
	case NL80211_RADAR_NOP_FINISHED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_NOP_FINISHED;
	case NL80211_RADAR_PRE_CAC_EXPIRED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_PRE_CAC_EXPIRED;
	case NL80211_RADAR_CAC_STARTED:
		return MTK_NL80211_VENDOR_EVENT_RADAR_CAC_STARTED;
	default:
		return MTK_NL80211_VENDOR_EVENT_MAX;
	}
}

int nl80211_send_event_radar_notify(struct wiphy *wiphy,
						const struct cfg80211_chan_def *chandef,
						enum nl80211_radar_event event)
{
	...
	enum mtk_nl80211_vendor_events vendor_event = radar_event_map_nl80211_to_vendor(event);

	MAC80211_PAD_GET(pAd, wiphy);
	wdev = pAd->pCfg80211_CB->pCfg80211_Wdev;

	if (vendor_event < MTK_NL80211_VENDOR_EVENT_RADAR_DETECTED ||
		vendor_event > MTK_NL80211_VENDOR_EVENT_RADAR_CAC_STARTED) {
		return -EINVAL;
	}
	...
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, (sizeof(u32)+sizeof(u32))*6, vendor_event, GFP_KERNEL);
	if (!skb) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
				"cfg80211_vendor_event_alloc fail!!\n");
		return -EINVAL;
	...
	if (nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_FREQ, chandef->chan->center_freq)) {
		kfree_skb(skb);
		return -ENOBUFS;
	}

	if (nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_FREQ_OFFSET, chandef->chan->freq_offset)) {
		kfree_skb(skb);
		return -ENOBUFS;
	}
	...
	case NL80211_CHAN_WIDTH_20:
	case NL80211_CHAN_WIDTH_40:
		if (nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_CHANNEL_TYPE, cfg80211_get_chandef_type(chandef))) {
			kfree_skb(skb);
			return -ENOBUFS;
		}
		break;
	...
	if (nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_CHANNEL_WIDTH, chandef->width)) {
		kfree_skb(skb);
		return -ENOBUFS;
	}

	if (nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_CHANNEL_CENTER_FREQ1, chandef->center_freq1)) {
		kfree_skb(skb);
		return -ENOBUFS;
	}

	if (chandef->center_freq2 &&
	    nla_put_u32(skb, MTK_NL80211_VENDOR_ATTR_RADAR_CHANNEL_CENTER_FREQ2, chandef->center_freq2)) {
		kfree_skb(skb);
		return -ENOBUFS;
	}
						 ...
						 const void *data, int len)
{
	...
	int ret = 0;
	UCHAR chan = 0, ht_coex = 0, ext_chan = 0, bypass_cac = 0;
	UINT32 set_flag = 0;
	UINT32 freq = 0, bw = 0;
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_CHAN_SET_BYPASS_CAC]) {
		bypass_cac = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_CHAN_SET_BYPASS_CAC]);
		set_flag |= MTK_CFG80211_CHAN_SET_FLAG_BYPASS_CAC;
	}

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_NOTICE, "set_flag=0x%x\n", set_flag);
	if (set_flag != 0) {
		ret = mtk_cfg80211_vndr_cmd_set_channel_attributes(
			pAd, set_flag, chan, bw, ext_chan, ht_coex, bypass_cac);
	...
	IN UINT32 reconfig_state)
{
	...
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, buflen, MTK_NL80211_VENDOR_EVENT_MLO_RECONF, GFP_ATOMIC);
	if (!skb) {
...
#ifdef WAPP_SUPPORT
#ifdef REPORT_CH_TXPWR_SUPPORT
#define IS_20M(__opclass) \
	((__opclass == 115) || (__opclass == 118) || (__opclass == 121) || (__opclass == 125) \
	|| (__opclass == 131) || (__opclass == 81) || (__opclass == 82))
...
static int mtk_cfg80211_reply_wapp_tx_pwr(struct wiphy *wiphy, const void *data, int len)
{
	...
	UINT ifIndex;
	POS_COOKIE pObj;
	struct _wdev_tx_power_ext *tx_pwr = NULL;
#ifndef MAP_6E_SUPPORT
	wdev_op_class_info *op_class;
#else
	struct _wdev_op_class_info_ext *op_class;
#endif
#ifdef REPORT_CH_TXPWR_SUPPORT
	struct _wdev_tx_power_limit *ptxpwrinfo = NULL;
	UINT8 i = 0;
#endif
...
#endif
	os_alloc_mem(pAd, (UCHAR **)&tx_pwr, sizeof(struct _wdev_tx_power_ext));
	if (tx_pwr == NULL) {
		os_free_mem(op_class);
		return -ENOMEM;
	}
	NdisZeroMemory(tx_pwr, sizeof(struct _wdev_tx_power_ext));

	tx_pwr->num_of_op_class = op_class->num_of_op_class;
#ifdef REPORT_CH_TXPWR_SUPPORT
	if (ifIndex == 1) {
		os_alloc_mem(pAd, (UCHAR **)&ptxpwrinfo, sizeof(struct _wdev_tx_power_limit));
		NdisZeroMemory(ptxpwrinfo, sizeof(struct _wdev_tx_power_limit));
		tx_pwr->band =  HcGetBandByWdev(wdev);
		tx_pwr->tx_power_set = 1;
		MtGetMaxTxPwrLimit(pAd, tx_pwr->band, ptxpwrinfo, POWER_LIMIT_TABLE_TYPE_SKU);
	} else {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_DEBUG, "ifindex not one\n");
		tx_pwr->tx_power_set = 0;
	}
#endif
	for (op = 0; op < op_class->num_of_op_class; op++) {
		...
		/* empty channel set. */
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_INFO,
			"ch_num:%u, opclass_num:%u\n", channel_set_num, op_class->num_of_op_class);
		if (channel_set_num == 0 || channel_set_num > MAX_NUM_OF_CHANNELS)
			continue;
		/*
		...
		*/
#ifdef REPORT_CH_TXPWR_SUPPORT
		tx_pwr->tx_pwr_limit[pwr_num].ch_num = channel_set_num;
#endif
		ChSetMinLimPwr = 0xFF;
...
#ifdef REPORT_CH_TXPWR_SUPPORT
			if (IS_20M(tx_pwr->tx_pwr_limit[pwr_num].op_class) == 0
				|| (tx_pwr->tx_power_set == 0)) {
				tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].channel = 0;
				tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].pwrlimit = 0;
				tx_pwr->tx_pwr_limit[pwr_num].ch_num = 0;
				continue;
			}

			tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].channel = channel_set[ch];
			for (i = 0; i < ptxpwrinfo->ch_num; i++) {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_DEBUG,
					"ch:%d pwr:%d/%d/%d/%d/%d\n", ptxpwrinfo->tx_pwr[ch].channel,
					ptxpwrinfo->tx_pwr[ch].pwrlimit[0], ptxpwrinfo->tx_pwr[ch].pwrlimit[1],
					ptxpwrinfo->tx_pwr[ch].pwrlimit[2], ptxpwrinfo->tx_pwr[ch].pwrlimit[3],
					ptxpwrinfo->tx_pwr[ch].pwrlimit[4]);
				if (ptxpwrinfo->tx_pwr[i].channel == tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].channel) {
					tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].pwrlimit = ptxpwrinfo->tx_pwr[i].pwrlimit[0];
					MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_DEBUG,
					"TX POWER ch:%u, power:%u, op_class:%u!\n",
					tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].channel,
					tx_pwr->tx_pwr_limit[pwr_num].ch_pwr[ch].pwrlimit,
					tx_pwr->tx_pwr_limit[pwr_num].op_class);
					break;
	...
	ret = mtk_cfg80211_vndr_cmd_reply_msg(pAd, wiphy,
					...
					tx_pwr, sizeof(struct _wdev_tx_power_ext));
	if (ret != 0)
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"mtk_cfg80211_vndr_cmd_reply_msg fail!\n");
	...
	if (op_class)
		os_free_mem(op_class);
#ifdef REPORT_CH_TXPWR_SUPPORT
	if (ptxpwrinfo && ifIndex == 1)
		os_free_mem(ptxpwrinfo);
#endif
	return ret;
}
...
static int mtk_cfg80211_reply_band_info_freqlist(struct wiphy *wiphy, const void *data, int len)
{
	...
	for (i = 0; (i < pChCtrl->FreqListNum) && (pChCtrl->FreqListNum <= MAX_NUM_OF_CHANNELS); i++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_INFO,
				"Freq = %d, channel = %d\n", pChCtrl->FreqList[i],
				frequency_to_channel(pChCtrl->FreqList[i]));

		LeftBufSize = TotalLen - strlen(msg);
		if (i == 0)
			Status = snprintf(msg + strlen(msg), LeftBufSize, "%d", frequency_to_channel(pChCtrl->FreqList[i]));
		else
			Status = snprintf(msg + strlen(msg), LeftBufSize, ", %d", frequency_to_channel(pChCtrl->FreqList[i]));
...
static int mtk_cfg80211_reply_eht_cap(struct wiphy *wiphy, const void *data, int len)
{
	...
	/* Report MAX BW to wapp for ap capability */
						 ...
						 const void *data, int len)
{
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_CHAN_LIST]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_NOP_CHANNEL_LIST]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_OP_CLASS]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_BSS_INFO]) {
...
#ifdef WSC_INCLUDED
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_WAPP_WSC_PROFILES]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_MAX_NUM_OF_SSIDS]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_EXTENSION_CHANNEL]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_TRANSMITPOWER]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_80211H]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_BASIC_RATE]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_SUPP_RATE]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_ACS_REFRESH_PERIOD]) {
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_GET_RUNTIME_INFO_GET_WMODE]) {
										...
										struct wifi_dev *wifi_wdev)
{
	...
	/*record ACS execute type and time*/
	pAutoChCtrl->AutoChSelCtrl.LatestACSTimeNode = jiffies;
	pAutoChCtrl->AutoChSelCtrl.LatestACSType = ACS_EXECUTE_TYPE_COMMAND;
						 ...
						 const void *data, int len)
{
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_CHECK_TIME]) {
		...
		pAutoChCtrl->AutoChSelCtrl.AutoCheckStartTime = 0;
		if (check_time == 0) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"disable Periodic ACS\n");
			pAutoChCtrl->AutoChSelCtrl.ACSCheckCount = 0;
		} else if (pAutoChCtrl->AutoChSelCtrl.ACSCheckCount >= check_time) {
			pAutoChCtrl->AutoChSelCtrl.ACSCheckCount = 0;
#ifdef EM_PLUS_SUPPORT
			if (!IS_MAP_TURNKEY_ENABLE(pAd))
#endif /* EM_PLUS_SUPPORT */
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_DATA_RATE_WT]) {
		u16 data_rate_wt = nla_get_u16(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_DATA_RATE_WT]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"Set Intlgnt ACS data_rate_wt=%d\n", data_rate_wt);
		pAutoChCtrl->AutoChSelCtrl.DataRateWeight =
			data_rate_wt;
	}

	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_PRIO_WT]) {
		u16 prio_wt = nla_get_u16(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_PRIO_WT]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"Set Intlgnt ACS PrioWeight=%d\n", prio_wt);
		pAutoChCtrl->AutoChSelCtrl.PrioWeight =
			prio_wt;
	}

	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_TX_POWER_CONS]) {
		u8 TXPowerCons = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_TX_POWER_CONS]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"is Intlgnt ACS TXPowerCons =%d\n", TXPowerCons);
		if (TXPowerCons)
			pAutoChCtrl->AutoChSelCtrl.TXPowerCons =
			TRUE;
		else
			pAutoChCtrl->AutoChSelCtrl.TXPowerCons =
			FALSE;
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_CH_UTIL_THR]) {
		u8 ch_util_thr = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_CH_UTIL_THR]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"ACS ch_util_thr =%d\n", ch_util_thr);

			pAd->ApCfg.AcsCfg.ChUtilThr = ch_util_thr;
	}

	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_ICE_CH_UTIL_THR]) {
		u8 ice_ch_util_thr = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_ICE_CH_UTIL_THR]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"ACS ice_ch_util_thr =%d\n", ice_ch_util_thr);

			pAd->ApCfg.AcsCfg.ACSIceChUtilThr = ice_ch_util_thr;
	}

	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_MAX_ACS_TIMES]) {
		u8 max_acs_times = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_MAX_ACS_TIMES]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"ACS max_acs_times =%d\n", max_acs_times);

			pAd->ApCfg.AcsCfg.ACSMaxACSTimes = max_acs_times;
	}

	if (tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_SWITCH_THR]) {
		u8 sw_ch_thr = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AUTO_CH_SWITCH_THR]);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_NOTICE,
				"ACS sw_ch_thr =%d\n", sw_ch_thr);

			pAd->ApCfg.AcsCfg.ACSSwChThr = sw_ch_thr;
	}
						 ...
						 const void *data, int len)
{
	...
	if (tb[MTK_NL80211_VENDOR_ATTR_AP_SET_NO_BMODE]) {
		unsigned char enable = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AP_SET_NO_BMODE]);

		ret = mtk_cfg80211_set_no_bmode(pAd, ifIndex, pObj->ioctl_if_type, enable);
...
#ifdef GTB_APP_SUPPORT
static int mtk_cfg80211_reply_ai_antinoise(RTMP_ADAPTER *pAd, struct wiphy *wiphy)
{
	int ret;

	ret = mtk_cfg80211_vndr_cmd_reply_msg(pAd, wiphy,
					MTK_NL80211_VENDOR_ATTR_AP_AI_ANTINOISE,
					&pAd->CommonCfg.ai_antinoise, sizeof(USHORT));
...
#endif
#ifdef GTB_APP_SUPPORT
	if (tb[MTK_NL80211_VENDOR_ATTR_AP_AI_ANTINOISE]) {
		USHORT ai_antinoise = nla_get_u16(tb[MTK_NL80211_VENDOR_ATTR_AP_AI_ANTINOISE]);

		if (ai_antinoise == 0xffff)
			ret = mtk_cfg80211_reply_ai_antinoise(pAd, wiphy);
		else
			ret = mtk_cfg80211_set_ai_antinoise(pAd, ai_antinoise);
...
#ifdef GTB_APP_SUPPORT
static int mtk_cfg80211_reply_gtbapp_sta_info(struct wiphy *wiphy, const UCHAR *staMac)
{
	PRTMP_ADAPTER pAd;
	struct sk_buff *skb;
	int ret = 0;
	...
	NdisZeroMemory(&sta, sizeof(sta));
	NdisCopyMemory(&sta.MacAddr, staMac, MAC_ADDR_LEN);
	Get_Gtb_Sta_Info(pAd, &sta);


	skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, sizeof(struct wifi_sta_info));
	if (!skb) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"fail to allocate reply msg.\n");
		return -EINVAL;
	...
	if (nla_put(skb, MTK_NL80211_VENDOR_ATTR_STA_INFO_GTBAPP_FORMAT, sizeof(struct wifi_sta_info), &sta)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR, "nla_put fail.\n");
		kfree_skb(skb);
		return -EINVAL;
	}
...
#ifdef GTB_APP_SUPPORT
	if (tb[MTK_NL80211_VENDOR_ATTR_STA_INFO_GTBAPP_FORMAT]) {
		StaMac = nla_data(tb[MTK_NL80211_VENDOR_ATTR_STA_INFO_GTBAPP_FORMAT]);
		ret = mtk_cfg80211_reply_gtbapp_sta_info(wiphy, StaMac);
		if (ret) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
				"mtk_cfg80211_reply_gtbapp_sta_info failed.ret=%x\n", ret);
			return -EINVAL;
		}
	}
#endif
...
static int mtk_cfg80211_fill_connected_sta_mld(PRTMP_ADAPTER pAd, struct wiphy *wiphy, struct bmgr_mld_sta *mld_sta)
{
	...
	int payload_len = 0;
	PRTMP_ADAPTER ad;
	signed char sta_rssi[MAX_RSSI_LEN];
	UINT8 rssi_len = 0;
	...
	for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
		...
		for (j = 0; j < rssi_len; j++)
			sta_rssi[j] = -127;
		rtmp_get_rssi(ad, pEntry->wcid, sta_rssi, rssi_len);
...
nla_policy SUBCMD_FTM_POLICY[MTK_NL80211_VENDOR_ATTR_FTM_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_FTM_FTMR_TIMEOUT] = { .type = NLA_U32 },
	[MTK_NL80211_VENDOR_ATTR_FTM_NON_ASAP_DELAY_TIME] = { .type = NLA_U16 },
};
...
nla_policy SUBCMD_SET_CHANNEL_POLICY[MTK_NL80211_VENDOR_ATTR_CHAN_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_CHAN_SET_BYPASS_CAC] = { .type = NLA_U8 },
};
...
nla_policy SUBCMD_SET_AUTO_CH_SEL_POLICY[MTK_NL80211_VENDOR_ATTR_AUTO_CH_SEL_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_DATA_RATE_WT] = { .type = NLA_U16 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_PRIO_WT] = { .type = NLA_U16 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_TX_POWER_CONS] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_CH_UTIL_THR] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_ICE_CH_UTIL_THR] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_CHECK_TIME] = { .type = NLA_U32 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_MAX_ACS_TIMES] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AUTO_CH_SWITCH_THR] = { .type = NLA_U8 },
};
...
nla_policy SUBCMD_SET_AP_BSS_POLICY[MTK_NL80211_VENDOR_AP_BSS_ATTR_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_AP_SET_NO_BMODE] = { .type = NLA_U8 },
};
...
nla_policy SUBCMD_SET_AP_RADIO_POLICY[MTK_NL80211_VENDOR_AP_RADIO_ATTR_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_AP_AI_ANTINOISE] = { .type = NLA_U16 },
...
nla_policy SUBCMD_GET_STA_POLICY[MTK_NL80211_VENDOR_ATTR_STA_ATTR_MAX + 1] = {
	...
	[MTK_NL80211_VENDOR_ATTR_STA_INFO_GTBAPP_FORMAT] = NLA_POLICY_ETH_ADDR,
...
#ifdef DOT11_EHT_BE
static int
mtk_cfg80211_vndr_mld_create_multi_grp(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	u8 tgt_mld_grp_idx)
{
	...
	u8 mld_type = BMGR_MLD_TYPE_MULTI;
	...
	ret = eht_ap_mld_available_address_search(
			wdev,
			tgt_mld_grp_idx,
			mld_addr,
			mld_type);
	if (!ret) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"unique mld addr get failed for mld grp idx:%u!!\n", tgt_mld_grp_idx);
		return NDIS_STATUS_FAILURE;
	}

	eht_ap_mld_fill_mld_param_from_selected_link(wdev, &mld_param);

	ret = eht_ap_mld_create(wdev,
			&tgt_mld_grp_idx,
			mld_addr,
			&mld_param,
			&mld_type);
	...
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"multi-link mld grp create failed: idx:%u!!\n", tgt_mld_grp_idx);
	}
...
static int
mtk_cfg80211_vndr_set_ap_mld_index_single2multi(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct eht_mld_info *src_mld_ptr,
	u8 tgt_mld_grp_idx,
	struct eht_mld_info *tgt_mld_ptr)
{
	int ret;

	if (!tgt_mld_ptr) {
		ret = mtk_cfg80211_vndr_mld_create_multi_grp(pAd, wdev, tgt_mld_grp_idx);
		if (ret != 0) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"multi-link mld grp create failed: idx:%u!!\n", tgt_mld_grp_idx);
			return ret;
	...
	return eht_ap_mld_link_transfer(wdev, tgt_mld_grp_idx);
}

static int
mtk_cfg80211_vndr_ap_mld_index_multi2multi(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct eht_mld_info *src_mld_ptr,
	u8 tgt_mld_grp_idx,
...
static int
mtk_cfg80211_vndr_set_ap_mld_index(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct nlattr *attr)
{
	...
	u8 src_mld_grp_idx;
	u8 tgt_mld_grp_idx;
	struct eht_mld_info src_mld;
	struct eht_mld_info tgt_mld;
	struct eht_mld_info *src_mld_ptr = NULL;
	struct eht_mld_info *tgt_mld_ptr = NULL;
	...
	tgt_mld_grp_idx = nla_get_u8(attr);
	pMbss = (struct _BSS_STRUCT *)wdev->func_dev;
	src_mld_grp_idx = pMbss->mld_grp_idx;

	if (tgt_mld_grp_idx != 0) {
		if (tgt_mld_grp_idx > MAX_MLD_NUM) {
			ret = -RTMP_IO_EINVAL;
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"invalid mld_grp_idx:%u!!\n", tgt_mld_grp_idx);
			return ret;
		}
		ret = eht_ap_mld_get(wdev, tgt_mld_grp_idx, &tgt_mld);
		if (ret == 0)
			tgt_mld_ptr = &tgt_mld;
		else
			tgt_mld_ptr = NULL;
	...
	if (src_mld_grp_idx != 0) {
		ret = eht_ap_mld_get(wdev, src_mld_grp_idx, &src_mld);
		if (ret != 0) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"mld get failed, mld_grp_idx:%u!!\n", src_mld_grp_idx);
			return ret;
		}
		src_mld_ptr = &src_mld;
	}
	...
	if (tgt_mld_grp_idx == 0) {
		/* src is multi */
		if (src_mld_ptr && src_mld_ptr->mld_type == BMGR_MLD_TYPE_MULTI) {
			ret = eht_ap_mld_link_transfer(wdev, 0);
			if (ret != 0) {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
					"change to singlelink mld failed!!\n");
				return ret;
			}
		} else {
			return 0;
		...
		if (src_mld_ptr && src_mld_ptr->mld_type == BMGR_MLD_TYPE_MULTI) {
			if (src_mld_ptr->mld_grp == tgt_mld_grp_idx)
				return 0;
			ret = mtk_cfg80211_vndr_ap_mld_index_multi2multi(
								pAd,
								wdev,
								src_mld_ptr,
								tgt_mld_grp_idx,
			...
			ret = mtk_cfg80211_vndr_set_ap_mld_index_single2multi(
								pAd,
								wdev,
								src_mld_ptr,
								tgt_mld_grp_idx,
...
SUBCMD_SET_AP_MLD_ADDRESS_POLICY[MTK_NL80211_VENDOR_ATTR_AP_MLD_ATTR_MAX + 1] = {
	[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AP_MLD_ADDRESS] = { .type = NLA_BINARY, .len = ETH_ALEN },
};

static int
mtk_cfg80211_vndr_set_ap_mld_addr(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct nlattr *data)
{
	...
	ret = nla_parse_nested(tb, MTK_NL80211_VENDOR_ATTR_AP_MLD_ATTR_MAX,
		(struct nlattr *)data, SUBCMD_SET_AP_MLD_ADDRESS_POLICY, NULL);

	if (ret) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"mld addr parse failed, ret:%d\n", ret);
		return -EINVAL;
	}

	if (!tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX] || !tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_ADDRESS]) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"mld group index or addr not exist\n");
		return -EINVAL;
	}

	mld_addr_len = nla_len(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_ADDRESS]);
	mld_addr = nla_data(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_ADDRESS]);
	mld_grp_idx = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX]);

	if (!mld_addr || mld_addr_len != MAC_ADDR_LEN) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"invalid mld_addr:%p, mld_addr_len:%u!!\n", mld_addr, mld_addr_len);
		return NDIS_STATUS_FAILURE;
	}

	ret = eht_ap_mld_set_addr(wdev, mld_grp_idx, mld_addr);
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"mld addr set failed, mld_grp_idx:%u!!\n", mld_grp_idx);
	}
...
static const struct nla_policy
SUBCMD_SET_AP_MLD_EML_MODE_POLICY[MTK_NL80211_VENDOR_ATTR_AP_MLD_ATTR_MAX + 1] = {
	[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_AP_MLD_EMLMR] = { .type = NLA_FLAG },
	[MTK_NL80211_VENDOR_ATTR_AP_MLD_EMLSR] = { .type = NLA_FLAG },
};

static int
mtk_cfg80211_vndr_set_ap_mld_eml_mode(
	struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev,
	struct nlattr *data)
{
	...
	ret = nla_parse_nested(tb, MTK_NL80211_VENDOR_ATTR_AP_MLD_ATTR_MAX,
		(struct nlattr *)data, SUBCMD_SET_AP_MLD_EML_MODE_POLICY, NULL);

	if (ret) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"eml mode parse failed, ret:%d\n", ret);
		return -EINVAL;
	}

	if (!tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX]) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"mld group index not exist\n");
		return -EINVAL;
	}

	mld_grp_idx = nla_get_u8(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_INDEX]);
	op_ctrl.pres_bmap |= MLD_OP_PARAM_PRES_GROUP_IDX;
	op_ctrl.mld_grp = mld_grp_idx;

	if (nla_get_flag(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_EMLMR]))
		op_ctrl.mld_param.eml_mode = EMLSR;
	else if (nla_get_flag(tb[MTK_NL80211_VENDOR_ATTR_AP_MLD_EMLSR]))
		op_ctrl.mld_param.eml_mode = EMLMR;
	else
		op_ctrl.mld_param.eml_mode = EML_DISABLE;
	op_ctrl.pres_bmap |= MLD_OP_PARAM_PRES_EML_MODE;

	ret = eht_ap_mld_attr_set(wdev, mld_grp_idx, &op_ctrl);
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"mld attr set failed, mld_grp_idx:%u!!\n", mld_grp_idx);
	}
...
static const struct nla_policy
SUBCMD_SET_AP_MLD_POLICY[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ATTR_MAX + 1] = {
	[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_INDEX] = { .type = NLA_U8 },
	[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ADDRESS] = { .type = NLA_NESTED },
	[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_EML_MODE] = { .type = NLA_NESTED },
};

static int
mtk_cfg80211_vndr_set_ap_mld(
	struct wiphy *wiphy,
...
{
	struct _RTMP_ADAPTER *pAd;
	struct nlattr *tb[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ATTR_MAX + 1];
	int ret;
	struct nlattr *attr;
	struct net_device *dev;
	struct wifi_dev *wdev;

	if (!wiphy || !wifi_dev || !data)
		return -EINVAL;
	...
	ret = nla_parse(tb, MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ATTR_MAX,
		(struct nlattr *)data, len, SUBCMD_SET_AP_MLD_POLICY, NULL);

	if (ret) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
			"set ap mld parse failed, ret:%d\n", ret);
		return ret;
	}

	attr = tb[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_INDEX];
	if (attr) {
		ret = mtk_cfg80211_vndr_set_ap_mld_index(
			pAd, wdev, attr);
		if (ret) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"set ap mld index failed, ret:%d\n", ret);
			return ret;
		}
	}

	attr = tb[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ADDRESS];
	if (attr) {
		ret = mtk_cfg80211_vndr_set_ap_mld_addr(
			pAd, wdev, attr);
		if (ret) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"set ap mld index failed, ret:%d\n", ret);
	...
	attr = tb[MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_EML_MODE];
	if (attr) {
		ret = mtk_cfg80211_vndr_set_ap_mld_eml_mode(
			pAd, wdev, attr);
		if (ret) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_WARN,
				"set ap mld eml_mode failed, ret:%d\n", ret);
			return ret;
		}
	}
...
static const struct wiphy_vendor_command mtk_vendor_commands[] = {
...
#ifdef DOT11_EHT_BE
	{
		.info = {
			.vendor_id = MTK_OUI,
			.subcmd = MTK_NL80211_VENDOR_SUBCMD_SET_AP_MLD,
		},
		.doit = mtk_cfg80211_vndr_set_ap_mld,
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
			 WIPHY_VENDOR_CMD_NEED_NETDEV,
#if KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE
		.policy = SUBCMD_SET_AP_MLD_POLICY,
		.maxattr = MTK_NL80211_VENDOR_ATTR_SET_AP_MLD_ATTR_MAX,
#endif
	},
#endif /* DOT11_EHT_BE */
};
...
static const struct nl80211_vendor_cmd_info mtk_vendor_events[] = {
	...
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_DETECTED,
	},
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_CAC_FINISHED,
	},
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_CAC_ABORTED,
	},
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_NOP_FINISHED,
	},
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_PRE_CAC_EXPIRED,
	},
	{
		.vendor_id = MTK_OUI,
		.subcmd = MTK_NL80211_VENDOR_EVENT_RADAR_CAC_STARTED,
	},
...
struct cfg80211_ops CFG80211_Ops = {
	...
	.get_tx_power               = CFG80211_GetTxPower,
};
	...
	IN struct net_device *pNetDev)
{
	...
	wiphy_ext_feature_set(pCfg80211_CB->pCfg80211_Wdev->wiphy, NL80211_EXT_FEATURE_RADAR_BACKGROUND);
#endif
