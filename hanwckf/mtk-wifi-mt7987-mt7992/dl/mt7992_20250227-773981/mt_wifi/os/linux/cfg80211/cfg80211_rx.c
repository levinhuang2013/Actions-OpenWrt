	...
	UINT32 freq)
{
	...
	rssi.AvgRssi[0] = pRxBlk->rx_signal.raw_rssi[0];
	rssi.AvgRssi[1] = pRxBlk->rx_signal.raw_rssi[1];
	rssi.AvgRssi[2] = pRxBlk->rx_signal.raw_rssi[2];
	rssi.AvgRssi[3] = pRxBlk->rx_signal.raw_rssi[3];
	real_rssi = RTMPAvgRssi(pAd, &rssi);
	...
	if (IS_BROADCAST_MAC_ADDR(pRxBlk->Addr1)) {
...
#endif
		{
			...
			CFG80211OS_RxMgmt(wdev->if_dev, freq,
				pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, real_rssi);
		}
#if defined(CCN67_BS_SUPPORT) || defined(BAND_STEERING)
		os_free_mem(ProbeReqParam);
#endif /* BAND_STEERING */
		...
		CFG80211OS_RxMgmt(pNetDev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, real_rssi);
...
BOOLEAN CFG80211_CheckSAQueryNeed(
	IN struct _RTMP_ADAPTER *pAd,
	IN struct wifi_dev *wdev,
	IN struct _AUTH_FRAME_INFO *auth_info)
{
	...
	if ((pEntry->SecConfig.PmfCfg.UsePMFConnect == TRUE)
		...
		NDIS_STATUS NStatus;
		USHORT Alg = auth_info->auth_alg;
		USHORT Seq = auth_info->auth_seq + 1;
		USHORT StatusCode = MLME_ASSOC_REJ_TEMPORARILY;
		...
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_NOTICE,
			...
			MAC2STR(auth_info->addr2), Alg, auth_info->auth_seq, auth_info->auth_status);
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_NOTICE,
			"send auth resp to "MACSTR" (alg=%d, seq=%d, status=%d)...\n",
			MAC2STR(auth_info->addr2), Alg, Seq, MLME_ASSOC_REJ_TEMPORARILY);

		MgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, auth_info->addr2,
						 auth_info->addr1, auth_info->addr1);
		...
		if (HAS_EHT_MLD_EXIST(auth_info->cmm_ies.ie_exists))
			FrameLen += eht_add_auth_rsp_ies(
					wdev, pOutBuffer, FrameLen, &auth_info->cmm_ies);
#endif /* DOT11_EHT_BE */
		...
		if (pEntry->SecConfig.PmfCfg.SAQueryStatus == SAQ_IDLE) {
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_ERROR,
				"[PMF] : need to trigger SA Query\n");
			PMF_MlmeSAQueryReq(pAd, pEntry);
		} else if ((pEntry->SecConfig.PmfCfg.SAQueryStatus == SAQ_RETRY)
				|| (pEntry->SecConfig.PmfCfg.SAQueryStatus == SAQ_SENDING)) {
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_INFO,
				"[PMF] : in SA Query process...\n");
		}
		return TRUE;
	}
	return FALSE;
...
/*
	Return:
		TRUE -  1. forward frames to upper layer
				2. no forward since it is invalid
		FALSE - forward frames to driver state machine
*/
	...
	UINT32 freq)
{
	...
	if (wdev && (wdev->wdev_type == WDEV_TYPE_STA)) {
		...
		if (IS_WPA_SUPPLICANT_UP(pStaCfg) &&
			...
			CFG80211OS_RxMgmt(wdev->if_dev, freq,
				pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
	...
	/* check if auth req is valid, if not, trigger SA Query */
	if (wdev && CFG80211_CheckSAQueryNeed(pAd, wdev, auth_info))
		return TRUE;

	if (wdev && ((auth_info->auth_alg == AUTH_MODE_FT)
		|| (auth_info->auth_alg == AUTH_MODE_SAE)
	)) {
		...
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_NOTICE,
			...
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_AUTH, DBG_LVL_INFO,
				"auth_alg=%d, AKMMap=0x%x\n", auth_info->auth_alg, wdev->SecConfig.AKMMap);
		CFG80211_AuthReqHandler(pAd, pRxBlk, (void *)pRxBlk->FC, auth_info);
		if (auth_info->auth_alg == AUTH_MODE_SAE
		)
			CFG80211OS_RxMgmt(pNetDev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
		return TRUE;
	}
	...
	if (wdev &&
		...
		CFG80211OS_RxMgmt(
			wdev->if_dev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
#endif
	return FALSE;
}
	...
	UINT32 freq)
{
	...
	if (wdev) {
		CFG80211OS_RxMgmt(wdev->if_dev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
	} else {
		CFG80211OS_RxMgmt(pNetDev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
...
BOOLEAN CFG80211_HandleMgmtFrame(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR OpMode)
{
	...
	freq = channel_to_frequency(pAd->BandSel, pRxBlk->channel_freq);
	...
	if ((pHeader->FC.SubType == SUBTYPE_PROBE_REQ) &&
		...
		if (pWdev) {
			CFG80211OS_RxMgmt(
				pWdev->if_dev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
		} else {
			CFG80211OS_RxMgmt(pNetDev, freq, pRxBlk->pData, pRxBlk->MPDUtotalByteCnt, 0);
		}
	}
