	...
	if (ScanType == SCAN_PARTIAL) {
		...
		if (ScanCtrl->PartialScan.bScanning) {
			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_ERROR,
					"partial scan is under process cannot trigger new scan\n");
			return;
		}
		...
		if (Ssid.SsidLength) {
			...
			NdisMoveMemory(ScanCtrl->Ssid,  pScan->pSsid, Ssid.SsidLength);
