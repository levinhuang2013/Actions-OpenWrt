...
VOID CFG80211_AuthRespHandler(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	if (pAd->RoamCalibEnable)
		RoamCalibData(pAd, mgmt->da, NULL, "Send Auth Resp to STA", __func__);
	...
	if (mgmt->u.auth.status_code != 0 && mgmt->u.auth.status_code != 126 &&
			mgmt->u.auth.status_code != 127)
		record_connect_info(pAd, mgmt->da, mgmt->u.auth.status_code, TRUE);
...
VOID CFG80211_AssocRespHandler(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
...
#ifdef ROAM_CALIB_SUPPORT
		if (pAd->RoamCalibEnable)
			RoamCalibData(pAd, mgmt->da, NULL, "Send Re-Assoc Resp", __func__);
#endif /* ROAM_CALIB_SUPPORT */
...
#ifdef ROAM_CALIB_SUPPORT
		if (pAd->RoamCalibEnable)
			RoamCalibData(pAd, mgmt->da, NULL, "Send Assoc Resp", __func__);
#endif /* ROAM_CALIB_SUPPORT */
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
	...
	if (mgmt->u.auth.status_code)
		record_connect_info(pAd, mgmt->da, mgmt->u.assoc_resp.status_code, TRUE);
	pMbss = &pAd->ApCfg.MBSSID[apidx];
	wdev = &pMbss->wdev;
	if (!wdev) {
...
NDIS_STATUS CFG80211_LEGACY_BtmReqHandler(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	while (pPayload->variable[i] == IE_RRM_NEIGHBOR_REP) {
		...
		/* Report AP has same authenticator as the AP. */
...
#endif /*DOT11_HE_AX*/
		pNei_payload->bssid_info = cpu2le32(BssidInfo.word);
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_NOTICE,
		...
		if ((ieee80211_is_auth(mgmt->frame_control)) && (mgmt->u.auth.auth_alg != AUTH_MODE_FT) &&
			...
			if (pEntry != NULL && pEntry->wdev->radius_mac_auth_enable) {
				if (mgmt->u.auth.status_code == MLME_SUCCESS) {
					...
					MlmeDeAuthAction(pAd, pEntry, REASON_PREV_AUTH_INVALID, FALSE);
				}
			}
#endif
			goto LabelOK;
