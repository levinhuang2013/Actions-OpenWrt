	...
	for (i = 0, macptr = rstrtok(Buffer, ";"); macptr;
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if ((mld_grp < BMGR_MAX_MLD_GRP_CNT) && (mld_grp != BMGR_INVALID_MLD_GRP)) {
				...
				wlan_config_set_crit_upd_mode(&pMbss->wdev, CRIT_UPD_MODE_FLAG_FW_CMD_ONLY);
				MTWF_PRINT("BSS%d mld_grp_idx=%d\n",
					i, pMbss->mld_grp_idx);
			} else
	...
	IN char *pBuffer)
{
	...
	if (RTMPGetKeyParameter("EHT_ApCritUpdMode", tmpbuf,
			MAX_PARAMETER_LEN, pBuffer, TRUE)) {
		...
		for (idx = 0, macptr = rstrtok(tmpbuf, ";");
			...
			wdev = &pAd->ApCfg.MBSSID[idx].wdev;
			if (kstrtol(macptr, 10, &value))
				MTWF_PRINT("Line.%d - AP Band%d(MBSS%d): CuMode error input(=%s)\n",
					__LINE__,
					band_idx,
					idx,
					macptr);
