...
void rtmp_read_ackcts_timeout_parms_from_file(
	...
	pAd->CommonCfg.cck_timeout = DEFAULT_TMAC_CDTR_VALUE;
	pAd->CommonCfg.ofdm_timeout = DEFAULT_TMAC_ODTR_VALUE;
	pAd->CommonCfg.ofdma_timeout = DEFAULT_TMAC_OMDTR_VALUE;
	...
	IN RTMP_STRING *pBuffer)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
		"CACMemoEn= %d, ChCtrlChListStat= %d, WMODE_CAP_5G %d\n",
	...
	do {
		...
		if (RTMPGetKeyParameter("ApcliPartialScan", tmpbuf, 128, pBuffer, TRUE)) {
			UINT_8 ApcliPartialScan = os_str_tol(tmpbuf, 0, 10);

			if (ApcliPartialScan)
				pAd->CommonCfg.ApcliPartialScan = TRUE;
			else
				pAd->CommonCfg.ApcliPartialScan = FALSE;
		}
		...
		if (RTMPGetKeyParameter("ACSIceChUtilThr", tmpbuf, 10, pBuffer, TRUE))
			acs_set_ch_ice_ch_util_thr(pAd, tmpbuf);
		if (RTMPGetKeyParameter("ACSMaxACSTimes", tmpbuf, 10, pBuffer, TRUE))
			acs_set_max_acs_times(pAd, tmpbuf);
		if (RTMPGetKeyParameter("ACSStaNumThr", tmpbuf, 10, pBuffer, TRUE))
			acs_set_sta_num_thr(pAd, tmpbuf);
		if (RTMPGetKeyParameter("ACSSwChThr", tmpbuf, 10, pBuffer, TRUE))
			acs_set_switch_thr(pAd, tmpbuf);
#ifdef CONFIG_6G_SUPPORT
	...
	/******Intlgnt ACS******/
	/*get Performance evaluation aspects*/
	if (RTMPGetKeyParameter("ACSTXPowerCons", tmpbuf, 10, pBuffer, TRUE)) {
		/*1-cnsdr, 0-not cnsdr*/
		...
		if (HcGetAutoChCtrl(pAd)) {
			...
			if (pAutoChSelCtrl) {
				pAutoChSelCtrl->TXPowerCons = os_str_tol(tmpbuf, 0, 10);
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_PROFILE, DBG_LVL_NOTICE,
					"ACSTXPowerCons%d\n",
					pAutoChSelCtrl->TXPowerCons);
			} else {
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
					"(caller:%pS), pAutoChSelCtrl is NULL.\n",
					OS_TRACE);
			}
		} else {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_ERROR,
					"(caller:%pS), AutoChSelCtrl is NULL.\n",
					OS_TRACE);
		}
	}

	/*Intlgnt ACS ratios*/
	if (RTMPGetKeyParameter("ACSDataRateWt", tmpbuf, 10, pBuffer, TRUE)) {
		/*1-cnsdr, 0-not cnsdr*/
		struct _AUTOCH_SEL_CTRL *pAutoChSelCtrl = NULL;
		...
		if (HcGetAutoChCtrl(pAd)) {
			pAutoChSelCtrl = &HcGetAutoChCtrl(pAd)->AutoChSelCtrl;
			if (pAutoChSelCtrl) {
				pAutoChSelCtrl->DataRateWeight = os_str_tol(tmpbuf, 0, 10);
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_PROFILE, DBG_LVL_NOTICE,
					"ACSDataRateWeight=%d\n",
	...
	if (RTMPGetKeyParameter("ACSPrioWt", tmpbuf, 10, pBuffer, TRUE)) {
		/*1-cnsdr, 0-not cnsdr*/
		struct _AUTOCH_SEL_CTRL *pAutoChSelCtrl = NULL;
		...
		if (HcGetAutoChCtrl(pAd)) {
			pAutoChSelCtrl = &HcGetAutoChCtrl(pAd)->AutoChSelCtrl;
			if (pAutoChSelCtrl) {
				pAutoChSelCtrl->PrioWeight = os_str_tol(tmpbuf, 0, 10);
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_PROFILE, DBG_LVL_NOTICE,
					"ACSPrioWt=%d\n",
	...
	if (RTMPGetKeyParameter("bhbss", tmpbuf, 32, pBuffer, FALSE)) {
		struct wifi_dev *wdev = NULL;

		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr; macptr = rstrtok(NULL, ";"), i++) {
			if (i < pAd->ApCfg.BssidNum) {
				UCHAR bhbss_enable = os_str_tol(macptr, 0, 10);
	...
	if (RTMPGetKeyParameter("fhbss", tmpbuf, 32, pBuffer, FALSE)) {
		struct wifi_dev *wdev = NULL;

		for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr; macptr = rstrtok(NULL, ";"), i++) {
			if (i < pAd->ApCfg.BssidNum) {
				UCHAR fhbss_enable = os_str_tol(macptr, 0, 10);
