...
UCHAR vht_cent_ch_freq_40mhz(struct wifi_dev *wdev, UCHAR prim_ch, UCHAR vht_bw, UCHAR ch_band)
{
	...
	if (vht_bw == VHT_BW_2040) {
		...
		else if (ch_band == CMD_CH_BAND_24G) {
			cen_ch = &prim_ch;
			op_ext_cha = wlan_operate_get_ext_cha(wdev);
			op_bw = wlan_operate_get_ht_bw(wdev);

			if (!op_ext_cha || !op_bw)
				return *cen_ch;
			if (cal_ht_cent_ch(prim_ch, op_bw, op_ext_cha, cen_ch) == TRUE)
				return *cen_ch;
		}
	}
...
INT build_vht_oper_ie(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR Channel, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	case VHT_BW_2040:
		...
		if (wlan_operate_get_bw(wdev) == BW_40)  // BW40
			vht_op.vht_op_info.ccfs_0 = vht_cent_ch_freq_40mhz(wdev, wdev->channel, VHT_BW_2040, ch_band);
		else
...
INT build_vht_ies(RTMP_ADAPTER *pAd, struct _build_ie_info *info)
{
	...
	if (
		...
		if (info->frame_subtype == SUBTYPE_ASSOC_RSP && info->pEntry) {
			VHT_CAP_IE *vht_cap = (VHT_CAP_IE *)(info->frame_buf + 2);
			UINT32 ap_txpath = pAd->Antenna.field.TxPath;
			UINT32 sta_bfee_sts_cap = info->pEntry->vht_cap_ie.vht_cap.bfee_sts_cap;

			vht_cap->vht_cap.num_snd_dimension = min(ap_txpath - 1, sta_bfee_sts_cap);
		}
