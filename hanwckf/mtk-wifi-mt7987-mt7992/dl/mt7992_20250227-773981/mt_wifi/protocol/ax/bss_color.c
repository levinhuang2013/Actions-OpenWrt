...
static void bss_color_trigger_collision(struct wifi_dev *wdev, UINT8 OP_MODE)
{
	...
	if (OP_MODE == BSS_COLOR_OPMODE_BAND) {
		...
		if (pAd->ApCfg.bss_color_cfg.collision_ignore == TRUE)
			return;
	...
	if (OP_MODE == BSS_COLOR_OPMODE_SINGLE) {
		...
		if (bss_color->collision_ignore == TRUE)
			return;
...
void bss_color_init(struct wifi_dev *wdev, struct _BSS_INFO_ARGUMENT_T *bssinfo)
{
	...
	bss_color->collision_ignore = FALSE;
...
void show_bss_color_info(struct _RTMP_ADAPTER *ad)
{
	...
	if (!ad) {
		MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_ERROR, "ad is NULL!!!\n");
		return;
	}
	...
	for (idx = 0; idx < WDEV_NUM_MAX; idx++) {
		...
		if (wdev->if_up_down_state &&
			...
			if (wdev->wdev_type == WDEV_TYPE_AP) {
				...
				band_idx = HcGetBandByWdev(wdev);
				if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx))
					ignore = ad->ApCfg.bss_color_cfg.collision_ignore;
				else
					ignore = bss_color->collision_ignore;
			}
#endif
#ifdef CONFIG_STA_SUPPORT
			if (wdev->wdev_type == WDEV_TYPE_STA) {
			...
			get_bss_color_bitmap(wdev, bitmap);
			MTWF_PRINT("wdev_idx type dis color ignore next ");
			MTWF_PRINT("collision running bitmap\n");

			MTWF_PRINT("-------- ---- --- ----- ------ ---- ");
			MTWF_PRINT("--------- ------- ------------------\n");

			MTWF_PRINT("%8d %4d %3d 0x%02x %6d %4d ",
					 idx, wdev->wdev_type,
					 bss_color->disabled, bss_color->color,
					 ignore, bss_color->next_color);
			MTWF_PRINT("%9d %7d 0x%02x%02x%02x%02x%02x%02x%02x%02x\n",
	...
	if (wdev->if_up_down_state &&
		...
		case BSS_COLOR_DBG_IGNORE_COLLISION:
			...
			if (value == 0 || value == 1) {
				...
				UINT8 band_idx = 0;
#endif
				BOOLEAN ignore = (value == 1) ? TRUE : FALSE;

#ifdef CONFIG_AP_SUPPORT
				if (wdev->wdev_type == WDEV_TYPE_AP) {
					band_idx = HcGetBandByWdev(wdev);
					if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx))
						ad->ApCfg.bss_color_cfg.collision_ignore = ignore;
					else
						bss_color->collision_ignore = ignore;
				}
#endif
#ifdef CONFIG_STA_SUPPORT
				if (wdev->wdev_type == WDEV_TYPE_STA)
