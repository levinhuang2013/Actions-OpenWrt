...
void wlan_config_set_crit_upd_mode(
	struct wifi_dev *wdev,
	u8 crit_upd_mode)
{
	struct wlan_config *cfg = (struct wlan_config *)wdev->wpf_cfg;
	...
	cfg->eht_conf.crit_upd_mode = crit_upd_mode;
	/*
	MTWF_PRINT("%s: cfg crit_upd_mode = %d\n",
		__func__,
		cfg->eht_conf.crit_upd_mode);
	*/
}
...
u8 wlan_config_get_crit_upd_mode(
	struct wifi_dev *wdev)
{
	struct wlan_config *cfg = (struct wlan_config *)wdev->wpf_cfg;
	...
	return cfg->eht_conf.crit_upd_mode;
}
