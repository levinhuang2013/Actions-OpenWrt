	...
	if (wdev->wdev_type == WDEV_TYPE_STA) {
		...
		if (ml_type == BMGR_QUERY_ML_IE_BCN) {
			for (repted_link = 0; repted_link < BSS_MNGR_MAX_BAND_NUM; repted_link++) {
				if (repted_link < ARRAY_SIZE(mld_query.ie.csa_ie.csa_offset_in_ml_ie)) {
					wdev->bcn_buf.csa.csa_ie_offset_in_ml[repted_link] = mld_query.ie.csa_ie.csa_offset_in_ml_ie[repted_link];
					wdev->bcn_buf.csa.csa_bss_idx[repted_link] = mld_query.ie.csa_ie.csa_bss_idx[repted_link];
				}
			}
		}
	}
	...
	struct wifi_dev *wdev, UINT8 *f_buf)
{
	...
	SET_DOT11BE_MAC_CAP_AMPDU_LEN_EXP(cap, 1);
...
static BOOLEAN eht_ap_mlo_is_valid_sta_profile_ie(UINT8 id, UINT8 id_ext)
{
	...
	/* list of IEs that appear multiple times: vendor IE */
...
#define get_bit(x) (1ULL << (x & 63))	/* bitmask (2^n)-1, n=6 */
	...
	UINT16 num_repted_vendor_ie = 0, num_repting_vendor_ie = 0;
	...
	while ((UINT8 *)repted_eid < (repted_frame_ie->buf + repted_frame_ie->buf_len)) {
		...
		if (eht_ap_mlo_is_valid_sta_profile_ie(repted_eid->Eid, *extension_id)) {
			...
			if (repted_eid->Eid == IE_VENDOR_SPECIFIC)
				num_repted_vendor_ie++;
	...
	while ((UINT8 *)repting_eid < (repting_frame_ie->buf + repting_frame_ie->buf_len)) {
		...
		if (eht_ap_mlo_is_valid_sta_profile_ie(repting_eid->Eid, *extension_id)) {
			...
			if (repting_eid->Eid == IE_VENDOR_SPECIFIC)
				num_repting_vendor_ie++;
	...
	/* Step 4. handle IEs that appear multiple times */
	if (num_repted_vendor_ie) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_CONN, DBG_LVL_DEBUG,
			"Step 4. Append %d Vendor IEs to profile\n", num_repted_vendor_ie);
	...
	/* Step 5. append non-inheritance element */
	...
	UINT8 *ie_head, struct common_ies *cmm_ies, struct frag_ie_info *frag_info)
{
	...
	case EID_EXT_EHT_OP:
		...
		break;
	case EID_EXT_EHT_TID2LNK_MAP:
		SET_EHT_ML_T2LM_EXIST(cmm_ies->ie_exists);
		break;
