	...
	case OID_802_11_CONFIGURATION:
		if (wrq->u.data.length != sizeof(NDIS_802_11_CONFIGURATION))
			...
			if (pConfig != NULL) {
				...
				wdev->channel = frequency_to_channel(pConfig->DSConfig);
				/* */
	...
	IN struct wifi_dev *wdev)
{
	...
	case OID_802_11_BSSID_LIST:
		...
		for (i = 0; i < ScanTab->BssNr; i++) {
			...
			Freq = channel_to_frequency(pAd->BandSel, ScanTab->BssEntry[i].Channel);
			Freq = Freq * 1000;
	...
	case OID_802_11_CONFIGURATION:
		...
		if (pConfiguration) {
			...
			Freq = channel_to_frequency(pAd->BandSel, wdev->channel);
			Freq = Freq * 1000;
	...
	IN	ULONG					Data)
{
	...
	m = channel_to_frequency(pAd->BandSel, ch);
	m = m * 1000;
	*(ULONG *)pData = (ULONG)m;
	return NDIS_STATUS_SUCCESS;
}
	...
	IN  USHORT                  priv_flags)
{
	...
	case (CMD_RTPRIV_IOCTL_STATION)CMD_RTPRIV_IOCTL_CHID_2_FREQ:
		*(UINT32 *)pData = channel_to_frequency(pAd->BandSel, Data);
		*(UINT32 *)pData = (*(UINT32 *)pData) * 1000;
		break;

	case (CMD_RTPRIV_IOCTL_STATION)CMD_RTPRIV_IOCTL_FREQ_2_CHID:
		*(UINT32 *)pData = frequency_to_channel(Data);
		break;
