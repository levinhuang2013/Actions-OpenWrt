...
int get_macaddr(lua_State *L)
{
	...
	if(snprintf(if_hw, 18, "%02X:%02X:%02X:%02X:%02X:%02X",
			(ptr[0] & 0377), (ptr[1] & 0377), (ptr[2] & 0377),
			(ptr[3] & 0377), (ptr[4] & 0377), (ptr[5] & 0377)) < 0){
		return -1;
	}
	...
	for (i = 0; i < len && (pBufLimit - pOut) >=7; i++) { // 6(maximum length of a HTML entity) + 1(null character) = 7
		...
		default:
			if ((str[i]>=0) && (str[i]<=31)) {
				...
				if(snprintf(pOut, BUF_SIZE - 1, "&#%02d;", str[i])<0){
					break;
				}
				pOut += 5;
				...
				//Device Control Characters
				if(snprintf(pOut, BUF_SIZE - 1, "&#%03d;", str[i])<0){
					break;
				}
				pOut += 6;
			} else {
